...
Property (1st order) If $f$ is differentiable, then $f$ is convex iff $f(z) \geq f(x)+(z-x)^{\prime} \nabla f(x), \ \forall x, z \in C .$
...
Linear Regression
\end{example}
$$\text{minimize } f(\mathbf{w})=\frac{1}{2}\left\|\mathbf{X}^{T} \mathbf{w}-\mathbf{y}\right\|^{2}$ subject to $\quad \mathbf{w} \in \mathbb{R}^{d}$$
$n$ data points, $d$ features
- X may be wide (under-determined), tall (over-determined), or rank-deficient
