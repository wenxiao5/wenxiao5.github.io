...
If $f$ is a continuous function on a lompact

set, $s \subseteq \mathbb{R}^{n}$, then $f$ attains its min and mox

$$
\text { on \&, ie, } \begin{aligned}
\exists x, \epsilon \mathcal{L} \text { s.t. } f\left(x_{1}\right) &=\inf _{x \in \&} f(x) \\
\exists x_{2} \in \mathcal{L} \text { s.t. } f\left(x_{2}\right) &=\sup _{x \in \mathbb{S}} f(x)
\end{aligned}
$$

Proof (for existence of $\min$; $m n$ is similar)

$$
\text { Let } \begin{aligned}
&\left\{z_{k}\right\} \subseteq \mathcal{L} \text { be s.t. } \\
& \inf f(x) \leqslant f\left(z_{k}\right) \leqslant \inf _{x \in s} f(x)+\frac{1}{k}
\end{aligned}
$$\[
\begin{array}{l}
$$x \in \&$$
...
$$
\text { Then } \lim _{k \rightarrow \infty} f\left(z_{k}\right)=\sin f(x)
$$\[
\begin{array}{l}
$$\lim _{k \rightarrow \infty} x \in \mathcal{L}$$
\end{array}
\]

$\mathcal{L}$ is bounded $\Rightarrow\left\{z_{k}\right\}$ has it least one limit point $x$,

$\mathcal{L}$ is closed $\Rightarrow x_{1} \in \mathcal{1}$

$f$ is continuous $\Rightarrow f\left(x_{1}\right)=\lim _{k \rightarrow \infty} f\left(z_{k}\right)=\inf _{x \in s} f(x)$
...
$$
\begin{aligned}
&\text { Square Matrices } A_{n \times n} \\
&\text { - } A \text { is singular if } \operatorname{det}(A)=0 \text {, else non-smigular }
\end{aligned}
$$

$$
\text { - If } \operatorname{det}(A) \neq 0, A^{-1} \text { exists and }=\frac{1}{\operatorname{det}(A)} \text { adj }(A)
$$\[
\begin{array}{l}
$$\text { - If } A, B \text { are } n \times n \text { matrices, }$$ \\
$$\operatorname{det}(A B)=\operatorname{det}(A) \operatorname{det}(B)=\operatorname{det}(B A)$$
\end{array}
\]
