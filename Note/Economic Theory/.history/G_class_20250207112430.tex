...
The optimal strategy is given by a critical cost $c^*$ such that the agent do the task if and only if $c\leq c^*$. Then, the expected total cost $v$ follows
\begin{equation}
    \begin{aligned}
        v=L+c^*\underbrace{\frac{c^*}{2}}_\textnormal{average cost conditional on $c\leq c^*$}+\delta(1-c^*)v
    \end{aligned}
    \nonumber
\end{equation}
which gives us
