...
\begin{proposition}[HJB of Stochastic Optimal Control]
    ...
    \end{equation}
\end{proposition}
(In Multivariate Case: $\rho v(x) = \max_{\alpha\in A}\ r(x,\alpha) + \sum_{i=1}^N \frac{\partial v(x)}{\partial x_i} f_i(x,\alpha) + \frac{1}{2}\sum_{i=1}^N\sum_{j=1}^N \frac{\partial^2 v(x)}{\partial x_i \partial x_j} \sigma_{ij}^2(x)$)
...
\begin{lemma}[Ito's Lemma]
    ...
    \begin{equation}
        \begin{aligned}
            d f(x)=\left(f'(x)\mu(x)+\frac{1}{2}f''(x)\sigma^2(x)\right)dt+\sigma(x)f'(x)dW
        \end{aligned}
        \nonumber
    \end{equation}
\end{lemma}
