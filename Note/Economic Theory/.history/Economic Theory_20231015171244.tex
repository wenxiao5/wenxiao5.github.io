...
\begin{definition}[Revealed Preferred]
    ...
    We say $x^t$ is
    \begin{enumerate}[-]
        \item $x^t R^D x$: \underline{\textbf{directly} revealed preferred} to $x$, if $p^t x^t \geq p^t x$;
        \item $x^t P^D x$: \underline{\textbf{strictly directly} revealed preferred} to $x$, if $p^t x^t > p^t x$;
        \item $x^t R x$: \underline{\textbf{indirectly} revealed preferred} to $x$, if $\exists$ a sequence $\{x_k\}_{k=1}^K$ with $x_1=x^t$ and $x_K=x$ such that $x^k R^D x^{k+1}$ for all $k=1,...,K-1$.
    \end{enumerate}
\end{definition}

\begin{definition}[Generalized Axiom of Revealed Preference (GARP)]
    \normalfont
    Consider two observation $(p^t,x^t)$ and $(p^s,x^s)$, 
\end{definition}
