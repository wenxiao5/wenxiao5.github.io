...
\begin{definition}[Majorization]
    \normalfont
    Consider right-continuous functions that map the unit interval $[0,1]$ into the real numbers. For two non-decreasing functions $f,g \in L^1$, we say that $f$ \textbf{majorizes} $g$, denoted by $g \prec f$, if the following two conditions hold:
    \begin{enumerate}
        \item $$\int_x^1 g(s)ds\leq \int_x^1 f(s)ds,\forall x\in [0,1]$$
        \item $$\int_0^1 g(s)ds=\int_0^1 f(s)ds$$
    \end{enumerate}
\end{definition}

\begin{definition}[Weak Majorization]
    \normalfont
    $f$ \textbf{weakly majorizes} $g$, denoted by $g \prec_w f$, if the first condition above holds (but not necessarily the second).
\end{definition}
