...
\end{definition}
\begin{proposition}
    If rational $\succeq$ is convex, then $\exists$ a \underline{quasi-concave} $u(\cdot)$ that represents $\succeq$, i.e., $\{y\in X: u(y)\geq u(x)\}$ is convex.
\end{proposition}
...
\begin{proposition}
    If $p>>0$ and $u$ is continuous, then \ref{UMP} has a solution.
\end{proposition}
