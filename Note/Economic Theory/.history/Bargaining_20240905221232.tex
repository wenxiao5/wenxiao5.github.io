...
\begin{equation}
    \begin{aligned}
        d f(x)=\left(\sum_{i=1}^N\frac{\partial f(x)}{\partial x_i}\mu_i(x)+\frac{1}{2}\sum_{i=1}^N\sum_{j=1}^N \frac{\partial^2 f(x)}{\partial x_i \partial x_j}\sigma_{ij}^2(x)\right)dt+\sigma(x)f'(x)dW
    \end{aligned}
    \nonumber
\end{equation}
