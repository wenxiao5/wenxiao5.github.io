...
\textit{Individual Choice}: Let's consider a simple model, where the cost of search for each price is $c$ and a consumer buys one or zero unit.
...
\end{definition}
The $R$ is constructed as the critical value that the expected return from an extra search equals to the marginal cost:
\begin{equation}
    \begin{aligned}
