...
\begin{definition}[Strategy-Proof, DSIC]
    \normalfont
    $f$ is \textbf{strategy-proof} (also called dominant-strategy-incentive-compatible, \textbf{DSIC}) if
    \begin{center}
        ``$s^*_i(\theta_i)=\theta_i$ (reveal agent $i$'s true type) for all $\theta_i\in\Theta_i$ and all $i\in I$"
    \end{center}
    is a dominant strategy (D-S) equilibrium of the direct mechanism $\Gamma=(\Theta,f(\cdot))$.
\end{definition}
