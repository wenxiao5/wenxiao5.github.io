...
\begin{definition}[Strong Weak Axiom]
    \normalfont
    The function $Z: \mathbb{R}_{++}^L \rightarrow \mathbb{R}^L$ satisfies the \underline{strong weak axiom} if for any $\bar{p}\in \mathbb{R}_{++}^L$ s.t. $Z(\bar{p})=0$ and $p\in \mathbb{R}^L_{++}$ s.t. $p\neq \alpha\bar{p}, \forall \alpha>0$, $$\bar{p}\cdot Z(p)>0$$
\end{definition}
...
A general auction mechanism: bidders have values $x$ and generate message (bids) $\beta: \mathcal{X}_i \rightarrow \mathcal{B}$ based on their values, then there is an allocation rule $\pi: \mathcal{B} \rightarrow \Delta N$ generates a distribution over all bidders and a payment rule $\mu: \mathcal{B} \rightarrow \mathbb{R}^N$ generates payment for all bidders.
