...
\begin{theorem}[Bondareva-Shapley Theorem]
    The core of $\left<N,v\right>$ is non-empty ($C(v,N)\neq \emptyset$) if and only if for every function $\alpha: 2^N\backslash\{\emptyset\} \rightarrow [0,1]$ where $\forall i\in N: \sum_{S:i\in S}\alpha(S)=1$, the following condition holds:
    \begin{equation}
        \begin{aligned}
