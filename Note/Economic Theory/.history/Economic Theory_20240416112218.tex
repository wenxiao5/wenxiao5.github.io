...
\normalfont
    ...
    \begin{equation}
        \begin{aligned}
            t_i(\theta_i)=\mathbb{E}_{\tilde{\theta}_{-i}}\left[\sum_{j\neq i}v_j(k^*(\theta_i,\tilde{\theta}_{-i}))\right]+h_i(\theta_i)
        \end{aligned}
        \nonumber
    \end{equation}
    where $\mathbb{E}_{\tilde{\theta}_{-i}}\left[\sum_{j\neq i}v_j(k^*(\theta_i,\tilde{\theta}_{-i}))\right]$ is the expected externality $i$ imposes on others, from $i$'s inteirim perspective when her type is $\theta_i$.
\end{definition}
