...
\begin{definition}
    \normalfont
    $\Gamma$ \textbf{implements} a social choice function $f$ if $\exists \left(S_1^a(\cdot),...,S_I^a(\cdot)\right)$ of a game induced by $\Gamma$ such that $g(S_1^a(\theta_q),...,S_I^a(\theta_I))=f(\theta_1,...,\theta_I)$ for all $(\theta_1,...,\theta_I)\in \Theta$
\end{definition}
