...
\begin{definition}[Arrow's Axioms]
    ...
    \begin{enumerate}[$\circ$]
        ...
        \item \textbf{Independence of Irrelevant Alternatives (IIA)}: For any $\theta, \theta' \in \Theta$, if $\mathbb{E}_x[u_i(x, \theta)] \geq \mathbb{E}_y[u_i(y, \theta)] \Leftrightarrow \mathbb{E}_x[u_i(x, \theta')] \geq \mathbb{E}_y[u_i(y, \theta')]$ for all $i$, then $x f(\theta) y \Rightarrow x f(\theta') y$.
    \end{enumerate}
\end{definition}
