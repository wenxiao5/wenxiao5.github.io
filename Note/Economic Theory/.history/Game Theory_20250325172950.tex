...
\begin{definition}[策略博弈]
策略博弈由以下三个要素组成：
\begin{itemize}
    \item 参与者集合 $N = \{1,2,\ldots,n\}$
    \item 每个参与者 $i$ 的策略集合 $S_i$
    \item 每个参与者 $i$ 的收益函数 $u_i: S_1 \times S_2 \times \cdots \times S_n \to \mathbb{R}$
\end{itemize}
\end{definition}

\begin{definition}[纯策略]
纯策略是指参与者在博弈中选择一个确定的行动。
\end{definition}

\begin{definition}[混合策略]
混合策略是指参与者在博弈中以一定概率选择不同的纯策略。
对于参与者 $i$，其混合策略是一个概率分布 $\sigma_i \in \Delta(S_i)$。
\end{definition}

\section{均衡概念}

\begin{definition}[Nash均衡]
策略组合 $s^* = (s_1^*,\ldots,s_n^*)$ 构成Nash均衡，如果对于任意参与者 $i$ 和任意策略 $s_i \in S_i$：
\[
u_i(s_i^*,s_{-i}^*) \geq u_i(s_i,s_{-i}^*)
\]
\end{definition}

\begin{theorem}[Nash定理]
对于任意有限策略博弈，至少存在一个混合策略Nash均衡。
\end{theorem}
...
\begin{itemize}
    \item 所有参与者同时行动
    \item 每个参与者完全了解其他参与者的收益函数
\end{itemize}

\subsection{完全信息动态博弈}
\begin{itemize}
    \item 参与者按顺序行动
    \item 后行动的参与者可以观察到先行动参与者的选择
    \item 使用逆向归纳法求解子博弈完美均衡
\end{itemize}

\subsection{不完全信息博弈}
\begin{itemize}
    \item 参与者不完全了解其他参与者的特征（类型）
    \item 引入贝叶斯均衡概念
    \item 常见应用：拍卖、信号博弈
\end{itemize}
