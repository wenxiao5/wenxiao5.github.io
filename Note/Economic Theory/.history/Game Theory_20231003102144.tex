...
\begin{enumerate}[$\circ$]
    ...
    \item $D_\textnormal{Hicksian}^i(p,u)=\{x^*:x^*\in\arg\min_{x} p\cdot x \textnormal{ s.t. }u^i(x)\geq u\}$
\end{enumerate}


\begin{definition}[Hicksian Valuation]
    \normalfont
    Hicksian valuation is defined by the money that can lead to the utility $u$ with goods $x_I$: $$v_\textnormal{Hicksian}^i(x_I,u)=(u^i(\cdot,x_I))^{-1}(u)$$
\end{definition}


\begin{proposition}
    $D_\textnormal{Hicksian}^i(p,u)=\arg\max_{x_I}\left\{v_\textnormal{Hicksian}^i(x_I,u)-p_I\cdot x_I\right\}$
\end{proposition}
