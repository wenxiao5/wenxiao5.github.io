...
My model uses unit demand setting to give equilibrium without collusion.

Suppose there are two firms $1,2$ that are competing by setting prices. Each firm $i$ faces an i.i.d. marginal cost $c_i\sim F\triangleq\textnormal{Unif}([0,1])$. The demand for firm $i$ is 
\begin{equation}
    \begin{aligned}
        D_i(p_i,p_j)=\left\{\begin{matrix}
            a-p_i,& p_i<p_j,\\
