...
\subsection{Second Welfare Theorem: P.O. with Endowments Used (Decentralized Market) $\Rightarrow$ CE}
...
To implement a more "equitable" allocation. Some possible mechanisms:
\begin{enumerate}[$\circ$]
...
\begin{definition}[Price Equilibrium with Transfers]
\normalfont
    An allocation $x^*$ is supportable as a \textbf{price equilibrium with transfers} if there exists a price vector $p^*\in \mathbb{R}^L$ and lump-sum budget-balancing transfers $\{T_i:i=1,...,m\}$ so that $\sum_{i=1}^m T_i=0$, such that $\forall i$:
    \begin{equation}
        \begin{aligned}
            x_i^*\in \arg\max_{x\in \mathbb{R}^L_+}u_i(x_i)\\
            \textnormal{s.t. } p^*\cdot x_i^*\leq p^*\cdot w_i + T_i
        \end{aligned}
        \nonumber
    \end{equation}
\end{definition}
