...
\normalfont
    ...
    \begin{enumerate}
        ...
        \begin{equation}
            ...
            \nonumber
        \end{equation}
        \item $$\sum_{i=1}^m x_i^*=\sum_{j=1}^J y_j^*+\sum_{i=1}^m w_i$$
    \end{enumerate}
\end{definition}
...
\end{theorem}
\begin{proof}
    Let $(x^*,y^*)$ be an interior P.O. allocation, so $x_i^*>>0, \forall i$. The same as exchange economy, for each agent $i$, let $P_i:=\{x_i\in \mathbb{R}_+^L: u_i(x_i)>u_i(x_i^*)\}$ and let $P:=\sum_{i=1}^mP_i$. Then $P$ is non-empty and convex.
