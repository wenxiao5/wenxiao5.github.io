...
\begin{enumerate}
    ...
    \item In collusion with transparency, the collusive price can be $p^M_i(c_i,c_j)=\left\{\begin{matrix}
        p_m,&c_i\leq c_j\\
        \textnormal{ any }p>p_m,&c_i>c_j
    \end{matrix}\right.$ that generates a profit $\Pi^M(c)=\frac{p_m-c}{2}$ for a firm with cost $c$. The expected profit from sustaining collusion is $W=\int_0^1 \Pi^M(c)dc=\frac{p_m-\frac{1}{2}}{2}$.
\end{enumerate}
