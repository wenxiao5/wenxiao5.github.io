...
\section{Axiomatic Complete Information Bargaining Models}
...
\subsection{Bilateral Negotiations: Nash Bargaining}
...
\begin{definition}[Bargaining Problem]
    A \textbf{bargaining problem} is a pair $(U,d)$ where $U\subseteq \mathbb{R}^2$ and $d\in U$. Usually, we assume
    \begin{enumerate}
...
\begin{definition}[Axioms]
    ...
    \begin{enumerate}
        \item \textbf{Axiom 1 (Pareto Efficient)}: A bargaining solution $f(U,d)$ is \textit{Pareto efficient} if there does not exist a $(v_1, v_2)\in U$ such that $v\geq f(U,d)$ and $v_i>f_i(U,d)$ for some $i$.
        \begin{note}
            Idea: an inefficient outcome is unlikely, since it leaves space for renegotiation.
        \end{note}
        \item \textbf{Axiom 2 (Symmetry)}: A bargaining solution $f$ is \textit{symmetric} if for any symmetric bargaining problem $(U,d)$ ($(u_1,u_2)\in U$ if and only if $(u_2,u_1)\in U$ and $d_1=d_2$), we have $f_1(U,d)=f_2(U,d)$.
        \begin{note}
            Idea: if the players are indistinguishable, the agreement should not discriminate between them. (Obviously ignores bargaining power. Can be easily relaxed.)
        \end{note}
        \item \textbf{Axiom 3 (Invariance to Linear Transformations)}: A bargaining solution $f$ is \textit{invariant} if for any bargaining problem $(U, d)$ and all $\alpha_i\in (0, \infty)$, $\beta_i\in \mathbb{R}$ ($i=1,2$), if we consider the bargaining problem $(U',d')$ with
        \begin{equation}
            \begin{aligned}
        ...
        \begin{note}
            Idea: utility functions are just one possible cardinal representation of ordinal preferences. They have no cardinal meaning. Therefore monotone (in particular linear) transformations should not affect the outcome.
        \end{note}
        ...
        \begin{note}
            Idea: taking away options that we didn't choose anyway shouldn't change what we do arguably more about behavior than the rest of Nash's axioms, and therefore most in need of a micro-foundation.
        \end{note}
    \end{enumerate}
\end{definition}
