...
\begin{equation}
    ...
    \nonumber
\end{equation}
where $a$ is assumed to be large enough, $a>\overline{c}$.

\paragraph{Case 1: Private cost shocks}
Suppose the cost is privately observed by each firm.
\begin{enumerate}
    \item Without collusion, firms compete as a first-price auction. That is, each firm sets price $p_i(c_i)=\mathbb{E}[c_j\mid c_i\leq c_j]$
\end{enumerate}
