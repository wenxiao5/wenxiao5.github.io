...
\section{Quadratic Motives}
Suppose there is a $\theta\sim N(\mu,\sigma^2)$, the real density is $f(\theta)$. The motivated density is
\begin{equation}
    \begin{aligned}
        \tilde{f}(\theta)&=\argmax_{f'(\theta)}-D_{KL}\left(f'\|f\right)+\int_{\theta} v(\theta) f'(\theta) d\theta\\
        \Rightarrow \tilde{f}(\theta)&=\frac{f(\theta)e^{v(\theta)}}{\int_{\theta'} f(\theta') v(\theta')d\theta}
    \end{aligned}
    \nonumber
\end{equation}
where $\int_{\theta'} f(\theta') v(\theta')d\theta$ is assumed to be finite.
