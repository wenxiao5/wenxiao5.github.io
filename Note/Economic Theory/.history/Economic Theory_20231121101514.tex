...
\begin{definition}[Direct Mechansim]
    ...
    Then a \textbf{direct mechanism} can be represented as $(Q,M)$, where $Q: \mathcal{X} \rightarrow \Delta N$ is the allocation rule and $M: \mathcal{X} \rightarrow \mathbb{R}^N$ is the payment rule, such that
    \begin{equation}
        \begin{aligned}
            Q(x)=\pi(\beta(x)),\quad M(x)=\mu(\beta(x))
        \end{aligned}
        \nonumber
    \end{equation}
\end{definition}
