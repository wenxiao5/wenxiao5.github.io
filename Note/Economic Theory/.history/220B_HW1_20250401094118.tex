...
Suppose there are two firms $1,2$ that are competing in a Bertrand competition by setting prices. Each firm $i$ faces an i.i.d. marginal cost $c_i\sim F\in\Delta([\underline{c},\overline{c}])$ with equal probability. The demand for firm $i$ is given by:
\begin{equation}
    \begin{aligned}
        D_i(p_i,p_j)=\left\{\begin{matrix}
            a-p_i,& p_i<p_j,\\
    ...
    \nonumber
\end{equation}
where $a$ is assumed to be large enough so that $a>\overline{c}$.
