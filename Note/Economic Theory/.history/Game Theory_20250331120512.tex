...
Note that garbling forms a partial order and does not require $|Y|<|Y'|$. For example, consider:
\begin{itemize}
    \item Original signal space $Y=\{1,2\}$ with $\pi_1(a)=p$, $\pi_2(a)=1-p$
    \item Garbled signal space $Y'=\{1,2,3\}$ with $q(1|1)=q(2|2)=0.5$, $q(3|1)=q(3|2)=0.5$
\end{itemize}

\begin{theorem}[Kandori]
    If $\Pi'$ is a garbling of $\Pi$, then $E_{\Pi'}(\delta)\subseteq E_{\Pi}(\delta)$.
\end{theorem}

\begin{proof}[Proof Sketch]
    For any PPE under $\Pi'$, construct a PPE under $\Pi$ by:
    \begin{itemize}
        \item Using same first-period actions
        \item For continuation values, first apply garbling matrix $Q$
        \item Then use continuation values from original PPE
    \end{itemize}
    This preserves incentive compatibility since $\Pi'=\Pi Q$.
\end{proof}

This shows that more informative signals (in Blackwell's sense) allow for weakly more equilibrium payoffs, as they provide better monitoring of deviations.
