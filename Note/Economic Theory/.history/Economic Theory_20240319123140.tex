...
\end{lemma}
\begin{proof}
    Suppose for a contradiction, $\exists$ an SPE where both worker types sign $(w_p=\mathbb{E}[\theta],t_p)$. Suppose one firm offers $(w_p,t_p)$, then another firm can only employ high type workers by offering $(\tilde{w},\tilde{t})$, where $\tilde{w}-e(\theta_H,\tilde{t})>\mathbb{E}[\theta]-e(\theta_H,t_p)$ \textbf{......}
\end{proof}
...
\begin{proof}
    \begin{enumerate}
        ...
        \item \underline{$w_H\geq \theta_H$:} Suppose $w_H<\theta_H$, firm profitably deviate by offering $(w'_H,t'_H)$ such that \textbf{......}
    \end{enumerate}
