...
\begin{corollary}
    Suppose that $f$ is non-decreasing and non-negative. A function $g$ is an extreme point of $\textnormal{MPS}_w(f)$ \underline{if and only if} there is $\theta \in[0,1]$ such that $g$ is an extreme point of $\textnormal{MPS}(f)$ and $g(x) = 0$ for a.e.x∈[0θ).
\end{corollary}
