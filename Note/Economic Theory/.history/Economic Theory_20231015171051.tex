...
\begin{definition}[Revealed Preferred]
    ...
    We say $x^t$ is
    \begin{enumerate}[-]
        \item \underline{\textbf{directly} revealed preferred} to $x$, denoted by $x^t R^D x$, if $p^t x^t \geq p^t x$;
        \item \underline{\textbf{strictly directly} revealed preferred} to $x$, denoted by $x^t P^D x$, if $p^t x^t > p^t x$;
        \item \underline{\textbf{indirectly} revealed preferred} to $x$, denoted by $x^t R x$, if $\exists$ a sequence $\{x_k\}_{k=1}^K$ with $x_1=x^t$ and $x_K=x$ such that $x^k R^D x^{k+1}$ for all $k=1,...,K-1$.
    \end{enumerate}
\end{definition}
