...
\begin{proof}
    ...
    \begin{equation}
        \begin{aligned}
            \tilde{W}_n&=\int_{\underline{v}}^{0}\frac{1-F(0)}{1-F(v)} 0 n(n-1)f(v)(1-F(v))F(v)^{n-2}dv+\int_{0}^{\bar{v}}v n(n-1)f(v)(1-F(v))F(v)^{n-2}dv\\
    ...
    0), but we need to note that this result depends on the optimal reservation price being $0$. The intuition is: $\tilde{V}_n-\tilde{V}_{n-1}$ is the difference between the highest value among first $n$ values and the highest value among first $n-1$ values. There is only $\frac{1}{n}$ probability that the $n^{th}$ value is the highest value among the first $n$ values. However, $\tilde{V}_n-\tilde{W}_n$ always gives the difference between the highest value and the second-highest value among first $n$ values.
