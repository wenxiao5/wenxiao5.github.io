...
Fixing one agent's strategy, the specified strategy is optimal for the other agent ex-post (even if they knew the other's type). Therefore this mechanism can be implemented as a dominant strategy incentive compatible (DSIC) direct mechanism.

Any $p \in (\underline{c}, \bar{v})$ will work as this fixed price.
\end{remark}
