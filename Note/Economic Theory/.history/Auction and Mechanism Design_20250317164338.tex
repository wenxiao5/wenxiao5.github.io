...
\begin{definition}[Harsanyi's three axioms]
    ...
    \begin{enumerate}
        \item \textbf{EU preferences}: each agent's preference $\succcurlyeq_i$ can be represented by the von Neumann-Morgenstern utility function, $u_i$, $$x\succcurlyeq_i y\Leftrightarrow\mathbb{E}_xu_i(x;\theta_i)\geq \mathbb{E}_yu_i(y;\theta_i).$$ Let $\mathcal{R}$ be the set of such preferences.
