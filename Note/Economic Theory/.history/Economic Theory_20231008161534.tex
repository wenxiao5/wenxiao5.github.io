...
\subsection{Hicksian Demand: Properties}
\begin{proposition}
    $u(\cdot)$ is monotone and $p>>0$.
    \begin{enumerate}[(i).]
        \item $h(p,u)$ is homogeneous of degree zero in $p$;
        \item $u ( x )$ is strictly quasi-concave $\Rightarrow$ $h ( p, u )$ is unique;
        \item For $u>u(0)$, \underline{constraint is active}: for all $x^*\in h(p,u)$, $$u(x^*)=u\textnormal{ and }x^*\in x(p,p\cdot x^*)$$
        \item \textbf{Law of Compensated Demand}: $\frac{\partial h_i}{\partial p_i}\leq 0$
