...
\begin{corollary}[Optimal Auction Mechanism]
    ...
    \begin{enumerate}[(1).]
        ...
        \item (We can) define $t(\cdot):\Theta \rightarrow K$,
        \begin{equation}
            \begin{aligned}
...
\begin{example}
    Suppose $\Theta_i=[0,1],\theta_i\sim U[0,1]$. $c_i(\theta_i)=\theta_i-\frac{1-\theta_i}{1}=2\theta_i-1$, which is strictly increasing in $\theta_i$ (regularity satisfied).
\end{example}
