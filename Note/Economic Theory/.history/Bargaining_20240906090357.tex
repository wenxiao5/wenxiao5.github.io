...
Now, we consider a more case that the value function $V$ is a function of both $x$ and $t$.
\begin{equation}
    \begin{aligned}
        v(t,x(t))=\max_{\{\alpha(s)\}_{s\geq t}}&\int_0^\infty e^{-\rho t}r(x(t),\alpha(t))dt\\
        \textnormal{s.t. }&\dot{x}(t)=f(x(t),\alpha(t))
    \end{aligned}
    \nonumber
\end{equation}
