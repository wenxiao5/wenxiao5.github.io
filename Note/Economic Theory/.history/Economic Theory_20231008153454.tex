...
\begin{enumerate}[(i).]
    ...
    \item $\lambda^*\left(x(p,w)+p\frac{\partial x(p,w)}{\partial p}\right)=0$.
\end{enumerate}
...
\begin{corollary}
    $\frac{\partial u(x(p, w))}{\partial w}=\lambda^{*}$.
\end{corollary}
\begin{proof}
...
\subsection{Roy's identity $x^*_i=\frac{\frac{\partial v}{\partial p_{i}}}{-\lambda^*}=-\frac{\frac{\partial v}{\partial p_{i}}}{\frac{\partial v}{\partial w}}$: recover $x(p, w)$ from $v(p, w)$}
