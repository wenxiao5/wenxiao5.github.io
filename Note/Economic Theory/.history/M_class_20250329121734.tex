...
\begin{enumerate}
    \item \underline{Observable prices}: as the search costs increase $\Rightarrow$ less likely to search more $\Rightarrow$ attract consumers to search first is more important $\Rightarrow$ Lower prices.
    \item \underline{Unobservable prices}: as the search costs increase $\Rightarrow$ less likely to search more $\Rightarrow$ try to exploit consumers $\Rightarrow$ Higher prices.
\end{enumerate}
...
\begin{enumerate}
    \item Random purchase: $x$ gets expected disutility $d_R=\int_0^1 t|x-z| dz=t\left(x-\frac{1}{2}\right)^2+\frac{t}{4}$, which is minimized for consumers located at $x=\frac{1}{2}$ who gets disutility $\min d_R=\frac{t}{4}$.
    \item Search: the optimal search strategy can be given as ``Stop searching if and only if the product searched falls in $[R_L(x),R_R(x)]$.'' The expected disutility for a consumer locates at $x$ is $$\frac{1}{F(R_R(x))-F(R_L(x))} \left(t\int_{R_L(x)}^{R_R(x)}|x-z|dF(z)+c\right)$$
    Thus, for the case of uniformly located alternatives, the optimal search strategy is ``Stop searching if and only if the product searched falls in $[x-\delta,x+\delta]$.''
    The expected disutility for a consumer locates at $x$ is
    \begin{equation}
        \begin{aligned}
            d(\infty;\delta)=\frac{1}{2\delta}\left(\int_{x-\delta}^{x+\delta}t|x-z|dz+c\right)=\frac{\delta^2t+c}{2\delta}
        \end{aligned}
        \nonumber
    \end{equation}
    which is minimized at $\delta=\sqrt{\frac{c}{t}}$ and the corresponding expected disutility is $$d(\infty)=\sqrt{ct}$$
\end{enumerate}
Therefore, all consumers choose to search instead of randomly purchase if and only if
\begin{equation}
    \begin{aligned}
        \min d_R=\frac{t}{4}&\geq d(\infty)=\sqrt{ct}\\
        \Rightarrow \frac{c}{t}&\leq \frac{1}{16}
    \end{aligned}
    \nonumber
\end{equation}
...
Let's consider a strategy (not optimal): \underline{Search until find the closest alternative}:
\begin{enumerate}
    \item Without search costs, it gives the expected disutility $\int_{0}^{\frac{1}{2\hat{N}}}tx \frac{1}{2\hat{N}}dx =\frac{t}{4\hat{N}}$ for a consumer on average.
    \item The expected search cost to find this product is
    \begin{equation}
        \begin{aligned}
            \left(\sum_{i=1}^{\hat{N}-1}\frac{i}{\hat{N}}+\frac{\hat{N}-1}{\hat{N}}\right)c=(\hat{N}-1)\left(\frac{1}{2}+\frac{1}{\hat{N}}\right)c
        \end{aligned}
        \nonumber
    \end{equation}
\end{enumerate}
(The probability that stopping at $n^{th}$ searching is $1/\hat{N}$ with search cost $n c$ for $n\leq\hat{N}-1$ and $(\hat{N}-1)c$ for $n=\hat{N}$.)
...
Suppose there is a two periods model. Consumers' values distribute over $[0,1]$. Consumers with $v\geq v^*\in (0,1)$ bought in the first period. In the second period, we can price based whether a consumer bought in the first period.

Forward-looking consumers hurt firms.

\section{Branding}
\subsection{Umbrella Branding}
Whether tell consumers two firms belong to me.
One of the reason: Signal quality.

\subsection{Brand Values}
Social interactions: you want to match with some types of people. Brand can signal type.


\section{Signaling}
High quality one wants to signal its quality.
\begin{example}[ (Money-back Guarantees)]
    Marginal costs are $c_H>c_L=0$. ($\phi$ is the probability of being high-quality).

    Let $v_i,i\in\{L,H\}$ be the reservation value if it works and is $i$-quality.

    Let $f_i,i\in\{L,H\}$ be the probability of falling if it is $i$-quality. ($f_H<f_L$ and $(1-f_H)v_H>(1-f_L)v_L$).

    Firms' decisions are (1). Price $p$; (2). Decision to offer or not offer money-back guarantees (if fails). Suppose there are transaction costs of using money-back guarantees: $t_B$ for buyers and $t_S$ for sellers.

    With complete information, there is no money-back guarantee in equilibrium.

    With incomplete information, there can have a separating equilibrium that H-quality offers MBG while L-quality does not, and may have a pooling equilibrium. The condition to make the separating equilibrium exist: the low-quality does not want to imitate the high-quality,
    \begin{equation}
        \begin{aligned}
            (1-f_L)p_H-t_Sf_L\leq p_L
        \end{aligned}
        \nonumber
    \end{equation}
    High-quality does not benefit from not offering MBG,
    \begin{equation}
        \begin{aligned}
            (1-f_H)p_H-t_Sf_H-c_H\geq \max\{0,p_L-c_H\}
        \end{aligned}
        \nonumber
    \end{equation}
    and $p_H>t_B$.

    Then, we define consistent on-path beliefs and off-paths beliefs that cannot lead to deviations.

    Result: the separating equilibrium exists if the $t_S$ is either not too small or not too large.
\end{example}
...
\subsection{Base Case}
Suppose there is a manufacturer, a retailer, and consumers. The demand of consumers is $D(p;s)$, where $s$ is the service offered by retailer.

\begin{enumerate}
    \item \underline{Uniform Pricing}: $T(q)=p_w\cdot q$.
    \item \underline{Franchise Fee}: $T(q)=A+p_w\cdot q$.
    \item \underline{Fixed Prices}: $p\in [\underline{p},\overline{p}]$.
    \item \underline{Fixed Quantities}: $q\in [\underline{q},\overline{q}]$.
\end{enumerate}

Some constraints:
\begin{enumerate}
    \item \underline{Exclusive Territories}: Constraints on manufacturer.
    \item \underline{Exclusive Dealing}: Constraints on retailer.
    \item \underline{Tie-In Sales/Royalties}.
\end{enumerate}


\subsection{Double Marginalization (under uniform price)}
Suppose $D(p)=1-p$.

Coordinated channel: $p^*_\textnormal{coordinated}=\argmax_p (p-c)(1-p)=\frac{1+c}{2}$. Total profit is $\frac{(1-c)^2}{4}$.

If not coordinated, the manufacturer chooses a price for retailer and the retailer chooses a price for consumers.

Given the price $p_w$ set by the manufacturer, the retailer can choose a price for consumers:
\begin{equation}
    \begin{aligned}
        p=\argmax_p \Pi^R(p)=\argmax_p(p-p_w)(1-p)=\frac{1+p_w}{2}
    \end{aligned}
    \nonumber
\end{equation}
Then, the corresponding profit of the manufacturer from setting $p_w$ is $\Pi^M(p_w)=\frac{(p_w-c)(1-p_w)}{2}$. The optimal $p_w$ is solved by
\begin{equation}
    \begin{aligned}
        p_w^*=\argmax_{p_w}\Pi^M(p_w)=\argmax_{p_w}\frac{(p_w-c)(1-p_w)}{2}=\frac{1+c}{2} \Rightarrow p^*_\textnormal{noncoordinated}=\frac{3+c}{4}
    \end{aligned}
    \nonumber
\end{equation}
Total profit is $\frac{(1-c)^2}{16}+\frac{(1+c)^2}{8}=\frac{3(1-c)^2}{16}$.

The price under coordinated $p^*_\textnormal{coordinated}$ is lower than the price under non-coordinated $p^*_\textnormal{noncoordinated}$. The total profit under coordinated is lower than the total profit under non-coordinated. This problem can be solved by contracts.

\subsection{Contracts}
\begin{enumerate}
    \item \underline{Two-part Tariff (Franchise Fee):} $T(q)=A+p_w\cdot q$. Set the $p_w=c$ such that the retailer chooses the price $p^*=p^*_\textnormal{coordinated}$ and the $A$ that extract all surplus of retailer.
    \item \underline{Price Ceiling}: there is a cap on the retailer's price, $p^*\leq p^*_\textnormal{coordinated}$.
    \item \underline{Quantity Forcing}: $q\geq D(p^*_\textnormal{coordinated})$.
\end{enumerate}

\paragraph*{Problems with Two-part Tariff}
\begin{enumerate}
    \item Exists uncertainty and the retailer is risk-averse: We need to give some of the risk to manufacturer, i.e., increase $p_w>c$ and decrease $A$.
    \item Retailer knows more about demand than manufacturer: Screening problem, solved by a menu that the highest type gets efficient quantity ($p_w=c$) and the lowest type gets zero profit ($p_w>c$).
    \item Manufacturer knows more about demand than retailer: Signaling problem. Signal through $p_w$ and $A$: higher $p_w$ is more difficult to be copied by a low demand manufacturer.
    \item Multiple competing retailers.
    \item If there is no contractability of product design, the fixed fee would be lower. Bargaining power will affect what happens in the market.
\end{enumerate}

\subsection{Service Externality}
Suppose there is a service $s$ chosen by the retailer that can affect the demand $D(p,s)$.

Given the $p_w$, the retailer chooses $p(p_w),s(p_w)$ as
\begin{equation}
    \begin{aligned}
        \max_{p,s}\Pi^R(p,s)=(p-p_w-\Phi(s))D(p,s)
    \end{aligned}
    \nonumber
\end{equation}
By the double marginalization, both the price and the service are distorted. Solutions to this problem are
\begin{enumerate}
    \item \underline{Two-part Tariff}: $T(q)=A+p_w\cdot q$. Still works.
    \item \underline{Price Ceiling}: $p^*\leq p^*_\textnormal{coordinated}$. Doesn't work.
    \item \underline{Quantity Forcing}: $q\geq D(p^*_\textnormal{coordinated})$. Still works.
\end{enumerate}


Now, suppose the manufacturer can also choose a service $s_M$ such that the demand is given by $D(p,s,s_M)$.
\begin{enumerate}
    \item \underline{Two-part Tariff}: $T(q)=A+p_w\cdot q$. Doesn't work. Marginal increase in demand gives $p_w-c$ for manufacturer and $p-p_w$ for retailer. High $p_w$ gives more incentive for manufacturer to provide service but less incentive for retailer to provide service.
\end{enumerate}


\subsection{Input Substitution}
Except the manufacturer that provides input $x$ with price $p_w$, there is a competitive industry that provides input $x'$ with price $p'_w=c'$. The retailer produce outputs as $q=f(x,x')$. The difference is the retailer has market power for $x'$.

The problem is the input distortion in equilibrium: use less $x$ than optimal,
\begin{equation}
    \begin{aligned}
        \frac{p_w}{p'_w}>\frac{c}{c'}
    \end{aligned}
    \nonumber
\end{equation}
...
\underline{Solutions:}
\begin{enumerate}
    \item Integrate vertically with retailer.
    \item Two-part tariff, $p_w=c$.
    \item Tie-in contract with price ceiling
\end{enumerate}



\subsection{Retailer Competition}
\underline{Retailer Competition}: Problem is Horizontal externality, i.e., Providing service benefits other retailers: manufacturer provides exclusive territories.

\underline{Definition of a firm}: When is the optimal for the manufacturer and the retailer to be integrated? Incomplete contracts.

\section{Sales force incentives}
\begin{enumerate}
    \item \textit{Moral hazard}: Let $t$ be the effort and $x$ be the output/sales (which follows a distribution $f(x\mid t)$ given the effort). The profit of the firm is $(1-c)x$ and the salary of a sales person is defined by $s(x)$. The net utility of the sales person is $U(s(x))-V(t)$. The sales person's decision is given by
    \begin{equation}
        \begin{aligned}
            \max_t \mathbb{E}\left[U(s(x))\mid t\right]-V(t)=\int_x U(s(x))f(x\mid t)dx-V(t)
        \end{aligned}
        \nonumber
    \end{equation}
    The F.O.C. is
    \begin{equation}
        \begin{aligned}
            \int_x U(s(x))f_t(x\mid t)dx=V'(t)
        \end{aligned}
        \nonumber
    \end{equation}
    ...
    \begin{equation}
        \begin{aligned}
            \max_{s(x)}\ &\left[(1-c)x-s(x)\right]f(x\mid t)dx\\
            \textnormal{s.t. }&\int_x U(s(x))f_t(x\mid t)dx=V'(t)\\
            &\int_x U(s(x))f(x\mid t)dx-V(t)\geq \underbrace{U(m)}_\textnormal{outside option}
        \end{aligned}
        \nonumber
    \end{equation}
    \item \textit{Adverse selection}: A menu of compensation scheme. The sales person self-selects the compensation scheme.
\end{enumerate}



\section{Advertising}
\underline{Dorfman-Steiner Rule}. Suppose the demand depends on the price and the advertising $A$.
\begin{equation}
    \begin{aligned}
...
\begin{equation}
    \begin{aligned}
        &(p-c)\frac{\partial D}{\partial A}-1=0\\
        &\frac{A}{p\cdot D}=\frac{p-c}{p}\underbrace{\frac{\partial D}{\partial A}\frac{A}{D}}_\textnormal{elasticity: demand wrt advertising}\\
        &\frac{A}{p\cdot D}=\frac{p-c}{p}E_\textnormal{demand wrt advertising}
    \end{aligned}
    \nonumber
\end{equation}


\subsection{Dynamic in Advertising}
Let $G$ be the state variable, e.g., goodwill, awareness. Let $A$ be the control variable.
\paragraph*{Nerlove-Arrow Model} $\frac{d G}{dt}=\dot{G}=A-\delta G$.
We want to maximize
\begin{equation}
    \begin{aligned}
        \max_{A,G}\ &\int_0^{\infty}e^{-rt}[\Pi(G)-A]dt\\
        \textnormal{s.t. }&\dot{G}=A-\delta G
    \end{aligned}
    \nonumber
\end{equation}
\underline{Solution}: Jump to $\hat{G}$ and stay there such that $\hat{A}=\delta\hat{G}$


\paragraph*{More General Model}
$\dot{G}=(1-G)f(A)-\delta G$.

Given the $f(A)$ is concave. \underline{Solution}: Approach $\hat{G}$ slowly, where $0=(1-\hat{G})f(\hat{A})-\delta \hat{G}$. Once you get there you stay there.

Given the $f(A)$ is `S'-shape, i.e., convex at first and concave later. \underline{Solution}: Mixing between no advertising and lots of advertising. Shattering and pulsing.

In competition, firms advertise at different times.




\section{Promotions}
\subsection{Competition of Promotions}
Competition model of promotions (search costs in competition $\Rightarrow$ mass of consumers with zero search costs)

Suppose there are $N$ sellers. Each seller has $M$ loyal consumers (not informed, positive search costs). Segment $S$ switchers who buy at lowest price (informed, zero search costs). All consumers have the same reservation value $r$.
...
\begin{equation}
    \begin{aligned}
        (\underline{p}-c)(M+S)=(r-c)M\\
        \underline{p}=c+\frac{(r-c)M}{M+S}
    \end{aligned}
    \nonumber
\end{equation}
Equilibrium in mixed strategies, $F(p)$, which is given by
\begin{equation}
    \begin{aligned}
        (r-c)M=(p-c)M+(p-c)S[1-F(p)]^{N-1}\\
        F(p)=1-\left[\frac{(r-c)M}{(p-c)S}\right]^{\frac{1}{N-1}}
    \end{aligned}
    \nonumber
\end{equation}


\subsection{Promotion to induce the trail}
