...
\begin{theorem}[Benoit-Krishna (1985)]
...
\begin{definition}[Perfect Public Equilibrium]
...
\begin{definition}[Generated Payoffs]
...
\begin{definition}[Self-Generation]
...
\textbf{Key Idea:}
\begin{itemize}
    \item Due to demand uncertainty, equilibrium features alternating periods of collusion and price wars
    \item Green and Porter (1984) solve for this equilibrium using recursive dynamic programming methods
\end{itemize}
...
\subsection{Rubinstein's Alternating-Offers Model}
Rubinstein (1982) provides a non-cooperative foundation for bargaining by modeling the actual process of negotiation. Two players negotiate over how to divide a "pie" of size 1 through alternating offers:

\begin{itemize}
    \item At $t=0$, player A makes an offer $(x_A, 1-x_A)$ where $x_A$ is A's share
    \item If B accepts, game ends with that division
    \item If B rejects, at $t=1$, B makes a counteroffer $(1-x_B, x_B)$
    \item If A accepts, game ends; if not, roles switch and game continues
\end{itemize}

Players discount future payoffs with factors $\delta_A, \delta_B \in (0,1)$. Zero payoff in periods without agreement.

\begin{theorem}[Rubinstein, 1982]
    The game has a unique subgame perfect Nash equilibrium characterized by:
    \begin{enumerate}
        \item Immediate agreement in period 0
        \item Equilibrium shares when player $i$ proposes:
        \begin{equation}
            \begin{aligned}
...
\begin{proof}[Sketch]
    Key steps:
    \begin{enumerate}
        \item In any subgame where A offers, B accepts if getting $\geq \delta_B x_B$ (discounted continuation value)
        \item When B offers, A accepts if getting $\geq \delta_A x_A$
        \item This yields system of equations:
        \begin{align*}
            x_A &= 1-\delta_B x_B\\
            x_B &= 1-\delta_A x_A
        \end{align*}
        \item Solving gives unique stationary equilibrium shares
        \item One-shot deviation principle proves uniqueness
    \end{enumerate}
\end{proof}

\begin{remark}[Connection to Nash Bargaining]
    As $\delta_A,\delta_B \to 1$, equilibrium shares approach the symmetric Nash bargaining solution. This provides a non-cooperative foundation for the axiomatic approach.
\end{remark}

\begin{remark}[First-Mover Advantage]
    Player A's equilibrium share $x_A$ exceeds B's discounted share $\delta_B x_B$, reflecting advantage of making first offer. This advantage diminishes as players become more patient.
\end{remark}
