...
Previous research shows that bettors have an intuitive bias, which makes them believe the superior team (i.e., the favorite) is more likely to win. Holding the intuitions lead to underweight of point spreads. This bias persists even if bettors are told the spreads are biased. Moreover, this bias is also reinforced as time goes, which can be explained by attribution. That is, bettors attribute the winning prediction to skill and intuitive while the losing prediction is attributed to luck. Finally, this paper shows that using a different elicitation manner can eliminate the bias in judgments.

This is an interesting paper shows how behavioral biases can affect the elicited information and the importance of choosing a proper elicitation method.
