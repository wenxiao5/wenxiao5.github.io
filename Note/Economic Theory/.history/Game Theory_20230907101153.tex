...
\begin{definition}[Mechanism]
    \normalfont
    A \textbf{mechanism} is represented as $$\Gamma=\left((S_1,...,S_I), g:\mathcal{S}^I \rightarrow X\right)$$
    where $(S_1,...,S_I)$ represents the set of strategies that agents can choose.
\end{definition}
...
\begin{definition}[Direct Mechanism]
    \normalfont
    A mechanism is \textbf{direct mechanism} if $S_i=\Theta_i$ for all $i\in I$ and $g(\theta)=f(\theta)$ for all $\theta\in\Theta$
\end{definition}
