...
\begin{definition}[Properties of a Social Choice Function]\label{SWF_properties}
    ...
    \begin{enumerate}[$\circ$]
        ...
        \item \textbf{Independence of Irrelevant Alternatives (IIA)}: For any $\theta, \theta' \in \Theta$, if $u_i(x, \theta) > u_i(y, \theta) \Leftrightarrow u_i(x, \theta') > u_i(y, \theta')$ for all $i\in I$ and some $\{x, y\} \subseteq X$, then $x = f(\theta) \Rightarrow y \neq f(\theta')$.
    \end{enumerate}
\end{definition}
...
\begin{enumerate}
    ...
    \item The set of alternatives (or societal outcomes) $X$ is the set of all lotteries on $\mathcal{C}$, i.e., $X=\Delta(\mathcal{C})$ (Note: not finite).
\end{enumerate}

\begin{definition}[Harsanyi's Three Axioms]
    Harsanyi's three axioms are as follows:
    \begin{enumerate}
        \item \textbf{EU Preferences}: Each agent's preference $\succcurlyeq_i$ (or $\theta_i$ equivalently) can be represented by a von Neumann-Morgenstern utility function $u_i(\cdot;\theta_i)$ such that $$x \succcurlyeq_i y \Leftrightarrow \mathbb{E}_x[u_i(x;\theta_i)] \geq \mathbb{E}_y[u_i(y;\theta_i)].$$ Let $\mathcal{R}$ denote the set of such preferences.
        \item \textbf{Pareto Indifference}: If $x \sim_i y$ for all $i \in I$, then $x \sim y$.
        \item \textbf{Social EU}: For each $(\succcurlyeq_1,\ldots,\succcurlyeq_I) \in \mathcal{R}^I$, the \textit{social welfare function} satisfies the vNM EU axioms: there exists $u(\cdot;\theta)$ such that $$x f(\theta) y \Leftrightarrow \mathbb{E}_x[u(x;\theta)] \geq \mathbb{E}_y[u(y;\theta)].$$
    \end{enumerate}
\end{definition}

\begin{theorem}
    If Harsanyi's three axioms are satisfied, then for each $\theta \in \Theta$, there exists $\lambda(\theta) \in \Delta(I)$ such that $u(\cdot;\theta) = \sum_{i \in I} \lambda_i(\theta) u_i(\cdot;\theta_i)$.
\end{theorem}

Recall that Arrow's axioms in this context are given as follows:
\begin{definition}[Arrow's Axioms]
    ...
    \begin{enumerate}[$\circ$]
        ...
        \item \textbf{Non-Dictatorship (ND)}: There is no agent $i \in I$ such that, for all $\{x, y\} \subseteq X$, $\mathbb{E}_x[u_i(x;\theta_i)] \geq \mathbb{E}_y[u_i(y;\theta_i)]$ implies $x f(\theta) y$. (In other words, no single agent always determines the social preference.)
        \item \textbf{Weak Pareto Efficiency (PA)}: For all $\{x, y\} \subseteq X$ and any preference profile $\theta \in \Theta$, if $\mathbb{E}_x[u_i(x;\theta_i)] \geq \mathbb{E}_y[u_i(y;\theta_i)]$ for all $i \in I$, then $x f(\theta) y$.
        \item \textbf{Independence of Irrelevant Alternatives (IIA)}: For any $\theta, \theta' \in \Theta$, if $\mathbb{E}_x[u_i(x, \theta)] \geq \mathbb{E}_y[u_i(y, \theta)] \Leftrightarrow \mathbb{E}_x[u_i(x, \theta')] \geq \mathbb{E}_y[u_i(y, \theta')]$ for all $i$ and some $\{x, y\} \subseteq X$, then $x f(\theta) y \Rightarrow x f(\theta') y$.
    \end{enumerate}
\end{definition}

When $\lambda(\theta)$ is constant and non-degenerate, we obtain a social choice function $f$ defined by
\begin{equation}
    \begin{aligned}
        x f(\theta) y \Leftrightarrow \sum_{i=1}^I \lambda_i \mathbb{E}_x[u_i(x;\theta)] \geq \sum_{i=1}^I \lambda_i \mathbb{E}_y[u_i(y;\theta)].
    \end{aligned}
    \nonumber
\end{equation}
This is often referred to as \textbf{utilitarian social choice}, which is widely used in practice. It has the following properties:
\begin{enumerate}
    \item It satisfies Pareto efficiency.
    \item It is not dictatorial.
    \item Its domain is limited to EU preferences.
    \item It violates IIA.
\end{enumerate}

\begin{lemma}
    The utilitarian social choice function violates IIA.
\end{lemma}
\begin{proof}
    We construct a counterexample with two individuals and two alternatives to demonstrate the violation of IIA.
    \begin{itemize}
        \item Individuals: $i = 1, 2$.
        \item Alternatives: $x$ and $y$.
        \item Fixed weights: $\lambda_1 = \lambda_2 = 0.5$ (non-dictatorial).
    \end{itemize}
...
\begin{note}
    Second-price auction and ascending auction are strategically equivalent.
\end{note}
...
\begin{example}[ (First-Price Auction as Pricing Competition with Random Marginal Costs)]
Consider a market with $n$ firms competing in prices. Each firm $i$ has a private marginal cost $c_i$ drawn independently from a distribution $F$ on $[\underline{c}, \overline{c}]$. Firms simultaneously set prices $p_i$, and the firm with the lowest price captures the entire market demand $D(p) = 1$ (unit demand). The profit of firm $i$ is:
\begin{equation}
    \begin{aligned}
        \pi_i(p_i, p_{-i}) = 
        \begin{cases}
...
Using our results from first-price auctions, the symmetric Bayesian Nash equilibrium pricing strategy is:
\begin{equation}
    \begin{aligned}
        p(c) = c + \frac{1}{F^{n-1}(c)}\int_{c}^{\overline{c}} F^{n-1}(t) dt
    \end{aligned}
    \nonumber
\end{equation}

This shows that firms mark up their prices above marginal cost, with the markup decreasing as the number of competitors increases. The markup also depends on the distribution of costs - more uncertainty about competitors' costs leads to higher markups.

The equilibrium expected revenue of the seller is:
\begin{equation}
    \begin{aligned}
        \mathbb{E}[p(c_{(1)})] = \mathbb{E}[c_{(2)}] + \mathbb{E}\left[\frac{1}{F^{n-1}(c_{(1)})}\int_{c_{(1)}}^{\overline{c}} F^{n-1}(t) dt\right]
    \end{aligned}
    \nonumber
\end{equation}
where $c_{(1)}$ is the lowest cost and $c_{(2)}$ is the second-lowest cost. This shows that the seller's expected revenue equals the expected second-lowest cost plus an additional term determined by the cost distribution.

Alternatively, we can express the expected revenue without conditioning on specific cost realizations:
\begin{equation}
    \begin{aligned}
        \mathbb{E}[\text{Revenue}] = \underline{c} + \int_{\underline{c}}^{\overline{c}} [1-F(t)]n(n-1)F^{n-2}(t)[1-F(t)]dt
    \end{aligned}
    \nonumber
\end{equation}

This is consistent with the Revenue Equivalence Theorem from standard auction theory, which states that any efficient auction mechanism in a symmetric independent private values model will generate the same expected revenue.
\end{example}
...
\section{Additional bidders vs. optimal reserve prices}
Consider a symmetric IPV setting with a regular distribution $F$. A bidder is defined as \textit{serious} if $a \geq v_0$.
\begin{theorem}[Bulow \& Klemperer, AER, 1996]
    Provided that all bidders are serious, the revenue from the optimal auction with $N$ bidders is strictly less than the revenue from a second-price auction without a reserve price and $N+1$ bidders.
\end{theorem}
The authors extend this result to asymmetric distributions and settings with interdependent values, as long as the signals received by the buyers about their potentially common value are independent.

\begin{lemma}[Bulow \& Klemperer, AER, 1996 (proof by Kirkegaard, 2006)]
    In the symmetric, regular, IPV case, among mechanisms which never keep the good (i.e. no reserve price, $\sum_{i}y_i(\theta)$ for all $\theta$) the second-price auction is optimal.
\end{lemma}
