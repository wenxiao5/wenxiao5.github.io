...
\begin{definition}[Substitutability Condition]
    ...
    ($Y'\subseteq Y\subseteq X \Rightarrow R_f(Y')\subseteq R_f(X)$)
\end{definition}
...
\end{theorem}
\begin{proof}
    Suppose that $Z$ is a block. So, $Z\subseteq C_f(A\cup Z)$ for all $f$ listed in $Z$. We can pick a $z\in Z$ such that $z\in C_f(A\cup Z)$.
\end{proof}
