...
\begin{definition}[Quantal Response Equilibrium]
    \normalfont
    A \textbf{quantal response equilibrium} of $G$ is a strategy profile $\sigma^*$ such that $\sigma^*=\Gamma_i\left(\vec{u}_i(\sigma^*_{-i})\right)$, where $\vec{u}_i(\sigma^*_{-i})=(u_i(s_i,\sigma^*_{-i}))_{s_i\in S_{i}}$.
\end{definition}
