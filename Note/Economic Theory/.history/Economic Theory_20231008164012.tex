...
\begin{lemma}[$\sum_{j=1}^{L} u_{j}\left(x^{*}\right) \frac{\partial h_{j}}{\partial p_{i}}=0$]
    If $u ( x )$ is strictly quasi-concave, the Hicksian demand satisfies $\sum_{j=1}^{L} u_{j}\left(x^{*}\right) \frac{\partial h_{j}}{\partial p_{i}}=0$
\end{lemma}
\begin{proof}
    $u(h(p,u))\equiv u \Rightarrow \sum_{j=1}^{L} \frac{\partial u}{\partial x}(x^*)\frac{\partial h_{j}}{\partial p_{i}}=0$.
\end{proof}
