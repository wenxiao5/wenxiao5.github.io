...
\begin{equation}
    ...
    \nonumber
\end{equation}
where $v: K\times\Theta_i \rightarrow \mathbb{R}$.
...
\begin{definition}[ex-post efficient]
    ...
    \begin{enumerate}[(1).]
        ...
        \item $u_i(k',\theta_i)+t_i> v_i(k(\theta),\theta_i)$, $\forall i\in I$
    \end{enumerate}
...
\begin{proposition}[ex-post efficient $\Leftrightarrow$ maximizing the sum of utilities]
    $\forall$ project-choice rule $k(\cdot)$, $k(\cdot)$ is ex-post efficient if and only if $k(\cdot)$ maximizes the sum of utilities, i.e., $\forall \theta\in\Theta$ and $\forall k'\in K$,
    \begin{equation}
        \begin{aligned}
