...
\begin{definition}
    \normalfont
    $\tilde{\mu}$ is \textbf{ordinally efficient (sd-efficient)} if there is no $\tilde{\mu}'$ which is $\succ_{FOSD}$ by all agents.\\
    $\tilde{\mu}$ is \textbf{ex-post efficient} if those are only Parteo-efficient outcome in the support.
\end{definition}
