...
\end{definition}
A choice function $C$ can be rationalized if there is a preference relation $\succeq$ on $X$ such that $C=C_\succeq$.

\begin{proposition}[Rubinstein's Condition $\alpha$]
    A choice function $C$ satisfies condition $\alpha$ if for any two problems $A,B$, if $A\subseteq B$ and $C(B)\in A$, then $C(A)=C(B)$.
\end{proposition}
