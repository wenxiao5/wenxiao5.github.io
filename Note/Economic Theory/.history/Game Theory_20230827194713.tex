...
Consider a persuation model where the state space is the interval $[0,1]$ with a common prior $F\in\Delta([0,1])$ that has full support (i.e., $[0,1]$ is the smallest closed set that has probability one).

The sender knows the realized state and the receiver is uninformed. Prior to the realization of the state, the sender commits to a \textbf{signaling policy} $\pi : [0, 1] \rightarrow \Delta(S)$ ,where $S$ is an arbitrary measurable space.
