...
\begin{proof}
    ...
    Given $w_i-p_i>w_j-p_j$. If $z_j^*\leq z_j$, we have $w_j=v_j+z_j^*-p_j$ and stop after visiting $j$. Then, $i$ must be visited before $j$. However, as $v_i+z_i^*-p_i\geq w_i-p_i>w_j-p_j=v_j+z_j^*-p_j$ and the order is descending in $v_j+z_j^*-p_j$, $i$ must be visited before $j$. .. The consumer does not want to visit $j$ after visit $i$ because $v_i+z_i-p_i>v_j+z_j^*-p_j$.
\end{proof}
...
Observable prices:
\begin{enumerate}
    \item As the search costs increase $\Rightarrow$ the benefit from search decreases $\Rightarrow$ attract consumers to search first is more important $\Rightarrow$ Lower prices.
\end{enumerate}
