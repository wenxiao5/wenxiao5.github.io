...
\begin{enumerate}
    ...
    \item logit/limited defender model, $u_{ij}=v_j-p_j+\epsilon_{ij}$, where $\epsilon_{ij}\sim EVI$. The outside option is modeled as $u_{i0}=\epsilon_{i0}\sim EVI$. The market share is given as
    \begin{equation}
        \begin{aligned}
            s_{j}=\textnormal{Pr}(u_{ij}\geq u_{ij'},\forall j')=\frac{\exp(v_j-p_j)}{1+\sum_{j'}\exp(v_{j'}-p_{j'})}
        \end{aligned}
        \nonumber
    \end{equation}
