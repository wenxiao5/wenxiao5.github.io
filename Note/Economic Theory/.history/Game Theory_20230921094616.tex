...
\section{Networks}
Considering a trading network represented by a directed graph, where nodes are firms $F$ and edges $X$ are contracts. The choice function of $f\in F$ is represented by $C^f$, the choice of $f$ over $Y_f$ is $C^f(Y_f)\subseteq Y_f$, where $Y_f\subseteq X_f$ and $X_f$ is the set of contracts involving $f$.
