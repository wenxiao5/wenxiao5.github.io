...
\end{enumerate}
In collusion with private cost shocks, the detectable collusive price cannot depend on the cost. Suppose the collusive price is $p^M$ that generates a profit $\Pi^M(c)=\frac{(p^M-c)(a-p^M)}{2}$ for a firm with cost $c$. The expected profit from sustaining collusion is $W=\int_0^1 \Pi^M(c)dF(c)=\frac{(a-c_L)^2+(a-c_H)^2}{16}$.
