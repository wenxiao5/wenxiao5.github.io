...
\begin{proof}
    ...
    \begin{equation}
        \begin{aligned}
            \max_{f'(\cdot)}\ & -\int_{\theta}f'(\theta)\log\left(\frac{f'(\theta)}{f(\theta)}\right)d\theta+\int_{\theta} \left[v(\theta)-\log\left(\frac{f'(\theta)}{f(\theta)}\right)\right] f'(\theta) d\theta\\
            \textnormal{ s.t. }& \int_{\theta} f'(\theta) d\theta = 1
        \end{aligned}
        \nonumber
    \end{equation}
