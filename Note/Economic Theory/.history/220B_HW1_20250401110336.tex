...
Suppose there are two firms $1,2$ that are competing in a Bertrand competition by setting prices. Each firm $i$ faces an i.i.d. marginal cost $c_i\sim \textnormal{Unif}([\underline{c},\overline{c}])$. The demand for firm $i$ is given by:
\begin{equation}
    \begin{aligned}
        D_i(p_i,p_j)=\left\{\begin{matrix}
            a-p_i,& p_i<p_j,\\
...
\begin{enumerate}
    \item Without collusion, firms compete as a first-price auction.
\end{enumerate}
