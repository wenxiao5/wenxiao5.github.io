...
\begin{theorem}[Form of Extreme Points in $\textnormal{MPC}(f)$: Kleiner et al. (2021), Theorem 1]
    Let $f$ be non-decreasing. Then $g$ is an extreme point in $\textnormal{MPC}(f)$ \underline{if and only if} there exists a collection of disjoint intervals $\{[\underline{x}_i,\bar{x}_i]\}$
\end{theorem}
