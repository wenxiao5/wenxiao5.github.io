...
\begin{theorem}[Equilibrium Existence Duality]
...
\begin{definition}
    \normalfont
    $U^i$ is \underline{net-substitute} if $\forall u$, $D^i_H(p)=\{x\}$ and $D^i_H(p'_j,p_{-j})=\{x'\}$, $p'_j>p_j \Rightarrow x'_k\geq x_k$ for all $k\neq j$.
\end{definition}
