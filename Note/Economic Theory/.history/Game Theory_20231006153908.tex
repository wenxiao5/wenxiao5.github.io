...
\begin{proposition}[Main General Results]
    \begin{enumerate}[$\circ$]
        ...
        \item If $\succeq$ is convex, then the set $\succeq$-best bundle $x\in B(p)$ is convex.
        \item If $\succeq$ is strictly convex, then the set $\succeq$-best bundle $x\in B(p)$ is (at most) a singleton.
        \item If $\succeq$ is differentiable, $x^*\in B(p)$ is a $\succeq$-best bundle, and the vector of marginal values at $x^*$ (as defined above) is denoted by $v(x^*)=(v_1(x^*),...,v_K(x^*))$, where $v_k(x^*)$ is usually taken by $\frac{\partial u}{\partial x_k}(x^*)$ in "classic" problem. Then, we have
        \begin{equation}
            \begin{aligned}
