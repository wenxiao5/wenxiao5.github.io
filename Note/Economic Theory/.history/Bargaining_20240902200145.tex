...
\begin{equation}
    \begin{aligned}
        v(x_0)=\max_{\{\alpha(t)\}_{t\geq 0}}&\int_0^\infty e^{-\rho t}u(x(t),\alpha(t))dt\\
        \textnormal{s.t. }&\frac{dx(t)}{dt}=\mu_t(\alpha_t,x_t)\\
        &x_0=x
    \end{aligned}
    \nonumber
\end{equation}
...
\end{note}
Here, $x_t\in\mathbb{X}\subset \mathbb{R}^N$ is the \textbf{state}, $\alpha_t\in \mathbb{A}\subset \mathbb{R}^M$ is the \textbf{control}, $\rho>0$ is the \textbf{discount factor}, $\mu(\cdot): \mathbb{A}\times \mathbb{X} \rightarrow \mathbb{R}^N$ the \textbf{drift}, and $u(\cdot): \mathbb{A}\times \mathbb{X} \rightarrow \mathbb{R}$ is the instantaneous \textbf{reward} (utility).
