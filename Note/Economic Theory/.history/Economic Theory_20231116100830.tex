...
\begin{proof}
    ...
    Define the correspondence $\varphi \Delta \rightarrow 2^\Delta$ by
    \begin{equation}
        \begin{aligned}
...
\begin{center}
    \begin{tabular}{ccc}
        ...
        \hline
            Allocation $x_j\in X_j,j=1,...,J$& \\
            Price $p\in \mathbb{R}^I$& \\
            Demand $D^j(p)=\arg\max_x\{u(x)\}$&\\
            CE: $(p^*,x^*): x_j^*\in D^j(p^*), \sum_{j}x_j^*\leq S$ (equality holds for no zero $p^*$)&\\
        \hline
    \end{tabular}
    $S$ is supply, which equals to $\sum_i w_i$ if the economy with endowments.
\end{center}
