...
\begin{proof}
    ...
    ``$\Rightarrow$'': Suppose by the way of contradiction that there exists $(\theta,k')$, $\sum_{i=1}^I v_i(k(\theta),\theta_i)\geq \sum_{i=1}^I v_i(k',\theta_i)$. Then, we can define a $t$ such that satisfies $\sum_{i=1}^I t_i=0$ and $u_i(k',\theta_i)+t_i> v_i(k(\theta),\theta_i)$, $\forall i\in I$.
\end{proof}
