...
\begin{corollary}
    Given non-decreasing $f$ and $g$, $f$ majorizes $g$ if and only if $f$ second-order stochastically dominates $g$
\end{corollary}
...
\begin{definition}
    \normalfont
    \begin{enumerate}
        \item The set of non-decreasing functions that are majorized by $f$ is denoted by $$\mathcal{MPS}(f)=\{g\in L^1\mid g \textnormal{ is non-decreasing and }g\prec f\}$$
