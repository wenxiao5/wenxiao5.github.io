...
\begin{theorem}
    Suppose preferences are strongly monotone and the asset structure is complete.
    \begin{enumerate}
...
\begin{theorem}[Reconstruct Assets from Arbitrage-free Prices]
    Consider an asset structure with return matrix $R$. If $q=\mathbb{R}^K$ is arbitrage-free, then there exists $\mu\in \mathbb{R}_{++}^S$ such that $$q=\mu^T R$$ that is, s.t. $$q_k=\sum_s \mu_s r_{ks}, \forall k$$
\end{theorem}
