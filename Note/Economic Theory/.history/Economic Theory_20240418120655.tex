...
\begin{equation}
    \begin{aligned}
        \star=\int_{\underline{\theta}_i}^{\overline{\theta}_i}\int_{s}^{\overline{\theta}_i}\bar{y}_i(s)f_i(\theta_i)d\theta_i ds=\int_{\underline{\theta}_i}^{\overline{\theta}_i}(1-F_i(s))\bar{y}_i(s) ds
    \end{aligned}
    \nonumber
\end{equation}
Hence,
...
\begin{theorem}[Revenue Equivalence Theorem]
    Direct, BIC, and IR auction mechanisms that have the same allocation rule $y(\cdot)$ and the same utilities $(U_i(\theta_i))_{i=1,...,I}$ generate the same revenues \eqref{eq:revenue}.\\
    Take two auction formats $A$ and $B$, fix a BNE of $A$ and a BNE of $B$, interim expected payoff of $\underline{\theta}_i$ is the same for both $A$ and $B$.
\end{theorem}
