...
Notations:
\begin{enumerate}
    \item Let $\mathcal{P}$ denote the set of all preference relations over $X$ (in the mechanism design case, $\mathcal{P}=\Theta$.)
...
\begin{definition}[Properties of a Social Welfare Function (SWF)]
...
\begin{definition}[Harsanyi's three axioms]
    ...
    \begin{enumerate}
        \item \textbf{EU preferences}: each agent's preference $\succcurlyeq_i$ can be represented by the von Neumann-Morgenstern utility function, $x\succcurlyeq_i y\Leftrightarrow\mathbb{E}_xu_i(x;\theta_i)\geq \mathbb{E}_yu_i(y;\theta_i)$. Let $\mathcal{D}$ be the set of such preferences. Then $F: \mathcal{D}^I \rightarrow \mathcal{P}$.
        \item \textbf{Pareto indifference}: If $x\sim_i y$ for all $i\in I$, then $x\sim y$.
        \item \textbf{Social EU}: For each $\trianglerighteq\in \mathcal{text}$
    \end{enumerate}
\end{definition}
