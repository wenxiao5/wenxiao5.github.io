...
Notations:
\begin{enumerate}
    \item Let $\mathcal{P}$ denote the set of all preference relations over $X$.
    \item Let $\mathcal{R} \subseteq \mathcal{P}$ denote the set of all rational preference relations.
    \item Use $\succcurlyeq \in \mathcal{R}$ to represent an individual rational preference relation.
\end{enumerate}

\begin{definition}[Social Welfare Function (SWF)]
    %\normalfont
    Given a preference domain $\mathcal{D} \subseteq \mathcal{R}^I$, a \textbf{social welfare (choice) function} (SWF) is a mapping $$f:\mathcal{D} \rightarrow \mathcal{P}.$$
    The relation $\trianglerighteq = f(\succcurlyeq_1, \ldots, \succcurlyeq_I)$ is interpreted as the \textbf{social preference relation}. Note that it is not required to be rational (i.e., complete and transitive).
\end{definition}
