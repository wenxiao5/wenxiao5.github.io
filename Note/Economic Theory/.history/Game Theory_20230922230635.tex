...
\begin{enumerate}
    ...
    Hence, the distribution of the posterior mean $s$ given the signal policy $\pi$, denoted by $F_\pi\in\Delta([0,1])$ is a \textit{mean-preserving contraction} of $F$ (i.e., $\exists \varepsilon_\omega\in\Delta([0,1])$ such that $\mathbb{E}[\omega\mid s]=0$, $\omega=s+\varepsilon_\omega$ for all $\omega\in F$ and $s\in F_\pi$).
