...
$$v_j = \max_{\alpha_j} \min_{\alpha_{-j}} u_j(\alpha_j,\alpha_{-j})$$
where $\alpha_{-j}$ denotes a profile of mixed strategies for all players except $j$.
\end{definition}

\begin{theorem}[Folk Theorem]
Let $a \in A$ be a feasible action profile such that $u_i(a) > v_i$ for all $i \in N$. Then there exists $\underline{\delta} < 1$ such that for all $\delta \in (\underline{\delta},1)$, there exists a subgame perfect equilibrium of the infinitely repeated game with discount factor $\delta$ where $a$ is played in every period on the equilibrium path.
\end{theorem}

The proof uses Nash reversion - if any player deviates, all players switch to playing the stage-game Nash equilibrium forever after. With sufficiently patient players (high $\delta$), the threat of punishment makes the cooperative outcome sustainable.

For a more general folk theorem (Fudenberg-Maskin 1986), given any feasible payoff profile that strictly dominates the minmax payoffs, there exists a discount factor above which that payoff profile can be approximated arbitrarily closely in some subgame perfect equilibrium.
