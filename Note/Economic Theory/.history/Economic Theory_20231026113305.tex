...
\begin{theorem}[First-order (fundamental) Welfare Theorem]
    If each agent's preference relation is locally non-satiated, then every competitive equilibrium allocation is Pareto optimal (Pareto efficient).
\end{theorem}
\begin{proof}
    ...
    By the definition of CE, $\hat{x}_j$ should not be affordable for $j$, i.e., $p^*\hat{x}_j>p^*w_j$. By the definitions of non-satiated and CE, $p^*\hat{x}_i>p^*w_i$ for all $i\neq j$.
\end{proof}
