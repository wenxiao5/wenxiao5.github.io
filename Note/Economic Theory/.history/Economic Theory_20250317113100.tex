...
\begin{definition}[Weak, Strict, Indifference]
    %\normalfont
    $\succcurlyeq$ referred to as the \textbf{weak preference relation}: "$x$ is at least as good as $y$". (ordinal);\\
    "\textbf{No better than}": $y \preceq x$ if and only if $x \succcurlyeq y$.\\
    "\textbf{Strict preference}": $x \succ y$ if and only if $x \succcurlyeq y$ and not $y \succcurlyeq x$.\\
    "\textbf{Indifference}": $x \sim y$ if and only if $x \succcurlyeq y$ and $y \succcurlyeq x$.
\end{definition}
...
\begin{definition}[Rantional Relation = Preference]
    %\normalfont
    A binary relation $\succcurlyeq$ on $X$ is a \textbf{preference relation} if it is a weak order, i.e., \textbf{complete} and \textbf{transitive}.\\
    {Rationality}: $\succcurlyeq$ is \textbf{rational} \underline{if and only if} it is \textbf{complete} and \textbf{transitive}.
    \begin{enumerate}[$\circ$]
        \item $\succcurlyeq$ is \textbf{complete} iff $\forall x,y\in X$, $x \succcurlyeq y$ or $y \succcurlyeq x$.
        \item $\succcurlyeq$ is \textbf{transitive} iff $\forall x, y, z \in X$, if $x \succcurlyeq y$ and $y \succcurlyeq z$, then $x \succcurlyeq z$.
    \end{enumerate}
\end{definition}
...
\begin{definition}[Unitility Function]
    %\normalfont
    We can say a function $u: X \rightarrow \mathbb{R}$ represents $\succcurlyeq$ if $\forall x,y\in X$, $$x\succcurlyeq y \Leftrightarrow u(x)\geq u(y)$$
\end{definition}

\begin{proposition}[Rational $\succcurlyeq$ $\Rightarrow$ $\exists u(\cdot)$]
    If $\exists$ a function $u: X \rightarrow \mathbb{R}$ represents $\succcurlyeq$, then $\succcurlyeq$ is rational (i.e., completeness and transitivity)
\end{proposition}
\begin{note}
...
\begin{definition}[Convex $\succcurlyeq$]
    %\normalfont
    $\succcurlyeq$ is \textbf{convex} if for every $x\in X$ the $\{y\in X: y\succcurlyeq x\}$ is convex, i.e., $y_1\succcurlyeq x$ and $y_2\succcurlyeq x$ $\Rightarrow$ $\alpha y_1+ (1-\alpha) y_2\succcurlyeq x$ for all $\alpha\in[0,1]$.
\end{definition}
Convex relations imply \textit{averages are preferred to extremes}.

\begin{definition}[Strictly Convex]
    %\normalfont
    $\succcurlyeq$ is \textbf{strictly convex} iff $\forall x, y, z \in X$, if $x \succcurlyeq z$ and $y \succcurlyeq z$, then $\alpha x+(1-\alpha) y \succ z$ for all $\alpha\in (0,1)$
\end{definition}
...
\begin{proposition}[Convex $\succcurlyeq$ $\Leftrightarrow$ quasi-concave $u(\cdot)$]
    $\succcurlyeq$ is convex, $\Leftrightarrow$ $\exists$ a \underline{quasi-concave} $u(\cdot)$ that represents $\succcurlyeq$.
\end{proposition}

\section{Preferences over Nearby Bundles}
\subsection{Monotone Preference}
\begin{definition}[Monotone $\succcurlyeq$]
    %\normalfont
    $\succcurlyeq$ is \textbf{monotone} if $x,y\in X$ with $x\geq y\Rightarrow x\succcurlyeq y$ (and $x> y\Rightarrow x\succ y$).
\end{definition}

\begin{proposition}[Monotone $\succcurlyeq$ $\Rightarrow$ monotone $u(\cdot)$]
    If $\succcurlyeq$ is monotone, then $\exists$ a monotone $u(\cdot)$ that represents $\succcurlyeq$.
\end{proposition}
...
\begin{definition}[Strongly Monotone $\succcurlyeq$]
    %\normalfont
    $\succcurlyeq$ is \textbf{strongly monotone} if and only if for any $x=(x_1,...,x_n), y=(y_1,...,y_n)\in X$, if $\forall i: x_{i} \geq y_{i}$ and $\exists j$ such that $x_{j}>y_{j}$, then $x \succ y$.
\end{definition}
...
\begin{definition}[Independence of Preference]
    %\normalfont
    \textbf{Independence}: For any $x,y,z\in X$ and $0<\alpha<1$, if $x\succcurlyeq y$ then $\alpha x+(1-\alpha)z \succcurlyeq \alpha y+(1-\alpha)z$.
\end{definition}



\subsection{Continuous Preference}
\begin{definition}[Continuous $\succcurlyeq$]
    %\normalfont
    $\succcurlyeq$ is \textbf{continuous} on $X$ \underline{if and only if} for any sequence $\{x^n,y^n\}_{n=1}^\infty$ with $x^n\succcurlyeq y^n$ and we note $x=\lim_{n \rightarrow \infty}x^n$, $y=\lim_{n \rightarrow \infty}y^n$, we have $x\succcurlyeq y$ (i.e., the graph $\{(x,y)\mid x\succcurlyeq y\subseteq X\times X\}$ is closed).
\end{definition}
\begin{proposition}[Debreu's Theorem, Continuous $\succcurlyeq$ $\Rightarrow$ continuous $u(\cdot)$]
    If $\succcurlyeq$ is continuous (on $X$, a convex subset of $\mathbb{R}^k$), then $\exists$ a continuous $u(\cdot)$ that represents $\succcurlyeq$.
\end{proposition}
...
\begin{example}[ (Utility Representation for Lexicographic Preferences)]
    Consider the lexicographic preference $\succcurlyeq$ over the restricted domain $X=\left(\mathbb{Q}\cap[0,1]\right)\times [0,1]$. Enumerate the rationals in $[0,1]$ as $\mathbb{Q}\cap[0,1]=\{q^1,q^2,q^3,...\}$ where $q^i\neq q^j$ if $i\neq j$. The utility representation of this preference is
...
\begin{definition}[Homotheticity]
    %\normalfont
    $\succcurlyeq$ are homothetic if $x\succcurlyeq y \Rightarrow \alpha x\succcurlyeq \alpha y$ for all $\alpha>0$.
\end{definition}

\begin{proposition}[Homothetic preference $\Leftrightarrow$ homogeneous $u(\cdot)$]
    A continuous $\succcurlyeq$ is homothetic $\Leftrightarrow$ $\exists$ a continuous homogeneous $u(\cdot)$ that represents $\succcurlyeq$ such that $u(\alpha x)=\alpha u(x)$ for all $x>0$.
\end{proposition}
...
\begin{definition}[Quasi-Linearity]
    %\normalfont
    $\succcurlyeq$ on $X$ is \textbf{quasi-linear} on $x_1$ if $$x\succcurlyeq y \Rightarrow (x+\epsilon e_1)\succcurlyeq (y+\epsilon e_1)$$ where $e_1=(1,0,...,0)$ and $\epsilon>0$.
\end{definition}

\begin{theorem}[Quasi-Linearity $\Leftrightarrow$ $u(x)=x_1+v(x_{-1})$]
    A continuous $\succcurlyeq$ on $(-\infty,\infty)\times \mathbb{R}_+^{K-1}$ is quasi-linear in $x_1$ $\Leftrightarrow$ $\exists$ a $u(\cdot)$ that represents $\succcurlyeq$ such that $$u(x)=x_1+v(x_{-1})$$
...
\begin{definition}[Separability]
    %\normalfont
    $\succcurlyeq$ satisfies \textbf{separability} if for any $x_i$
    \begin{equation}
        \begin{aligned}
            (x_i, x_{-i})\succcurlyeq (x'_i, x_{-i}) \Leftrightarrow (x_i, x'_{-i})\succcurlyeq (x'_i, x'_{-i})
        \end{aligned}
        \nonumber
    \end{equation}
\end{definition}
...
\begin{theorem}[Separability $\Rightarrow$ Additive $u(\cdot)$]
    $\succcurlyeq$ with \textbf{separability} admits additive $u$-representation
    \begin{equation}
        \begin{aligned}
...
for any $\lambda\leq \lambda^*$. Let $D_{\succcurlyeq}(x)$ be the set of all improvement directions at $x$.

Any improvement is an improvement direction if
\begin{enumerate}[-]
    \item $\succcurlyeq$ are strictly convex.
    \item $\succcurlyeq$ are convex, strongly monotonic, and continuous.
\end{enumerate}

\begin{definition}[Differentiable Preference]
    %\normalfont
    $\succcurlyeq$ is \textbf{differentiable} if there exists a function $v(x): X \rightarrow \mathbb{R}_+^K$ such that $$D_{\succcurlyeq}(x)=D_v(x),\ \forall x\in X$$
\end{definition}

\begin{example}
    $\succcurlyeq$ represented by
    \begin{enumerate}[(1).]
...
\begin{proposition}[Sufficient condition for differentiable $\succcurlyeq$]
    Any (monotonic and convex) $\succcurlyeq$ can be represented by a (strongly monotonic and quasi-concave) and differentiable $u$ is differentiable.
\end{proposition}


\chapter{Choice and Social Choice}
\section{Choice}
...
\begin{definition}[Induced Choice Rule]
    %\normalfont
    Given a binary relation $\succcurlyeq$, the \textbf{induced choice rule} $C_\succcurlyeq$ is defined by
    $C(A)=C_\succcurlyeq(A)=\{x\in A:x\succcurlyeq y, \forall y\in A\}, \forall A\subseteq X$.\\
    A choice function $c$ can be \textbf{rationalizable} if there is a preference relation $\succcurlyeq$ on $X$ such that $c=c_\succcurlyeq$.
\end{definition}

\begin{definition}[Revealed Preference]
    %\normalfont
    Given a choice rule $\succcurlyeq$, its \textbf{revealed preference relation $\succcurlyeq_C$} is defined by $x \succcurlyeq_C y$ if there exists some $A$ such that $x, y \in A$ and $x \in C(A)$.
\end{definition}

\begin{proposition}
    If $C$ is rationalized by $\succcurlyeq$, then $\succcurlyeq=\succcurlyeq_C$.
\end{proposition}
...
\end{definition}
%Condition $\alpha$ is sufficient for $C$ is being formulated by maximizing a preference $\succcurlyeq$.
...
\begin{proposition}[Rubinstein's Condition $\alpha$ $\Rightarrow$ Rationalizable Choice Function $c$]
    \begin{enumerate}[(1).]
        \item  Let $c$ be a choice function defined on a domain containing at least all subsets of $X$ of \underline{size of at most $3$}. If $c$ satisfies condition $\alpha$, then there is a preference $\succcurlyeq$ on $X$ such that $c=c_\succcurlyeq$.
        \item Let $c$ be a choice function with a domain $D$ satisfying that if \underline{$A, B \in D$, then $A \cup B \in D$}. If $c$ satisfies condition $\alpha$, then there is a preference relation $\succcurlyeq$ on $X$ such that $c = c_\succcurlyeq$.
    \end{enumerate}
\end{proposition}
...
\begin{proposition}[Rational $\Rightarrow$ WARP]
    Given $\succcurlyeq$ is rational, then $(C^*_{\succcurlyeq},\mathcal{B})$ satisfies WARP.\\
    ($C^*_{\succcurlyeq}$ is the choice rule that picks the maximal alternatives by $\succcurlyeq$)
\end{proposition}

\begin{proposition}[Sen's Condition $\alpha,\beta$ $\Rightarrow$ Rationalizable Choice Correspondence $C$]
    Let $C$ be a choice correspondence defined on a domain containing at least all subsets of $X$ of \underline{size of at most $3$}. If $C$ satisfies condition $\alpha$ and $\beta$, then there is a preference $\succcurlyeq$ on $X$ such that $C=C_\succcurlyeq$.
\end{proposition}
...
\begin{lemma}[Four Lemmas obtained by the three axioms]
    ...
    \begin{enumerate}[(L1).]
        ...
        \item If $p\succcurlyeq q\succcurlyeq r$ and $p\succ r$ $\Rightarrow$ there exists a unique $\alpha^*\in[0,1]$ such that $$q\sim \alpha^*p+(1-\alpha^*)r$$
        ...
        \begin{equation}
            \begin{aligned}
                \delta_{x_1}\succcurlyeq p\succcurlyeq \delta_{x_2}
            \end{aligned}
            \nonumber
        \end{equation}
    \end{enumerate}
...
\begin{enumerate}
    ...
    \item We use $\succcurlyeq\in \mathcal{R}$ to represents individual rational preference relation.
\end{enumerate}

\subsection{Social Welfare Function and Properties}
\begin{definition}[Social Welfare Function (SWF)]
    %\normalfont
    Given the preference domain $\mathcal{D}\subseteq \mathcal{R}^I$,
    a \textbf{social welfare (choice) function} (SWF) is a mapping $$f:\mathcal{D} \rightarrow \mathcal{P}$$
    $\trianglerighteq=f(\succcurlyeq_1,...,\succcurlyeq_I)$ is interpreted as the \textbf{social preference relation}. It doesn't need to be rational (i.e., complete and transitive).
\end{definition}

\begin{definition}[Properties of a Social Welfare Function (SWF)]\label{SWF_properties}
    A social welfare function $f: \mathcal{D} \rightarrow \mathcal{P}$ satisfies the following properties:
    \begin{enumerate}[$\circ$]
        \item \textbf{Unrestricted Domain (UD)}: The domain $\mathcal{D}$ includes all possible preference profiles, i.e., $\mathcal{D} = \mathcal{R}^I$.
        \item \textbf{Transitivity (T)}: The social preference relation $f(\succcurlyeq_1, \ldots, \succcurlyeq_I)$ is transitive for all $(\succcurlyeq_1, \ldots, \succcurlyeq_I) \in \mathcal{D}$.
        \item \textbf{Non-Dictatorship (ND)}: There is no agent $i \in I$ such that, for all $\{x, y\} \subseteq X$, $x \succcurlyeq_i y$ implies $x \trianglerighteq y$. (In other words, no single agent always determines the social preference.)
        \item \textbf{Weak Pareto Efficiency (PA)}: For all $\{x, y\} \subseteq X$ and any preference profile $(\succcurlyeq_1, \ldots, \succcurlyeq_I) \in \mathcal{D}$, if $x \succcurlyeq_i y$ for all $i \in I$, then $x \trianglerighteq y$.
...
The DM's problem is finding the $\succcurlyeq$-optimal bundle $x\in B(p)$. With the corresponding utility function $u(x)$, we can consider a consumer's problem
\begin{equation}
    \begin{aligned}
        \max_{x\in X} u(x)\\
        s.t.\ p\cdot x\leq w
    ...
    \label{UMP}
\end{equation}
The set $\succcurlyeq$-optimal bundle is represented by $x(p,w)$.
...
\begin{proposition}[Continuous Preference$\Rightarrow$ Solution $x(p,w)$ Existence]
    If $\succcurlyeq$ ($u(\cdot)$) is continuous, then all such problems have a solution $x(p,w)$.
\end{proposition}
\begin{proof}
...
\begin{proposition}[Convex Preference$\Rightarrow$ Convex $x(p,w)$]
    If $\succcurlyeq$ is convex ($u(\cdot)$ is quasi-concave), then $x(p,w)$ is convex.
\end{proposition}
\begin{proof}
    Suppose $x,x'\in X$. The optimal utility $u^*=u(x)=u(x')$. For any $\alpha\in[0,1]$, let $x''=\alpha x+(1-\alpha)x'$. Because $\succcurlyeq$ is convex, we have $u(\cdot)$ is quasi-concave, that is $u(x'')\geq u^*$. $x''$ is also feasible. So, $x''\in x(p,w)$.
\end{proof}

\begin{proposition}[Strictly Convex Preference$\Rightarrow$ Singleton $x(p,w)$]
    If $\succcurlyeq$ is strictly convex ($u(\cdot)$ is strictly quasi-concave), then $x(p,w)$ is (at most) a singleton.
\end{proposition}

\begin{proposition}[Differentiable Preference$\Rightarrow$ Margianl Utility equals to Price]
    If $\succcurlyeq$ is differentiable, $x^*\in x(p,w)$, and the vector of marginal values at $x^*$ (as defined above) is denoted by $v(x^*)=(v_1(x^*),...,v_K(x^*))$, where $v_k(x^*)$ is usually taken by $\frac{\partial u}{\partial x_k}(x^*)$ in "classic" problem. Then, we have
    \begin{equation}
        \begin{aligned}
...
\begin{corollary}[Sufficient Conditions for Optimality]
    If $\succcurlyeq$ is strongly monotonic, convex, continuous, and differentiable and if $p\cdot x^*=w$ and \eqref{*} is satisfied then $x^*\in x(p,w)$
\end{corollary}

\begin{definition}[Rationalize]
    %\normalfont
    $\succcurlyeq$ \textbf{fully rationalize} the demand function $x$ if for any $(p,w)$, the bundle $x(p,w)$ is the \underline{unique} $\succcurlyeq$-maximal bundle within $B$.\\
    A monotonic $\succcurlyeq$ \textbf{rationalize} the demand function $x$ if for any $(p,w)$, the bundle $x(p,w)$ is a $\succcurlyeq$-maximal bundle within $B$.
\end{definition}
...
\begin{proposition}[Properties of Marshallian Demand]
    \begin{enumerate}[(i).]
        \item \textbf{Walras' Law}: If $\succcurlyeq$ is local nonsatiation, $\forall x^*\in x(p,w): p\cdot x^*=w$.
        \item \textbf{Homogeneity of degree zero in $( p, w )$:} $x (\alpha p, \alpha w )\equiv x ( p, w ),\ \forall \alpha > 0$.
        \item \textbf{Continuous in prices and in wealth} if the $\succcurlyeq$ is continuous.
    \end{enumerate}
\end{proposition}
...
\begin{enumerate}
    ...
    \item Each agent has a preference relation $\succcurlyeq_i$ on $\mathbb{R}_+^L$ represented by a utility function $u_i: \mathbb{R}_+^L \rightarrow \mathbb{R}$.
...
\begin{definition}[Pareto Optimal]
    %\normalfont
    An allocation $x$ is \textbf{Pareto optimal/efficient} if there doesn't exist an allocation $y$ s.t. $u_i(y_i)\geq u_i(x_i)$ ($y_i\succcurlyeq_i x_i$) for each $i$ and $u_j(y_j)> u_j(x_j)$ ($y_j\succ_j x_j$) for some $j$.
\end{definition}
...
\begin{definition}[Individually Rational]
    %\normalfont
    A \underline{bundle} $x_i$ is \textbf{individually rational} (IR) for agent $i$ if $x_i\succcurlyeq_i w_i$.\\
    An \underline{allocation} $x=(x_1,...,x_m)$ is \textbf{individually rational} (IR) if $x_i\succcurlyeq_i w_i$ for all $i=1,...,m$.
\end{definition}
...
\begin{definition}[Block]
    ...
    \begin{enumerate}[1.]
        ...
        \item $y_i\succcurlyeq_i x_i$ for all $i\in S$;
...
\begin{proof}
    ...
    Suppose by way of contradiction that $x^*$ is \underline{not} P.O. allocation. Then, there is another allocation $\hat{x}=(\hat{x}_1,...\hat{x}_n)$ such that $\hat{x}_j\succ_j x^*_j$ for some $j\in N$ and $\hat{x}_i\succcurlyeq_i x^*_i$ for all $i\neq j$.\\
    By the definition of CE, $\hat{x}_j$ should not be affordable for $j$, i.e., $p^*\hat{x}_j>p^*w_j$. By the definitions of non-satiated and CE, $p^*\hat{x}_i\geq p^*w_i$ for all $i\neq j$. (If $p^*\hat{x}_i<p^*w_i$, $\exists \tilde{x}_i$ s.t. $p^*\tilde{x}_i\leq p^*w_i$ and $\tilde{x}_i\succ_i\hat{x}_i\succcurlyeq_i x_i^*$, which contradicts to the definition of CE.)\\
...
\begin{definition}[Supported by a price; Supported]
    %\normalfont
    An allocation $\bar{x}=\left(\bar{x}_1,...,\bar{x}_m\right)$ in an exchange economy is \textbf{supported by a non-zero price vector $p\in \mathbb{R}^L$} if $$\forall i: x_i\succcurlyeq_i\bar{x}_i \Rightarrow p\cdot x_i\geq p\cdot\bar{x}_i$$
    If an allocation $\bar{x}=\left(\bar{x}_1,...,\bar{x}_m\right)$ is \textbf{supported}, then a \underline{common} price $p$ supports each agent's ``better-than set'' at $\bar{x}_i$ ($\{x_i\in \mathbb{R}_L^+: x_i\succcurlyeq_i\bar{x}_i\}$): $\forall x_i\in \{x_i\in \mathbb{R}_L^+: x_i\succcurlyeq_i\bar{x}_i\}: p\cdot x_i\geq p\cdot \bar{x}_i$.
\end{definition}
\begin{note}
...
If $\succcurlyeq_i$ is locally non-satiated for all $i$, then $\forall i$: $p\cdot x_i(p)=p\cdot w_i, \forall p$. Adding over agents: $p\cdot \sum_{i=1}^mx_i(p)=p\cdot w, \forall p$.
...
\begin{enumerate}
    ...
    \item There are $m$ consumers. Each consumer $i=1,...,m$ has a preference relation $\succcurlyeq_i$ represented by a utility function $u_i: \mathbb{R}_+^L \rightarrow \mathbb{R}$, an initial endowment $w_i\in \mathbb{R}_+^L$, and owns shares $\{\theta_{ij}:j=1,...,J\}$ in the $J$ firms, where $\theta_{ij}\geq 0, \forall i,j$ and $\sum_{i=}^m\theta_{ij}=1, \forall j$.
...
\begin{definition}[Pareto Optimal]
    %\normalfont
    An allocation $(x^*,y^*)$ in a private ownership production economy is \textbf{Pareto optimal} if there is no other allocation $(x,y)$ s.t. $x_i\succcurlyeq_i x_i^*, \forall i$ and $x_h\succ_h x_h^*$ for some $h$.
\end{definition}
...
\end{theorem}
\begin{proof}
    Let $(x^*,y^*)$ be a competitive equilibrium allocation with corresponding equilibrium price vector $p^*$. Suppose by the way of contradiction that $(x,y)$ is not Pareto optimal. That is, $\exists$ an allocation $(x,y)$ s.t. $x_i\succcurlyeq_i x_i^*, \forall i$ and $x_h\succ_h x_h^*$ for some $h$. Then, by the \eqref{eq:x_star},
    \begin{equation}
        \begin{aligned}
...
Consider an exchange economy. Let $p\in \partial \Delta$. Let $w_i>>0$. If $\succcurlyeq_i$ is strongly monotone on $\mathbb{R}^L_{++}$, then demand of agent $i$ is undefined at $p_i$ (infinity for the zero price good).
...
Let $\mathcal{E}(\vec{w})$ denote the economy with fixed preference relations $\{\succcurlyeq_i:i=1,...,m\}$ and endowment profile $\vec{w}$.
...
\begin{enumerate}
    ...
    \item Each consumer has preference relation $\succcurlyeq_i$ over $\mathbb{R}_+^{L\times(S+1)}$ represented by utility function $u_i$ and initial endowment under different states $w_i=(w_{i0},w_{i1},...,w_{iS})\in \mathbb{R}_+^{L\times(S+1)}$, where $w_{i0}$ is the endowment at $t=0$ and $w_{is}$ is the endowment at $t=1$ with state $s\in\{1,...,S\}$.
\end{enumerate}
