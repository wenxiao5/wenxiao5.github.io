...
Assume a TU (transferable utility) Economy. Consider a payoffs vector for all players, $x\in \mathbb{R}^n$. The efficiency requires $\sum_{i\in N}x_i=v(N)$.

Individual Rational (IR) requires $x_i\geq v(\{i\})$.

Define the \underline{core}: $$C(v,N)=\left\{x\in \mathbb{R}^n: \sum_{i\in N}x_i=v(N), \sum_{i\in S}x_i\geq v(S), \forall S\subseteq N\right\}$$

$B(N)$ be the solutions to: $\left\{\begin{matrix}
    \sum_{S:i\in S}y_S=1,&\forall i\in N\\
    y_S\geq 0,& \forall S\subseteq N
\end{matrix}\right.$

\begin{theorem}[Bondareva-Shapley]
    $C(v,N)\neq \emptyset$ if and only if $v(N)\geq \sum_{S\subseteq N}v(S)y_S, \forall y\in B(N)$
\end{theorem}
\begin{proof}
    \begin{lemma}[Favkas Lemma]
        Let $A$ be $m\times n$ matrix, $b\in \mathbb{R}^m$ and $F=\{x\in \mathbb{R}^n: Ax=b,x\geq 0\}$. Then, either $F\neq \emptyset$ or $\exists y\in \mathbb{R}^m$ such that $A^Ty\geq 0$ and $b^T y<0$, but not both.
    \end{lemma}
    \begin{lemma}[(Alternative) Favkas Lemma]
        Let $A$ be $m\times n$ matrix, $b\in \mathbb{R}^m$ and $F=\{x\in \mathbb{R}^n: Ax\geq b, Cx=d,x\geq 0\}$. Then, either $C^Tz-A^Ty_S=0, y_S\geq 0$ or $\exists z$ such that $d^Tz-b^Ty_S<0$, but not both.
    \end{lemma}
    By using this lemma, we can conclude either $\left\{\begin{matrix}
        v(N)z-\sum_S v(S)y_S<0\\
        z-\sum{y_S}=0\\
        y_S\geq 0
    \end{matrix}\right.$ (let $z=1$, the last two lines are $B(N)$) or 
