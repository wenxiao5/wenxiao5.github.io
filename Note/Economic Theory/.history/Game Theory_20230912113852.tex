...
\begin{proposition}[Convex $\succeq$ $\Leftrightarrow$ quasi-concave $u(\cdot)$]
    Rational $\succeq$ is convex, $\Leftrightarrow$ $\exists$ a \underline{quasi-concave} $u(\cdot)$ that represents $\succeq$.
\end{proposition}
...
\begin{proposition}[Homothetic preference $\Rightarrow$ homogeneous $u(\cdot)$]
...
\begin{theorem}[Quasi-Linearity $\Rightarrow$ $u(x)=x_1+v(x_{-1})$]
