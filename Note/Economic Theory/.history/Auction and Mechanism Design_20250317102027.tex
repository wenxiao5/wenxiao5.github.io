...
\begin{definition}[SCF's Properties]\label{SWF_properties}
    ...
    \begin{enumerate}[$\circ$]
        \item has \textbf{unrestricted domain / universal domain} (UD) if its domain contains all preferences of agents, $f:\Theta \rightarrow X$ (generally satisfied);
        \item is \textbf{non-dictatorial} (ND) $\nexists i^*\in\{1,...,I\}$ such that $f(\theta)\in \argmax_{x\in X}u_{i^*}(x;\theta_{i^*})$ for all $\theta\in \Theta$. (That is there is no agent that the social outcome always aligns his preference.)
        \item is \textbf{weakly Paretian} (PA, i.e., Unanimity) if, $\forall i\in\{1,...,I\}$ and any preference profile $(\theta_1,...,\theta_I)\in \Theta$, we have $x\succeq_i y,\forall i\in I \Rightarrow x\trianglerighteq y$.
