...
\begin{definition}[Mean-Preserving Spread]
    \normalfont
    Let $x_F$ and $x_G$ be the random variables associated with lotteries $F$ and $G$. Then $G$ is a \textbf{mean-preserving spread} of $F$ if and only if $$x_G \stackrel{d}{=} x+\varepsilon$$
    for some $x \sim F$, $y \sim G$ and $\varepsilon$ such that $\mathbb{E}(\varepsilon\mid x)=0$ for all $x$.
\end{definition}
