...
\textbf{Facts}: A upper hemicontinuous function attains the maximum over a comapct set. Sum of upper hemicontinuous functions is upper hemicontinuous.
...
\begin{proof}
    ...
    \begin{enumerate}
        \item \underline{Case 1:} ``$k^*(\theta_i,\theta_{-i})=k^*(\hat{\theta}_i,\theta_{-i})$'', which requires
        \begin{equation}
            \begin{aligned}
        ...
        \item \underline{Case 2:} ``$k^*(\theta_i,\theta_{-i})\neq k^*(\hat{\theta}_i,\theta_{-i})$'', WLOG, we consider $k^*(\theta_i,\theta_{-i})>k^*(\hat{\theta}_i,\theta_{-i})$.\\
        There is an $\epsilon>0$ s.t. $k^*(\theta_i,\theta_{-i})\neq k^*(\hat{\theta}_i,\theta_{-i})+\epsilon$.\\
        ...
        \begin{enumerate}[$\circ$]
            ...
            \begin{equation}
                ...
                \nonumber
            \end{equation}
            Hence, we can conclude $k^*(\theta^\epsilon_i,\theta_{-i})=k^*(\hat{\theta}_i,\theta_{-i})$.
            \item Because $f(\cdot)=\left(k^*(\cdot),t_i(\cdot)\right)$ is DSIC, the agent with $\theta^\epsilon_i$ gets the highest profit from truthfully reproting
            \begin{equation}
                \begin{aligned}
                    v_i(k^*(\hat{\theta}_i,\theta_{-i}),\theta^\epsilon_i)+t_i(\theta^\epsilon_i,\theta_{-i})\geq v_i(k^*(\theta_i,\theta_{-i}),\theta^\epsilon_i)+t_i(\theta_i,\theta_{-i})\\
                    \Rightarrow -\sum_{j\neq i}v_j(k^*(\hat{\theta}_i,\theta_{-i}),\theta_j)+\epsilon+t_i(\theta^\epsilon_i,\theta_{-i})\geq-\sum_{j\neq i}v_j(k^*(\theta_i,\theta_{-i}),\theta_j)+t_i(\theta_i,\theta_{-i})
                \end{aligned}
                \nonumber
            \end{equation}
        \end{enumerate}
