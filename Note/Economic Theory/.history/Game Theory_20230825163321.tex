...
\begin{proposition}[Non-decreasing $f$ $\Rightarrow$ $\textnormal{MPS}(f)$, $\textnormal{MPS}_w(f)$, and $\textnormal{MPC}(f)$ have extreme points]
    Suppose $f\in L^1$ is non-decreasing. Then $\textnormal{MPS}(f)$, $\textnormal{MPS}_w(f)$, and $\textnormal{MPC}(f)$ are convex and compact in the norm topology $\Rightarrow$ they all have non-empty set of extreme points.
\end{proposition}
\begin{note}
    We use $\textnormal{ext} A$ to denote the set of extreme points of set $A$.
\end{note}
\begin{proposition}
    Suppose $f\in L^1$ is non-decreasing. For any $g\in \textnormal{MPS}(f)$, $\exists$ a probability measure $\lambda_g$ over $\textnormal{ext} \textnormal{MPS}(f)$
\end{proposition}
