...
\begin{definition}[Majorization]
    \normalfont
    Consider right-continuous functions that map the unit interval $[0,1]$ into the real numbers. For two non-decreasing functions $f,g \in L^1$, we say that $f$ \textbf{majorizes} $g$, denoted by $g \prec \succ f$, if the following two conditions hold:
\end{definition}
