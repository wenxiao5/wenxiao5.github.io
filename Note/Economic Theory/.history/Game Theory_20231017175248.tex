...
\begin{enumerate}
    ...
    \subitem $C$ accepts ($R$ and $C$ get period payoffs $(1-x_k,x_k)$ and $\textnormal{Pr}(k_{t+1}=h)=q$, $\textnormal{Pr}(k_{t+1}=l)=1-q$) or rejects ($R$ and $C$ get period payoffs $(1-p_k-f,p_k+f)$ and $\textnormal{Pr}(k_{t+1}=w_C)=p_k$, $\textnormal{Pr}(k_{t+1}=w_R)=1-p_k$).
\end{enumerate}
