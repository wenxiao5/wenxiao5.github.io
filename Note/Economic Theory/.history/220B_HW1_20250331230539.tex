...
If without cost shocks $c_L=c_H$, we have $\Pi^M(c_L)=\Pi^M(c_H)=\frac{\Pi^D(c_L)}{2}=W$ and $\Pi^P=0$. Thus, the denominator of the right-hand side increases and the numerator (weakly) decreases, so the collusion is more likely to sustain without cost shocks.
