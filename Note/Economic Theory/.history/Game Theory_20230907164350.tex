...
\begin{definition}[Convex $\succeq$]
    \normalfont
    $\succeq$ is convex if $\forall x\in X$ the $\{y\in X: y\succeq x\}$ is convex, that is, $y_1\succeq x$ and $y_2\succeq x$ $\Rightarrow$ $\alpha y_1+ (1-\alpha) y_2\succeq x$ for all $\alpha\in[0,1]$.
\end{definition}
