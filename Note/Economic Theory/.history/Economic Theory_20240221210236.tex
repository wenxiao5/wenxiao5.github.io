...
\begin{enumerate}
    ...
    \begin{definition}[Information Function]
        \normalfont
        We define the function $P : \Omega \rightarrow 2^\Omega$ an \textbf{information function}, which \textcolor{red}{is the set of states that the agent considers possible when the actual state is $\omega$}.
    \end{definition}
    ...
    Interpretation: $K(E)$ (which we write as $KE$) is the event (the set of states) at which the agent knows that the event $E$ has occurred.
    ...
    \begin{definition}[Knowledge given Information Function]
        \normalfont
        Given an information function $P: \Omega \rightarrow 2^\Omega$, we define the \textbf{knowledge} $K : 2^\Omega \rightarrow 2^\Omega$ by, $$KE=\{\omega\in\Omega:P(\omega)\subseteq E\}$$
    \end{definition}
    \item Given a state $\omega$, $\{E \subseteq \Omega : KE \ni \omega\}$ is the set of all events that the agent ``knows'' (all events that the agent believes are possible). The most accurate information provided by it is $\cap\{E \subseteq \Omega : KE \ni \omega\}$. $$P(\omega) = \cap\{E \subseteq \Omega : KE \ni \omega\}$$
    \begin{definition}[Information Function given Knowledge]
        \normalfont
        Given knowledge function $K:2^\Omega \rightarrow 2^\Omega$, we define the function $P : \Omega \rightarrow 2^\Omega$ an \textbf{information function}, which \textcolor{red}{is the set of states that the agent considers possible when the actual state is $\omega$}.
    \end{definition}
\end{enumerate}
