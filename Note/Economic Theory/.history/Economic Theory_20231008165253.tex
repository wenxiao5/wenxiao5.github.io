...
\begin{proof}[Proof for Derivative]
    \begin{enumerate}
        ...
        & \equiv \sum_{j=1}^{K} \lambda^{*} u_{j}\left(x^{*}\right) \frac{\partial h_{j}}{\partial p_{i}}+h_{i} \\
        &=\lambda^{*}\left[\sum_{j=1}^{K} u_{j}\left(x^{*}\right) \frac{\partial h_{j}}{\partial p_{i}}\right]+h_{i}\\&=h_i
        \end{aligned}
        $$
        \item 
    \end{enumerate}
