...
We consider \underline{finite} set of alternatives $X$ and \underline{finite} set of agents $I$. We use $\mathcal{R}$ to denotes the set of all rational preference relations. We use $\succeq\in \mathcal{R}$ to represents individual rational preference relation.

\begin{definition}[Social Welfare Function (SWF)]
    \normalfont
    A \textbf{social welfare function} (SWF) is a mapping $f: \mathcal{A}\subseteq \mathcal{R}^I\rightarrow \mathbb{R}$
\end{definition}

\begin{definition}[Social Preference Relation]
    \normalfont
    $\trianglerighteq=f(\succeq_1,...,\succeq_I)$ is interpreted as the \textbf{social preference relation}. It doesn't need to be rational (i.e., complete & transitive).
\end{definition}
$\trianglerighteq\in \mathcal{R}$ to represent social (rational) preference relation.

\begin{proposition}[Pareto Condition]
    The SWF $f: \mathcal{A}\rightarrow \mathbb{R}$ satisfies Pareto condition if $\forall |{x,y}\subseteq X$ and any preference profile $(\succeq_1,...,\succeq_I)\in \mathcal{A}$, we have $x\succeq_i y,\forall i\in I \Rightarrow x\trianglerighteq y$.
\end{proposition}

\begin{proposition}[Independence of Irrelevant Alternatives]
    $\forall \{x,y\}\subseteq X$, and any $\succeq$ and $\succeq'$ with $\succeq_i\mid_{x,y}=\succeq'_i\mid_{x,y}, \forall i\in I$, if $x\trianglerighteq y$ then $x\trianglerighteq' y$.
\end{proposition}

\begin{proposition}[ND]
    No agent $i\in I$ such that, $\forall \{x,y\}\subseteq X$ $x\succeq_i y \Rightarrow x\trianglerighteq y$.
\end{proposition}

\begin{theorem}
    Suppose $|X|\geq 3$, $\mathcal{A}=\mathcal{R}^I$. Then there is no $f'$ that satisfies (1), (2), (3).
\end{theorem}
...
\section{Extreme Points and Majorization}
