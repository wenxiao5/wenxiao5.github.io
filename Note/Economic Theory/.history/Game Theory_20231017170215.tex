...
\begin{example}
    Consider PD(Prison Dilemma) as a stochastic game, $K=\{PD\}$, $A_i(PD)=\{0,1\}$, $u_i(a,PD)=1-a_{i,t}+2 a_{-i,t}$, $\textnormal{Pr}(PD|PD,a_t)=1$.\\
    ...
    \begin{equation}
        \begin{aligned}
            \sigma_i=\argmax_{\sigma'_i\in[0,1]}\delta^{t'}(1-\sigma'_i+2\sigma_{-i})+\sum_{t=t'+1}^\infty \delta^{t-1}(1-\sigma_i+2\sigma_{-i})=0
        \end{aligned}
        \nonumber
    \end{equation}
\end{example}
