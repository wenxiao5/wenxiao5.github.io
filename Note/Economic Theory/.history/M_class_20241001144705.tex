...
\begin{proof}
    ...
    Given $w_i-p_i>w_j-p_j$. If $z_j^*\leq z_j$, we have $w_j=v_j+z_j^*-p_j$ and stop after visiting $j$. Then, $i$ must be visited before $j$. However, as $v_i+z_i^*-p_i>w_i-p_i>w_j-p_j=v_j+z_j^*-p_j$ and the order is descending in $v_j+z_j^*-p_j$, $i$ must be visited before $j$.
\end{proof}
