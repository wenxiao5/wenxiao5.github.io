...
\subsection{Two Models of Collusion}
\subsubsection{Rotemberg \& Saloner (1986): Collusion in Booms and Busts}
\begin{itemize}
    \item Study implicit collusion by oligopoly when demand fluctuates over time
    \item Key insight: Collusion is harder when demand is high
    \begin{itemize}
        \item Benefit from deviating is larger during booms
        \item This limits firms' ability to charge high prices
        \item Results in countercyclical prices
    \end{itemize}
\end{itemize}

\textbf{Model Setup:}
\begin{itemize}
    \item N symmetric firms produce homogeneous good
    \item Infinitely repeated Bertrand competition
    \item Demand subject to i.i.d. shocks $\epsilon_t \in [a,b]$ with cdf $F$
    \item Inverse demand $P(Q_t,\epsilon_t)$ increasing in $\epsilon$
    \item Constant marginal costs $c$
    \item Common discount factor $\delta$
\end{itemize}

\textbf{Timing:}
\begin{enumerate}
    \item All firms learn $\epsilon_t$
    \item All choose prices simultaneously
    \item Complete information and perfect monitoring
\end{enumerate}

\textbf{Equilibrium Analysis:}
\begin{itemize}
    \item Under price competition, harshest punishment is reversion to marginal cost pricing
    \item Let $\Pi^M(\epsilon)$ be $\frac{1}{N}$ monopoly profits in state $\epsilon$:
    \[\Pi^M(\epsilon) = \frac{1}{N}\max_Q(P(Q,\epsilon)-c)Q\]
    \item Deviation yields $N\Pi^M(\epsilon)$
    \item Let $W$ be collusive continuation value
    \item Deviation occurs if:
    \[N\Pi^M(\epsilon) > \Pi^M(\epsilon) + \delta W \Leftrightarrow \Pi^M(\epsilon) > \frac{\delta W}{N-1}\]
    \item Define threshold $\epsilon^*$ where $\Pi^M(\epsilon^*) = \frac{\delta W}{N-1}$
    \item Sustainable profits:
...
\textbf{Key Results:}
\begin{itemize}
    \item Countercyclical prices emerge in equilibrium
    \item Monopoly pricing during recessions
    \item Prices below monopoly level during booms
    \item Empirical evidence from supermarket pricing supports theory
\end{itemize}

\subsubsection{Green \& Porter (1984): Imperfect Monitoring}
\begin{itemize}
