...
\begin{proof}[Derivation from Discrete-time Bellman]
    ...
    \begin{equation}
        ...
        \nonumber
    \end{equation}
    where $x(t+\Delta)=x(t)+\Delta [f(x(t),\alpha(t))]$. For small $\Delta$ (will take $\Delta \rightarrow 0$), $e^{-\rho\Delta}=1-\rho\Delta$.
    \begin{equation}
        \begin{aligned}
            v(x(t))=\max_{\alpha_t}\ r(x(t),\alpha(t)) + (1-\rho\Delta)v(x(t+\Delta))
        \end{aligned}
        \nonumber
    \end{equation}
\end{proof}
