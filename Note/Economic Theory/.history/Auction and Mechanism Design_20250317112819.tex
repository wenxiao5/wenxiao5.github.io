...
To formally show this theorem, we need to formally define the social choice function as the Arrow framework. To distinguish it with the social choice function in our mechanism design framework (which is actually social choice mechanism), we call the social choice function in the Arrow framework a \textbf{social welfare function (SWF)}.

Notations:
\begin{enumerate}
    \item We use $\mathcal{P}$ to denotes the set of all preference relations over $X$
...
\begin{definition}[SWF's Properties]\label{SWF_properties}
    ...
    \begin{enumerate}[$\circ$]
        \item has \textbf{unrestricted domain / universal domain} (UD) if $\mathcal{D}=\mathcal{R}^I$;
        ...
        \item is \textbf{independent of irrelevant alternatives} (IIA) if, given $\trianglerighteq,\trianglerighteq'\in \mathcal{R}^I$, if $x \succcurlyeq_i y \Leftrightarrow x \succcurlyeq'_i y$ for all $i\in I$ and $x \trianglerighteq y$, then $x \trianglerighteq' y$.
    \end{enumerate}
\end{definition}
...
\begin{definition}[Harsanyi's three axioms]
    ...
    \begin{enumerate}
        \item \textbf{EU preferences}: each agent's preference can be represented by the von Neumann-Morgenstern utility function, $x\succcurlyeq_i y\Leftrightarrow\mathbb{E}_xu_i(x;\theta_i)\geq \mathbb{E}_yu_i(y;\theta_i)$.
        \item \textbf{Pareto indifference}: If $x\succcurlyeq_i y$ for all $i\in I$, then $x\sim y$.
        \item 
    \end{enumerate}
\end{definition}
