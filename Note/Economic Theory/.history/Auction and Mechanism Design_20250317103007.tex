...
\begin{definition}[SCF's Properties]\label{SWF_properties}
    %\normalfont
    A social choice function $f$'s properties are defined as follows:
    \begin{enumerate}[$\circ$]
        \item \textbf{Unrestricted domain / universal domain} (UD): the domain of $f$ contains all preferences of agents, $f:\Theta \rightarrow X$ (generally satisfied);
        \item \textbf{Non-dictatorial} (ND): $\nexists i^*\in\{1,...,I\}$ such that $f(\theta)\in \argmax_{x\in X}u_{i^*}(x;\theta_{i^*})$ for all $\theta\in \Theta$. (That is there is no agent that the social outcome always aligns his preference.)
        \item \textbf{Weakly Paretian} (PA, i.e., Unanimity): $\nexists x\neq f(\theta)$ such that $u_i(x;\theta_i)\geq u_i(f(\theta);\theta_i),\forall i\in I$ and $u_j(x;\theta_j)> u_j(f(\theta);\theta_j)$ for some $j\in I$.
        \item is \textbf{independent of irrelevant alternatives} (IIA) if, $\forall \theta,\theta'\in \Theta$, $\forall x,y\subseteq X$, and any $\succeq$ and $\succeq'$ with $\succeq_i\mid_{x,y}=\succeq'_i\mid_{x,y}, \forall i\in I$, if $x\trianglerighteq y$ then $x\trianglerighteq' y$.
    \end{enumerate}
\end{definition}
