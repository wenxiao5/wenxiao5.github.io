...
\begin{definition}[First-order Stochastic Dominance]
    \normalfont
    For any lotteries $F$ and $G$, $F$ \textbf{first-order stochastically dominates} $G$ if and only if the decision maker weakly prefers F to G under every weakly increasing 
utility function u , i.e.,  u ( x ) dF ≥  u (x)dG. 
\end{definition}
