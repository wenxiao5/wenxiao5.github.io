...
\end{equation}
\begin{note}
    If we choose the $h(\cdot)$ that is a differentiable function vanishing on its boundary, i.e., $h(\underline{x})=h(\bar{x})=0$. We have
    \begin{equation}
        \begin{aligned}
...
Consider the two functionals over function $y: X \rightarrow \mathbb{R}$, $W(y)=\int_X F(y(x))dx$ and $T(y)=\int_X G(y(x))dx$.

The optimization problem is
\begin{equation}
    \begin{aligned}
        \max_{y(\cdot)}\ &W(y):=\int_X F(y(x))dx\\
        \textnormal{s.t. }& T(y):=\int_X G(y(x))dx=0
    \end{aligned}
    \label{P1}
    \tag{P1}
\end{equation}
...
\begin{equation}
    \begin{aligned}
        \mathcal{L}(y;\lambda):&=W(y)+\lambda T(y)\\
...
\begin{equation}
    \begin{aligned}
        \frac{\partial \mathcal{L}(y;\lambda)}{\partial y(x)}\bigg|_{y^*(x)}=F'(y^*(x))+\lambda G'(y^*(x))=0, \textnormal{ for each }x\in X
    \end{aligned}
    \label{P1_N1}
    \tag{P1-N1}
\end{equation}
