...
\begin{proposition}[Gree \& Laffont 77]
...
\end{proposition}
\begin{proof}
    Take any $f(\cdot)=\left(k^*(\cdot),t_i(\cdot)\right)$ such that it is ex-post efficient and DSIC.\\
    ...
    \begin{enumerate}
        ...
        \begin{equation}
            ...
            \nonumber
        \end{equation}
        Since $v_i(k^*(\hat{\theta}_i,\theta_{-i}),\hat{\theta}_i)=v_i(k^*(\theta_i,\theta_{-i}),\hat{\theta}_i)$, we have $t_i(\hat{\theta}_i,\theta_{-i})\geq t_i(\theta_i,\theta_{-i})$.
    \end{enumerate}
\end{proof}
