...
\end{proposition}
\begin{proof}
    By choosing the $h(\cdot)$ that is a differentiable function vanishing on its boundary, i.e., $h(\underline{x})=h(\bar{x})=0$, we have the equation \eqref{GD_van}. Then, the Euler-Lagrange Equation is directly given by \eqref{N1}.
\end{proof}
