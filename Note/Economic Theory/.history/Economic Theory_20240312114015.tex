...
\begin{proof}
    ...
    \begin{enumerate}
        ...
        \item Case 1: suppose $w^*$ doesn't give full-employment in CE. Take a intervention $w_e\&w_u$. Then, $\{\theta:r(\theta)+w_u\leq w_e\}=[\underline{\theta},\hat{\theta}]$ for some $\hat{\theta}\in[\underline{\theta},\overline{\theta}]$ such that
        \begin{equation}
            \begin{aligned}
                r(\hat{\theta})+w_u=w_e
            \end{aligned}
        ...
        \begin{equation}
            ...
            \nonumber
        \end{equation}
        Let $\theta^*$ be s.t. $r(\theta^*)=w^*$. Because $w^*$ is a CE price, $\mathbb{E}[\theta|\theta\leq\theta^*]=r(\theta^*)=w^*$. So, $w_u(\theta^*)=0$ and $w_e(\theta^*)=w^*$.
    \end{enumerate}
\end{proof}
