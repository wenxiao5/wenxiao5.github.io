...
\begin{proposition}[Non-decreasing $f$ $\Rightarrow$ $\textnormal{MPS}(f)$, $\textnormal{MPS}_w(f)$, and $\textnormal{MPC}(f)$ have extreme points]
    Suppose $f\in L^1$ is non-decreasing. Then $\textnormal{MPS}(f)$, $\textnormal{MPS}_w(f)$, and $\textnormal{MPC}(f)$ are convex and compact in the norm topology $\Rightarrow$ (by \ref{KMT}) they all have non-empty set of extreme points.
\end{proposition}
\begin{note}
