...
\begin{definition}
    \normalfont
    A signaling policy $\pi$ is called a bi-pooling policy if there exists a collection of pairwise disjoint intervals $\{(y_i,y_i)\}_{i\in A}$ such that for every stateω∈(yi,yi)we have supp ( π ( ω))⊂{z
i , z i } for some pair of pointsyi≤z
i ≤ z i ≤ y i (i.e., the policy bi-
pools the interval ( y i , y i ) into the pair of points{z
i , z i } . For every ω/∈  i ∈A (yi,yi),the
policy sends the signal π ( ω ) = ω (i.e., it reveals the state). In the case wherez
i = z ifor
all i ∈ A , we refer to π as a pooling policy.
\end{definition}
