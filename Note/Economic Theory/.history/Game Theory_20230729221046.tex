...
\begin{definition}[Canonical Game]
    ...
    \begin{enumerate}
        ...
        \item $S$ holds more information than $R$: the value of some random variable $t$ with support $T$. (We say that $t$ is the \textbf{type} of $S$)
        \item Prior belief of $R$ concerning $t$ are given by a probability distribution $\rho$ over $T$ (common knowledge)
        \item $S$ sends a signal $s$ to $R$ drawn from a signal set $\mathcal{S}$.
    \end{enumerate}
\end{definition}
