...
\begin{equation}
    \begin{aligned}
        U(v_j,z_j,p_j,N)=v_j+z_j-p_j-\underbrace{\sum_{k\in N}s_j}_\textnormal{search costs based on his search history}
    \end{aligned}
    \nonumber
\end{equation}
where $(v_j,z_j)$ are the parameters of the consumer that reflects his preference, h $v_j$ is known before search, $z_j$ is learning during search, $p_j$ is the price, and $N$ is set of the products searched by the consumer.
