...
We let $\Delta \mathcal{A}$ denote the set of probability distributions on $\mathcal{A}$. For each $s\in \mathcal{S}$ and $\mathcal{T}'\subseteq \mathcal{T}$, we let $\Delta\mathcal{A}(s,T')$ be the set of mixed strategies that are the best responses by $\mathbf{R}$ to s for some probability distribution with support 
T'. 
