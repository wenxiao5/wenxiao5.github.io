...
\begin{example}[ (Spatial Discrimination)]
    Suppose the firm locates at $0$ and consumers located uniformly at $x$. There is a transportation cost $t\cdot x$ incurred by the firm to sell to consumers\\
    Without price discrimination, the optimal price is charged as
    \begin{equation}
        \begin{aligned}
            p^*(x)=\argmax_p (p-tx-c) D(p)=\frac{}{}
        \end{aligned}
        \nonumber
    \end{equation}
    where $D(p)=a-bp$.
