...
\begin{proposition}[Properties of Hicksian Demand]
    \begin{enumerate}[(i).]
        ...
        \item \textbf{Law of Compensated Demand}: Hicksian demand is downward sloping in its own price, $$\frac{\partial h_i}{\partial p_i}\leq 0$$
    \end{enumerate}
\end{proposition}

\begin{lemma}[$\sum_{j=1}^{L} \frac{\partial u}{\partial x_j} \frac{\partial h_{j}}{\partial p_{i}}=0$]
    If $u ( x )$ is strictly quasi-concave, the Hicksian demand satisfies $\sum_{j=1}^{L} \frac{\partial u}{\partial x_j} \frac{\partial h_{j}}{\partial p_{i}}=0$
\end{lemma}
\begin{proof}
    $u(h(p,u))\equiv u \Rightarrow \sum_{j=1}^{L} \frac{\partial u}{\partial x_j}\frac{\partial h_{j}}{\partial p_{i}}=0$.
\end{proof}
...
\begin{proposition}[Properties of Expenditure Function]
    \begin{enumerate}[(i).]
...
\begin{proof}[Proof for Derivative]
    \begin{enumerate}
        ...
        \frac{\partial e}{\partial p_{i}} & \equiv \sum_{j=1}^{L} p_{j} \frac{\partial h_{j}}{\partial p_{i}}+h_{i} \\
        & \equiv \sum_{j=1}^{L} \lambda^{*} u_{j}\left(x^{*}\right) \frac{\partial h_{j}}{\partial p_{i}}+h_{i} \\
        &=\lambda^{*}\left[\sum_{j=1}^{L} u_{j}\left(x^{*}\right) \frac{\partial h_{j}}{\partial p_{i}}\right]+h_{i}\\&=h_i
        \end{aligned}
        $$
        \item 
    \end{enumerate}
