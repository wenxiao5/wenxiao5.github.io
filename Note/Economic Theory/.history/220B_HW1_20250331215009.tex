...
Suppose there are two firms $1,2$ that are competing in a Bertrand competition by setting prices. Each firm $i$ faces an i.i.d. marginal cost $c_i\in\{c_L,c_H\}$, where $c_L<c_H$ and $\textnormal{Pr}(c_i=c_H)=\theta$. The demand for firm $i$ is given by:
\begin{equation}
    \begin{aligned}
    ...
    \nonumber
\end{equation}
The price that maximizes long-term profit is $p^M=\argmax_{p}\theta(p-c_H)(a-p)+(1-\theta)(p-c_L)(a-p)=\frac{a+\theta c_H+(1-\theta)c_L}{2}$.

\paragraph{Case 1: Private cost shocks}
Suppose the cost is privately observed by each firm. The firm $i$'s pricing strategy can be written as $p_i(c_i)$.
