...
\begin{definition}[Extensive Perfect Information Game]
    \normalfont
    Am extensive game with prefect information is defined as $G=\{N,A,H,Z,P,O,o,\succ_{n\in N}\}$, where $N$ is the set of players, $A$ is the set of players, $H$ is the set of all histories, $Z$ is the set of all histories that are terminals, $P:H/Z \rightarrow N$ is a mapping from a non-terminal histories to a player (who moves after a non-terminal history), and $O$ is the set of outcomes. $o$ is a function from $Z$ to $O$.
\end{definition}
...
\normalfont
A subgame of a PIG $G=\{N,A,H,Z,P,O,\succ_{n\in N}\}$ is a game (a PIG)that starts after a given finite history $h\in H$. Formally, the subgame $G(h)$ associated with $h=(h_1,...,h_n)\in H$ is $G(h)=\{N,A,H_h,Z,P,O,\succ_{n\in N}\}$G(h)=(N,A,Hh,O,oh,Ph,{⪯i}i∈N)
\end{definition}
