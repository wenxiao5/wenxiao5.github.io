...
Uncertainty of evolution process: Transportation probability is $\textnormal{Prob}\left(s_{t+1}\mid s_t,a_t\right)$. Then,
\begin{equation}
    \begin{aligned}
        V^i(s_t)=\max_{a^i_t}\Pi_t^i((a^i_t,a^{-i}_t)s_t)+\sum_{s'}\delta V^i(s')\textnormal{Prob}\left(s'_{t+1}'\mid s_t,a_t\right)
    \end{aligned}
    \nonumber
\end{equation}
