...
\end{lemma}
\begin{proof}
    Firms must make profits $\geq 0$. Suppose by the way of contradiction that the total profit $\Pi>0$. Let $(w_L,t_L)$ be the contract signed by $\theta_L$ and $(w_H,t_H)$ be the contract signed by $\theta_H$. One firm can profitably deviate by offering $(w_L+\Delta,t_L)$ and $(w_H+\Delta,t_H)$, where $\Delta \rightarrow 0^+$.
\end{proof}
...
\end{lemma}
\begin{proof}
    Suppose for a contradiction, $\exists$ an SPE where both worker types sign $(w_p=\mathbb{E}[\theta],t_p)$. Suppose one firm offers $(w_p,t_p)$, then another firm can only employ high type workers by offering $(\tilde{w},\tilde{t})$, where $\tilde{w}-e(\theta_H,\tilde{t})>\mathbb{E}[\theta]-e(\theta_H,t_p)$
\end{proof}
...
\begin{proof}
    \begin{enumerate}
        \item \underline{$w_L\geq \theta_L$:} Suppose $w_L<\theta_L$. Either firm can profitably deviate by setting $(w'_L,t_L)$ such that $w_L<w'_L<\theta_L$.
        \item \underline{$w_H\geq \theta_H$:} Since $w_L\geq \theta_L$, $w_H< \theta_H$ gives negative profit of firms, which is impossible by Lemma \ref{lemma:zero_profit}.
    \end{enumerate}
    If any inequality holds strictly, firms get negative profit. So, $w_L= \theta_L$ and $w_H=\theta_H$.
\end{proof}
