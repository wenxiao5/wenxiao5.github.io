...
\begin{enumerate}
    ...
    \subitem $C$ accepts ($R$ and $C$ get period payoffs $(1-x_k,x_k)$ and $\textnormal{Pr}(k_{t+1}=h)=q$, $\textnormal{Pr}(k_{t+1}=l)=1-q$) or rejects ($R$ and $C$ get period payoffs $(1-p_k-f,p_k+f)$ and $\textnormal{Pr}(k_{t+1}=w_C)=p_k$, $\textnormal{Pr}(k_{t+1}=w_R)=1-p_k$), where $f\in (0,1)$ and $0<p_l<p_h<1$.
    \item If enter $w_C$ ($R$ and $C$ get period payoffs $(0,1-f)$); If enter $w_R$ ($R$ and $C$ get period payoffs $(1-f,0)$).
\end{enumerate}
