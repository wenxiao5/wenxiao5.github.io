...
\begin{lemma}\label{Lemma:opt_q}
    Given $R_n=0$, in independent private value auctions, $\frac{\partial S}{\partial q}(q^*,\Omega)=0$.
\end{lemma}
\begin{proof}
    ...
    \begin{equation}
        ...
        \nonumber
    \end{equation}
    We don't prove this in detail here (Milgrom and Weber, 1982 theorem
    0). The intuition is: $\tilde{V}_n-\tilde{V}_{n-1}$ is the difference between the highest value among first $n$ values and the highest value among first $n-1$ values. There is only $\frac{1}{n}$ probability that the $n^{th}$ value is the highest value among the first $n$ values. However, $\tilde{V}_n-\tilde{W}_n$ always gives the difference between the highest value and the second-highest value among first $n$ values.
...
\begin{proof}
    ...
    Suppose the set $\{q^s,R^s\}$ maximizes $S(q,R)$. Then,
    \begin{equation}
        \begin{aligned}
            S(q^s,R^s,N,C)\geq S(q^*,R,N,c)=\Pi(q^*,R,N,c)
        \end{aligned}
        \nonumber
    \end{equation}
    for all $R$
