...
\begin{definition}[Competitive Equilibrium]
    \normalfont
    An allocation $(x^*,y^*)$ and a price vector $p^*\in \mathbb{R}^L$ are a \textbf{competitive equilibrium} in a \underline{private ownership production economy} if
    \begin{enumerate}[(i).]
        \item $x_i^*\in x_i(p^*)$ (given by \eqref{eq:x_star}) for all agent $i$.
        \item $y_j^*\in x_j(p^*)$ (given by \eqref{eq:y_star}) for all firm $j$.
