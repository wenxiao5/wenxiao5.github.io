...
\subsection{How to work for non-monotonic functions? -- Rearrangement}
\begin{note}
    \normalfont
    ...
    \begin{definition}[Rearrangement]
        \normalfont
        Given a function $f$, let $m(x)$ denote the Lebesgue measure of the set $\{s \in[0, 1]: f(s)\leq x\}$, that is $m(x)=\int_{s\in\{s \in[0, 1]: f(s)\leq x\}}1ds$ (the "length" of the set).
    \end{definition}
\end{note}
