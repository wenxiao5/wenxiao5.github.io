...
\textbf{Basic case:} Spatial Competition: There are consumers in $[0,1]$ (uniform distribution). The position chosen by both firms is $\frac{1}{2}$ (the center of the market).

\textbf{With price competition:} transportation cost is $t x^2$, where $x$ is the distance. Suppose the firm $A$ locates at $0$ and the firm $B$ locates at $1$. The profit of consumer $x$ from purchasing $A$ is $v-p_A-tx^2$ and the profit of consumer $x$ from purchasing $B$ is $v-p_B-t(1-x)^2$. The indifferent consumer is
\begin{equation}
    \begin{aligned}
...
\begin{equation}
    \begin{aligned}
        p_A^*(p_B)=\argmax_{p_A}(p_A-c)\left(\frac{1}{2}-\frac{p_A-p_B}{2t}\right)\\
        p_B^*(p_A)=\argmax_{p_B}(p_A-c)\left(\frac{1}{2}+\frac{p_A-p_B}{2t}\right)
    \end{aligned}
    \nonumber
\end{equation}
