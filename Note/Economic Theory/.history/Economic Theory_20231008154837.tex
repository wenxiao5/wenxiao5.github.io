...
\subsection{Lagrangian Approach: $\frac{\partial u\left(x^{*}\right)}{\partial x_{i}}=\lambda^{*} p_{i}$ and $\lambda^*\left(x(p,w)+p\frac{\partial x(p,w)}{\partial p}\right)=0$}
...
\begin{lemma}
    \begin{enumerate}[(i).]
        ...
        \item $\lambda^*\left(x(p,w)+p\cdot\frac{\partial x(p,w)}{\partial p}\right)=0$.
    \end{enumerate}
\end{lemma}
...
\begin{proposition}[Roy's Identity]
    $x^*_i(p,w)=-\frac{\frac{\partial v}{\partial p_{i}}}{\frac{\partial v}{\partial w}}$.
\end{proposition}
\begin{proof}
    By the definition,
    \begin{equation}
        \begin{aligned}
            v(p, w) & \equiv u(x(p, w))\\
            \frac{\partial v}{\partial p_{i}} & \equiv \sum_{j=1}^{L} \frac{\partial u}{\partial x_{j}} \frac{\partial x_{j}}{\partial p_{i}}\\
            \textnormal{By }\frac{\partial u\left(x^{*}\right)}{\partial x_{i}}=\lambda^{*} p_{i}&=\sum_{j=1}^{L} \lambda^{*} p_{j} \frac{\partial x_{j}}{\partial p_{i}}\\
            \textnormal{By }\lambda^*\left(x(p,w)+p\cdot\frac{\partial x(p,w)}{\partial p}\right)=0&= \lambda^{*}\left(\sum_{j=1}^{L} p_{j} \frac{\partial x_{j}}{\partial p_{i}}\right) \\&=-\lambda^{*} x_{i}^{*}
        \end{aligned}
        \nonumber
    \end{equation}
\end{proof}
