...
\begin{proof}
    ...
    \begin{equation}
        \begin{aligned}
            \textnormal{Pr}[V_{(n)}\geq R_n]\mathbb{E}[V_{(n)}\mid V_{(n)}\geq R_n]&=\textnormal{Pr}[V_{(n)}\geq R_n]
...
\end{proposition}
\begin{proof}
    Given $N>n^*$, the symmetric equilibrium gives $q^s=q^s(N)<1$. Suppose $N$ drops by $1$ to $N-1$ while each remaining member continuous to use $q^S(N)$.
\end{proof}
