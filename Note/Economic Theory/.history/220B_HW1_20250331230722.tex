...
If without cost shocks $c_L=c_H$, we have $\Pi^M(c_L)=\Pi^M(c_H)=\frac{\Pi^D(c_L)}{2}=W$ and $\Pi^P=0$. Thus, the LHS of the inequality decreases and the RHS increases, so the inequality is more likely to hold, so the collusion is more likely to sustain without cost shocks.
...
\begin{equation}
    \begin{aligned}
        \Pi^D_T(c_L)+\frac{\delta}{1-\delta}\Pi^P\leq \Pi^M_T(c_L,c_L)+\frac{\delta}{1-\delta}W_T\\
        \delta\geq\frac{\Pi^D_T(c_L)-\Pi^M_T(c_L,c_L)}{\Pi^D_T(c_L)+W_T-\Pi^M_T(c_L,c_L)-\Pi^P}
    \end{aligned}
    \nonumber
\end{equation}
where $\Pi^M_T(c_L,c_L)=\frac{(a-c_L)^2}{8}$. Then, $W_T-\Pi^M_T(c_L,c_L)=-\frac{(a-c_L)^2-(a-c_H)^2}{32}=\frac{(c_L-c_H)(2a-c_H-c_L)}{32}$. In the previous case, $W-\Pi^M(c_L)=\frac{\Pi^M(c_H)-\Pi^M(c_L)}{2}=\frac{(c_L-c_H)(a-p^M)}{4}$.

Therefore, the numerator of the right-hand side increases and the denominator (weakly) decreases, so the collusion is more likely to sustain with transparency.
Therefore, the numerator of the right-hand side increases and the denominator (weakly) decreases, so the collusion is more likely to sustain with transparency.
