...
\begin{theorem}[Reconstruct Assets from Arbitrage-free Prices]
    Consider an asset structure with return matrix $R$. If $q\in \mathbb{R}^K$ is arbitrage-free, then there exists $\mu\in \mathbb{R}_{++}^S$ such that $q\in \mu^T R$, that is, s.t. $q_k=\sum_s \mu_s r_{ks}, \forall k$.
\end{theorem}
