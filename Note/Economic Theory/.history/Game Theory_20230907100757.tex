...
\begin{definition}
    \normalfont
    $\Gamma$ \textbf{implements} a social choice function $f$ if $\exists \left(S_1^*(\cdot),...,S_I^*(\cdot)\right)$ of a game induced by $\Gamma$ such that $g(S_1^*(\theta_q),...,S_I^*(\theta_I))=f(\theta_1,...,\theta_I)$ for all $(\theta_1,...,\theta_I)\in \Theta$
\end{definition}
