...
\begin{equation}
    \begin{aligned}
        ...
        \nonumber
\end{equation}
where $\Pi^P>0$ and $\Pi^M(c_H)<\Pi^M(c_L)<\frac{(a-c)^2}{8}$.

If without cost shocks $c_L=c_H$, we have $\Pi^M(c_L)=\Pi^M(c_H)=\frac{(a-c)^2}{8}$ and $\Pi^P=0$. Thus, the denominator of the right-hand side increases and the numerator decreases, so the collusion is more likely to sustain without cost shocks.
