...
\begin{note}
    Different to the original paper, we focus on the ``free entry'' case (i.e., the mechanism can be denoted by the reserve prices $R=\{R_1,...,R_N\}$).
\end{note}
We let $T$ represent the event that trade occurs and let $T_n(R_n)$ represent the probability of trade given $n$ and the seller's mechanism.
