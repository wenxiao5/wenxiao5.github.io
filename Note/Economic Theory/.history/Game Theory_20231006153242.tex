...
\begin{proposition}[Main General Results]
    \begin{enumerate}[$\circ$]
        ...
        \item If $\succeq$ is differentiable, $x^*\in B(p)$ is a $\succeq$-best bundle, and the vector of marginal values at $x^*$ (as defined above) is denoted by $v(x^*)=(v_1(x^*),...,v_K(x^*))$. Then
        \begin{equation}
            \begin{aligned}
            ...
            \nonumber
        \end{equation}
        where $v_k(x^*)$ is usually taken by $\frac{\partial u}{\partial x_k}(x^*)$ in "classic" problem.
    \end{enumerate}
\end{proposition}
