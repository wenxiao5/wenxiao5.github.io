...
\begin{theorem}[Second-order Stochastic Dominance Equivalence]
...
\end{enumerate}
Let $X_F$ and $X_G$ be now random variables with distributions $F$ and $G$, respectively, defined on the interval $[0,1]$.
\begin{theorem}[Shaked \& Shanthikumar (2007), Section 3.A]
    $$G\prec F \Leftrightarrow F^{-1}\prec G^{-1} \Leftrightarrow X_{G}$$
\end{theorem}
