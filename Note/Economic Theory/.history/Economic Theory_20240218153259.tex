...
\begin{definition}[Saddle Point]
    \normalfont
    Let $X,Y$ be sets and $f:X\times Y \rightarrow \mathbb{R}$ a real function. $(x^*,y^*)\in X\times Y$ is a \textbf{saddle point} of $f$ if $x^*\in\argmax_{x\in X}f(x,y^*)$ and $y^*\in\argmin_{y\in Y}f(x^*,y)$.
\end{definition}
...
\begin{enumerate}
    \item Let $\Omega$ be a (finite) set of possible states of the world.
    \item Subsets $E\subseteq\Omega$ are \textbf{events}.
    \item \textbf{Knowledge} is modeled through a function $$K:2^\Omega \rightarrow 2^\Omega$$
    Interpretation: $K(E)$ (which we write as $KE$) is the set of states at
    which the agent knows that the event $E$ has occurred.
    \item Define the function $P : \Omega \rightarrow 2^\Omega$ by, $$P(\omega) = \cap\{E \subseteq \Omega : KE \ni \omega\}$$
    an \textbf{information function}.\\
    Means that, if $\omega' \in P(\omega)$, then when the state is $\omega$, information doesn't allow one to distinguish between $\omega$ and $\omega'$.
    Then, the knowledge satisfies $$KE=\{\omega\in\Omega:P(\omega)\subseteq E\}$$
\end{enumerate}
...
\subsection{S5 and Partitional Information Function}
\begin{definition}[S5]
    ...
    \begin{enumerate}
        ...
        \item Axiom of knowledge. $K A \subseteq A$. If Alice knows $A$, then $A$ has indeed occurred.
        \item Axiom of positive introspection. $K K A=K A$. If Alice knows $A$ then she knows that she knows $A$.
        \item Axiom of negative introspection. $(K A)^c=K\left((K A)^c\right)$. If Alice doesn't know $A$ then she knows that she doesn't know $A$.
    \end{enumerate}
...
\begin{theorem}[S5 $\Leftrightarrow$ Partitional]
    $K$ satisfies S5 iff it is partitional
\end{theorem}
...
\subsection{Self-evident}
\begin{definition}[Self-evident]
    \normalfont
    An event $A \in 2^\Omega$ is \textbf{self-evident} if $KA = A$.
\end{definition}

\begin{proposition}[Self-evident $\Leftrightarrow$ Unions of Elements of Partition]
    $A$ is self-evident iff it is the union (1 or more) of elements of the partition $P$.
\end{proposition}
\begin{proof}
...
\begin{corollary}
    Let $\Sigma$ be the collection of self-evident events. Then $\Sigma$ is an algebra (in fact, $\Sigma = \Sigma_P$, the algebra generated by the partition $P$).
\end{corollary}

\begin{corollary}
    For any $A, KA \in \Sigma$. In fact, $$KA = \cup\{S \in \Sigma : S \subseteq A\}$$
\end{corollary}
