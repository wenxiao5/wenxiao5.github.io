...
Consider a symmetric IPV with regular $F$. Define a bidder as \textit{serious} whenever $a\geq v_0$.
\begin{theorem}[Bulow \& Klemperer, AER, 1996]
    As long as bidders are serious, the optimal auction with $N$ bidders has smaller revenue than a second price auction without reserve and $N+1$ bidders.
\end{theorem}
In the paper, they prove this for asymmetric distributions and possibly interdependent values, as long as the signals the buyer's receive about their possibly common value are independent.
