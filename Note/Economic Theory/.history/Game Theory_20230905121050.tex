...
\begin{definition}[Choice Correspondence (More than one choice)]
    ...
    $$\emptyset\neq C(A)\subseteq A$$
\end{definition}
\textbf{Properties:} ($\alpha$): If $a\in A\subseteq B$, then $a\in C(B) \Rightarrow a\in c(A)$; ($\beta$): If $a,b\in A\subseteq B$, then $a,b\in C(A)$ and $b\in c(B) \Rightarrow a\in C(B)$.
