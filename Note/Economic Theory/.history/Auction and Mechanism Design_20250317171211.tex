...
\begin{definition}[Harsanyi's three axioms]
    ...
    \begin{enumerate}
        \item \textbf{EU preferences}: each agent's preference $\succcurlyeq_i$ (or $\theta_i$ equivalently) can be represented by the von Neumann-Morgenstern utility function $u_i(\cdot;\theta_i)$ s.t. $$x\succcurlyeq_i y\Leftrightarrow\mathbb{E}_xu_i(x;\theta_i)\geq \mathbb{E}_yu_i(y;\theta_i).$$ Let $\mathcal{R}$ be the set of such preferences.
        \item \textbf{Pareto indifference}: If $x\sim_i y$ for all $i\in I$, then $x\sim y$.
        \item \textbf{Social EU}: For each $(\succcurlyeq_1,\ldots,\succcurlyeq_I)\in \mathcal{R}^I$, the \textit{social welfare function} satisfies the vNM EU axioms: there exists $u(\cdot;\theta)$ such that $$a f(\theta) b \Leftrightarrow \mathbb{E}_au(a;\theta)\geq \mathbb{E}_bu(b;\theta).$$
    \end{enumerate}
\end{definition}
...
Remind the Arrow's axioms in this context are given by the following:
...
When $\lambda(\theta)$ is constant and non-degenerate then we get a social choice function $f$ defined by
\begin{equation}
    \begin{aligned}
        a f(\theta) b \Leftrightarrow \sum_{n=1}^N \lambda_n \mathbb{E}_au(a;\theta)\geq \sum_{n=1}^N \lambda_n \mathbb{E}_bu(b;\theta).
    \end{aligned}
    \nonumber
\end{equation}
This is often referred to as \textbf{utilitarian social choice}, which is widely used in practice.
\begin{enumerate}
    \item It satisfies Pareto
    \item not dictatorship
    \item domain is limited to EU preferences
