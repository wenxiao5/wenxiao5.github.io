...
Suppose there are two firms $1,2$ that are competing in a Bertrand competition by setting prices. Each firm $i$ faces an i.i.d. marginal cost $c_i\in \textnormal{Unif}[\underline{\theta},\overline{\theta}]$. The demand for firm $i$ is given by:
\begin{equation}
    \begin{aligned}
        D_i(p_i,p_j)=\left\{\begin{matrix}
            D(p_i),& p_i<p_j,\\
            \frac{D(p_i)}{2},& p_i=p_j,\\
            0,& p_i>p_j
        \end{matrix}\right.
    \end{aligned}
    \nonumber
...
Suppose the cost is privately observed by each firm. Suppose in collusion, firms set price $p^M$, which generates a monopoly profit $\Pi^M(c)=(p^M-c)D(p^M,p^M)$.

The price that maximizes long-term profit is $p^M=\argmax_{p}\int(p-c)(a-p)+(1-\theta)(p-c_L)(a-p)=\frac{a+\theta c_H+(1-\theta)c_L}{2}$. Without collusion, firm $i$ with $c_i=c_L$ sets price $\argmax_{p}(p-c_L)()$

The firm $i$'s pricing strategy can be written as $p_i(c_i)$.
