...
\textbf{Sufficient Conditions for Highest Profit Equilibrium:}
\begin{enumerate}
    \item $g(a) < 0$, requiring:
    \[N < \frac{1}{1-\delta}\]
    
    \item $g(b) > 0$, requiring:
    \[\frac{\delta}{1-\delta}\frac{1}{N-1} < \frac{\Pi^M(b)}{\int_a^b \Pi^M(\epsilon)F(d\epsilon)}\]
\end{enumerate}

Under these conditions, the equilibrium exhibits countercyclical pricing behavior:
\begin{itemize}
    \item Monopoly pricing during economic downturns (low $\epsilon$)
    \item Prices below monopoly levels during economic booms (high $\epsilon$)
\end{itemize}
