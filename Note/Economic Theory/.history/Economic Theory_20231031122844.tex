...
\begin{proof}[Second Welfare Theorem \ref{SWT_sup}]
    ...
    \begin{equation}
        \begin{aligned}
            P_i:=\left\{x_i\in \mathbb{R}_+^L:u_i(x_i)\geq u_i(x_i^*)\right\}, \ \forall i
        \end{aligned}
        \nonumber
    \end{equation}
    By strong monotonicity, $P_i\neq \emptyset$ (interior allocation), and by convexity, $P_i$ is convex, $\forall i$.

    Let
    \begin{equation}
        \begin{aligned}
            P:&=P_1+\cdots+P_m\\
            &=\left\{z\in \mathbb{R}_L^+: z=\sum_{i=1}^m x_i \textnormal{ for some }(x_1,...,x_m)\in \mathbb{R}_+^{L\times m} \textnormal{ s.t. }u_i(x_i)\geq u_i(x_i^*), \forall i\right\}
        \end{aligned}
        \nonumber
    \end{equation}
\end{proof}
