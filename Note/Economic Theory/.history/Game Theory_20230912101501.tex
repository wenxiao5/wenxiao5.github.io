...
\begin{proposition}[Convex $\succeq$ $\Rightarrow$ quasi-concave $u(\cdot)$]
    If rational $\succeq$ is convex, then $\exists$ a \underline{quasi-concave} $u(\cdot)$ that represents $\succeq$.
\end{proposition}
...
\begin{definition}[$D$-Optimal Matching]
    \normalfont
    A matching $\mu: D\cup H \rightarrow D\cup H$ is \textbf{$D$-optimal}, denoted by $\mu^D$, if for any $\mu'$ we have that $\mu^D\succeq_D \mu'$ (good for all doctors).
\end{definition}

\begin{theorem}
    DA terminates in $\mu^D$.
\end{theorem}
