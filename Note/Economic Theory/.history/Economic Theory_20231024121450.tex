...
\begin{definition}[Competitive Equilibrium]
    \normalfont
    Given endowment $\{w^i\}_{i\in N}$. A \textbf{competitive (Walrasian) equilibrium} in an exchange economy is a pair $p^*\in \mathbb{R}^L$ (price vector over $L$ goods) and an allocation $(x^i)_{i\in N}$ such that:
    \begin{enumerate}[(i).]
        \item $x^i\in \argmax u^i(x)$ s.t. $p\cdot x^i\leq p\cdot w^i, \forall i\in N$.
        \item $\sum_{i\in N}x^i_\rho (p,w)=\sum_{i\in N}w^i_\rho, \forall \rho\in L$.
    \end{enumerate}
\end{definition}
