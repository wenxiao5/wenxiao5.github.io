...
\begin{definition}[Competitive Equilibrium]
    \normalfont
    Given endowment $w=(w_i)_{i\in N}$. A \textbf{competitive (Walrasian) equilibrium} in an exchange economy is a pair $p^*\in \mathbb{R}^L$ (price vector over $L$ goods) and an allocation $(x_i)_{i\in N}$ such that:
    \begin{enumerate}[(i).]
        \item $x_i^*\in \argmax u^i(x)$ s.t. $p\cdot x^i\leq p\cdot w^i, \forall i\in N$.
        \item $\sum_{i\in N}x^i^*=w$.
    \end{enumerate}
\end{definition}
