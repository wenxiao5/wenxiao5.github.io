...
\begin{proposition}[Continuous Preference$\Rightarrow$ Marshallian Demand]
    If $\succeq$ ($u(\cdot)$) is continuous, then all such problems have a solution.
\end{proposition}
The solution is called Marshallian (Uncompensated) Demand.

\begin{proposition}[Convex Preference$\Rightarrow$ Convex Demand]
    If $\succeq$ is convex ($u(\cdot)$ is quasi-concave), then the set $\succeq$-optimal bundle $x\in B(p)$ is convex.
\end{proposition}

\begin{proposition}[Main General Results]
    \begin{enumerate}[$\circ$]
        \item If $\succeq$ is continuous, then all such problems have a solution.
        \item If $\succeq$ is convex, then the set $\succeq$-optimal bundle $x\in B(p)$ is convex.
        \item If $\succeq$ is strictly convex, then the set $\succeq$-optimal bundle $x\in B(p)$ is (at most) a singleton.
        \item If $\succeq$ is differentiable, $x^*\in B(p)$ is a $\succeq$-optimal bundle, and the vector of marginal values at $x^*$ (as defined above) is denoted by $v(x^*)=(v_1(x^*),...,v_K(x^*))$, where $v_k(x^*)$ is usually taken by $\frac{\partial u}{\partial x_k}(x^*)$ in "classic" problem. Then, we have
        \begin{equation}
            \begin{aligned}
...
\begin{corollary}[Sufficient Conditions for Optimality]
    If $\succeq$ is strongly monotonic, convex, continuous, and differentiable and if $p\cdot x^*=w$ and \eqref{*} is satisfied then $x^*$ is a $\succeq$-optimal bundle.
\end{corollary}
