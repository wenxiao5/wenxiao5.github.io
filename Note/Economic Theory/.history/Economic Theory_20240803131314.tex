...
\begin{enumerate}
    ...
    \item A \textit{partition} $(S_k)_{k=1,...,\bar{k}}$ of $N$ (i.e., $S_j\cap S_k=\emptyset,\forall j\neq k$ and $\cup_{k=1}^{\bar{k}}S_k=N$) and an \textit{allocation} $(x_i)_{i\in N}$ specifying the division of the worth each $S_k$ among its members (i.e., $\sum_{i\in S_k}x_i=v(S_k),\forall k\in\{1,...,\bar{k}\}$.)
\end{enumerate}
