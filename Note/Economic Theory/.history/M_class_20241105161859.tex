...
\begin{enumerate}
    ...
    \item Uncertainty on profit function $\Pi^i(a_t,s_t,\epsilon_t)$,
    \begin{equation}
        \begin{aligned}
            V^i(s_t,\epsilon_t)=\max_{a^i_t}\Pi_t^i((a^i_t,a^{-i}_t),s_t)+\sum_{s'}\delta V^i(s',\epsilon_{t+1})\textnormal{Prob}\left(s',\epsilon_{t+1}\mid s_t,a_t,\epsilon_t\right)
        \end{aligned}
        \nonumber
    \end{equation}
\end{enumerate}
