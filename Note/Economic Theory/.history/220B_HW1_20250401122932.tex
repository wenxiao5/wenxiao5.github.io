...
\begin{equation}
    \begin{aligned}
        \Pi^D(c)+\frac{\delta}{1-\delta}\Pi^P\leq \Pi^M(c)+\frac{\delta}{1-\delta}W,\forall c\in[0,1]\\
...
\begin{equation}
    ...
    \nonumber
\end{equation}
where $\max_{(c_i,c_{-i})}\left(\Pi^D_T(c_i,c_{-i})-\Pi^M_T(c_i,c_{-i})\right)=\Pi^D_T(0,0)-\Pi^M_T(0,0)=\frac{p_m}{2}$.
