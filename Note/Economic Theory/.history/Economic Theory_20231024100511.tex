...
\begin{definition}
    \normalfont
    A setting with $\tilde{q}_s$ has \textbf{stronger quota} than setting $q_s$ if $\tilde{q}_s=q_s$ but $q_s^M\geq \tilde{q}_s^M$.
\end{definition}

Mechanism $\Phi$ is \textbf{good}, if whenever a setting with $\tilde{q}_s$ has stronger quotas than setting $q_s$, then it is not the case that all \underline{minority} students are worse off under the $\tilde{setting}$.
