...
\begin{definition}[Properties of a Social Choice Function]\label{SWF_properties}
    ...
    \begin{enumerate}[$\circ$]
        ...
        \item \textbf{Independence of Irrelevant Alternatives (IIA)}: For any $\theta, \theta' \in \Theta$, if $u_i(x, \theta) > u_i(y, \theta) \Leftrightarrow u_i(x, \theta') > u_i(y, \theta')$ for all $i$, then $x = f(\theta) \Rightarrow y \neq f(\theta')$.
    \end{enumerate}
\end{definition}
