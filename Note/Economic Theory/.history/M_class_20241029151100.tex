...
Another equilibrium can be achieved by the strategy: Collude at monopoly price $p^M:=\argmax_{p}(p-c)D(p)$ (denote the profit in each period as $\Pi^M:=\frac{\max_{p}(p-c)D(p)}{2}$) and move to $p=c$ if the competitor deviates. The existence of this equilibrium requires
\begin{equation}
    \begin{aligned}
        \sum_{t=1}^\infty\delta^{t-1}\Pi^M\geq 2\Pi^M
    \end{aligned}
    \nonumber
\end{equation}
