...
\begin{proposition}[Strictly Convex Preference$\Rightarrow$ Singleton $x(p,w)$]
    If $\succeq$ is strictly convex, then $x(p,w)$ is (at most) a singleton.
\end{proposition}

\begin{proposition}[Main General Results]
    If $\succeq$ is differentiable, $x^*\in B(p)$ is a $\succeq$-optimal bundle, and the vector of marginal values at $x^*$ (as defined above) is denoted by $v(x^*)=(v_1(x^*),...,v_K(x^*))$, where $v_k(x^*)$ is usually taken by $\frac{\partial u}{\partial x_k}(x^*)$ in "classic" problem. Then, we have
    \begin{equation}
        \begin{aligned}
