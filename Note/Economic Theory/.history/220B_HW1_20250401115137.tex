...
\end{enumerate}
In collusion with private cost shocks, the detectable collusive price cannot depend on the cost. The collusive price should be $p^M=p_m$ that generates a profit $\Pi^M(c)=\frac{p_m-c}{2}$ for a firm with cost $c$. The expected profit from sustaining collusion is $W=\int_0^1 \Pi^M(c)dc=\frac{p_m-\frac{1}{2}}{2}$.
