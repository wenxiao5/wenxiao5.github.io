...
If without cost shocks that $c\equiv \frac{1}{2}$, we have $\Pi^D(c)-\Pi^M(c)=\frac{p_m-\frac{1}{2}}{2}=W$ and $\Pi^P=0<\frac{1}{6}$. Thus, the collusion sustains with probability $1$ if $\frac{\delta}{1-\delta}\geq 1$ and $0$ if $\frac{\delta}{1-\delta}<1$. Therefore, the collusion is more likely to sustain without cost shocks if and only if $\frac{\delta}{1-\delta}\geq 1$. That is, \textbf{the collusion is more likely to sustain without cost shocks if firms are very patient and the collusion is more likely to break without cost shocks if firms are not very patient}.
...
\end{enumerate}
In this case, the probability of the collusion can sustain is:
\begin{equation}
    \begin{aligned}
...
Consider both $\textnormal{Pr}[\Pi^D(c)+\frac{\delta}{1-\delta}\Pi^P\leq \Pi^M(c)+\frac{\delta}{1-\delta}W]$ in \eqref{eq_p} and $\textnormal{Pr}[\Pi^D_T(c_i,c_{-i})+\frac{\delta}{1-\delta}\Pi^P_T\leq \Pi^M_T(c_i,c_{-i})+\frac{\delta}{1-\delta}W_T]$ in \eqref{eq_t} as functions of $\frac{\delta}{1-\delta}$. They are both increasing functions of $\frac{\delta}{1-\delta}$. As $p_m>1$, we can find that
\begin{enumerate}
    \item The zero point of \eqref{eq_p} $\frac{p_m-1}{\frac{p_m}{2}-\frac{5}{6}}$ gives a value $\frac{1}{2}$ for \eqref{eq_t}.
    \item The $\frac{p_m}{p_m-\frac{5}{6}}$ gives value $1$ for \eqref{eq_p} while gives a value in $(\frac{1}{2},1)$ for \eqref{eq_t}.
    \item The increasing rate of \eqref{eq_t} is smaller than \eqref{eq_p}, $\left(\frac{p_m}{2}-\frac{1}{3}\right)\left(p_m-\frac{\delta}{1-\delta}\left(\frac{p_m}{2}-\frac{1}{3}\right)\right)<p_m-\frac{5}{6}$.
\end{enumerate}
By the continuity of the two functions, we can find that there exists a unique critical value $d^*$ such that for all $\frac{\delta}{1-\delta}<d^*$, the value of \eqref{eq_t} is higher than \eqref{eq_p}, and for all $\frac{\delta}{1-\delta}>d^*$, the value of \eqref{eq_t} is lower than \eqref{eq_p}.
That is, \textbf{the collusion is more likely to sustain with transparency when firms are not very patient and the collusion is less likely to sustain with transparency when firms are very patient}.
