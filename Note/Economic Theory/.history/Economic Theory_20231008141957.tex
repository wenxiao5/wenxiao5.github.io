...
\begin{equation}
    \begin{aligned}
        ...
        \lambda^*\geq 0 \textnormal{ ($\lambda^*=0$ must hold if $p\cdot x^*<w$)}
    \end{aligned}
    \nonumber
\end{equation}
...
\begin{theorem}[Envelope Theorem]
    ...
    \begin{enumerate}[$\bullet$]
        \item The Lagrangian for this problem is $$L(x,\lambda)=f(x; \theta)-\lambda g(x; \theta)$$
        \item Let $x^{*}$ and $\lambda^{*}$ denote the optimized values of the variables. (By KKT necessary conditions, we have $\frac{\partial f}{\partial x}(x^*;\theta)=\lambda^* \frac{\partial g}{\partial x}(x^*;\theta)$)
    \end{enumerate}
    Then the following is true for any $\bar{\theta}\in \mathbb{R}^m$
    \begin{equation}
        \begin{aligned}
            \frac{\partial f}{\partial \theta_i}(x^*(\bar{\theta});\theta(\bar{\theta}))=\left.\frac{\partial L}{\partial \theta_i}\right|_{x^{*}, \lambda^{*}}=\left.\frac{\partial f}{\partial \theta_i}\right|_{x^{*}}-\lambda^*\left.\frac{\partial g}{\partial \theta_i}\right|_{x^{*}, \lambda^{*}}
        \end{aligned}
        \nonumber
    \end{equation}
\end{theorem}
