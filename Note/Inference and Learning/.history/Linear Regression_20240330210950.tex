...
\begin{equation}
    \begin{aligned}
        \underbrace{f(Y,X)}_\textnormal{joint distribution of $Y$ and $X$}:=\underbrace{\textnormal{Pr}(Y=1|X)}_{=F(X^T\beta_0)}\cdot \underbrace{f_X(X)}_\textnormal{marginal density of $X$ (no parameters of interest)}
    \end{aligned}
    \nonumber
\end{equation}
Then,
