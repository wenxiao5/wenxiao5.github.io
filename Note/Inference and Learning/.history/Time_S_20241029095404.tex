...
\begin{definition}[Stability of $VAR(1)$]
    The $VAR(1)$ model is \textbf{stable} iff one of the above conditions is satisfied, e.g., $|I_n-\Phi z|=0 \Rightarrow |z|>1$.
\end{definition}
...
\begin{example}
\begin{enumerate}
    \item \underline{$VMA(\infty)$ Case:} Suppose $Y_t=\mu+\sum_{i=0}^\infty\psi_i\epsilon_{t-i}$ for all $t$, where $\epsilon_t\sim \textnormal{WN}(0,\Sigma)$ and $\sum_{i=0}^\infty\|\psi_i\|^2<\infty$.
    \item 
\end{enumerate}
\end{example}
