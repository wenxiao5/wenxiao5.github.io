...
\begin{equation}
    \begin{aligned}
        \hat{R}_\textnormal{SURE}(\mathcal{Z})=K\sigma^2+2\sigma^2\sum_{k=1}^K \frac{\partial g_k(\mathcal{Z})}{\partial \mathcal{Z}_k}+\underbrace{\sum_{k=1}^K\left(\hat{\theta}_k- \mathcal{Z}_k\right)^2}_{\|g(\hat{\theta})\|^2}
    \end{aligned}
    \nonumber
\end{equation}
which has expectation
