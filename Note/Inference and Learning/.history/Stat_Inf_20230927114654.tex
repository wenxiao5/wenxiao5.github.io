...
\begin{proposition}[Sufficient and Complete Statistic for Exponential Family]
...
\begin{definition}["Regularity" Condition]
    \normalfont
    "Regularity" condition: If $X$ are continuous, then
    \begin{equation}
        \begin{aligned}
            \mathbb{E}_\theta\left[\frac{\partial}{\partial \theta}\log f(x|\theta)\right]&=\int_{-\infty}^\infty \left[\frac{\partial}{\partial \theta}\log f(x|\theta)\right]f(x|\theta) dx\\
            &=\int_{-\infty}^\infty \left[\frac{\partial}{\partial \theta}f(x|\theta)\right]dx\\
            &=\frac{\partial}{\partial \theta}\underbrace{\int_{-\infty}^\infty f(x|\theta)dx}_{=1}\\
            &=0
        \end{aligned}
        \nonumber
    \end{equation}
...
\begin{enumerate}[$\bullet$]
    ...
    \begin{enumerate}
        ...
        \item[] A \underline{testing procedure} is a (data-based) rule for choosing between $H_0$ and $H_1$.
        \item[] The rule: $$\textnormal{"Reject $H_0$ iff $(X_1,...,X_n)\in C$" (for some $C\in \mathbb{R}^n$)}$$ is a testing procedure with \underline{critical region} $C$.
    \end{enumerate}
    \begin{example}
...
\begin{remark}
    ...
    \begin{enumerate}
        \item Any set $C\in \mathbb{R}^n$ can be written as $C=\{(X_1,...,X_n): T(X_1,...,X_n)>c\}$ for some $T: \mathbb{R}^n \rightarrow \mathbb{R}$ and some $c\in \mathbb{R}$.
    \end{enumerate}
\end{remark}
