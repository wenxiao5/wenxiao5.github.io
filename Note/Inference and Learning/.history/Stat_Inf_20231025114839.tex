...
Given $\vec{X}=\{X_1,...,X_N\}$, we derive a decision rule $d(\vec{Y})$ given $\vec{Y}$

Define $\vec{m}\triangleq \left[m(X_1),...,m(X_N)\right]'$, its estimation is denoted by $\hat{\vec{m}}$.

\begin{definition}[Sum of Squared Residual]
    \normalfont
    Sum of Squared Residual (SSR) of an estimation $\hat{\vec{m}}$ is given by
    \begin{equation}
        \begin{aligned}
            \mathbb{E}\left[\|Y-\hat{\vec{m}}\|^2\right]&=\mathbb{E}\left[\|Y-m\|^2\right]+\mathbb{E}\left[\|\hat{\vec{m}}-\vec{m}\|^2\right]-2 \mathbb{E}\left[(Y-\vec{m})'(\hat{\vec{m}}-\vec{m})\right]\\
            &=N\sigma^2+\sum_{i=1}^N\left(\hat{m}(X_i)-m(X_i)\right)^2-2\sigma^2 df(\hat{\vec{m}})
        \end{aligned}
        \nonumber
    \end{equation}
    where the norm is $\|\vec{X}\|=\left[\sum_{i=1}^N X_i^2\right]^{1/2}$,\\
    and $df(\hat{\vec{m}})=\frac{\sum_{i=1}^N \textnormal{Cov}(Y_i,\hat{m_i})}{\sigma^2}$.\\
    The second equality is because
    \begin{equation}
        \begin{aligned}
            \mathbb{E}\left[(Y-\vec{m})'(\hat{\vec{m}}-\vec{m})\right]&=\sum_{i=1}^N \mathbb{E}\left[(Y_i-m_i)(\hat{m_i}-m_i)\right]\\
        ...
        \nonumber
    \end{equation}
    $\mathbb{E}\left[(Y-\vec{m})'\hat{\vec{m}}\right]=\mathbb{E}\left[(Y-\vec{m})'(\hat{\vec{m}}-\mathbb{E}\hat{\vec{m}})\right]=\textnormal{Cov}(Y,\hat{\vec{m}})$.
\end{definition}
