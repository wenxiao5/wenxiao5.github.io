...
\begin{example}[ (Logistic Regression)]
    ...
    which implies that $g(Y\mid X;\beta)=\left(Y-F(X^T\beta)\right)X$. In this case, $w(X_i,\beta)=X_i$ doesn't depend on $\beta$. Therefore, if $F$ is logit, $\sum_{i=1}^n X_i\cdot (Y_i-F(X_i^T\hat{\beta}_\textnormal{MLE}))=0^d$.
\end{example}
...
By the Corollary \ref{cor:asymptotic_normality_of_m_estimators}, $$\sqrt{n}\left(\hat{\beta}_\textnormal{MLE}-\theta_0\right) \stackrel{d}{\Rightarrow} N\left(0,I(\theta_0)^{-1}\right)$$
where
\begin{equation}
    \begin{aligned}
        I(\theta_0)&=G(\theta_0)=\textnormal{Var}\left(g(Y\mid X;\theta_0)\right)\\
        &=\frac{\textnormal{Var}\left(Y\mid X;\theta_0\right)}{F(X^T\beta)^2(1-F(X^T\beta))^2}\cdot\left(F'(X^T\beta)\cdot X\right)\cdot\left(F'(X^T\beta)\cdot X\right)^T
    \end{aligned}
    \nonumber
\end{equation}
Since $Y|X\sim \textnormal{Bernoulli}(F(X^T\beta_0))$, $\textnormal{Var}(Y|X)=F(X^T\beta_0)\cdot(1-F(X^T\beta_0))$.
...
\subsection{Asymptotic Normality of $\hat{\beta}_\textnormal{MLE}$}
