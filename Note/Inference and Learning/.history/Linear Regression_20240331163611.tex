...
Let the weight of observation $i$ be $w(X_i,\beta):=\frac{F'(X_i^T\beta)}{F(X_i^T\beta)(1-F(X_i^T\beta))}\cdot X_i$. Then, $\sum_{i=1}^n w(X_i,\hat{\beta}_{BR})\cdot (Y_i-F(X_i^T\hat{\beta}_{BR}))=0^d$.
