...
\end{lemma}
\begin{proof}
    It can proven by $P(|\bar{Y}-\mu|>\eta)\stackrel{T \rightarrow \infty}{\longrightarrow} 0$ for all $\eta>0$. This can be given by \underline{Chebyshev's inequality}: $P(|\bar{Y}-\mu|>\eta)\leq \frac{\textnormal{Var}(\bar{Y})}{\eta^2}$ for all $\eta>0$.
\end{proof}
\begin{enumerate}
