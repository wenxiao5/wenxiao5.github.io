...
\begin{enumerate}[$\bullet$]
    \item \underline{Input:} Dataset of $m$ objects $X=\{\vec{x}_1,\vec{x}_2,...,\vec{x}_m\}$, where each object $\vec{x}_i=(x_{i1},x_{i2},...x_{in})$ has $n$ numerical attributes.
    \item \underline{How to calculate:}
    \begin{enumerate}[$(1)$]
    ...
    \end{enumerate}
    \item \underline{How to interpret:}\\The dataset is \underline{clusterable} if the Hopkins Statistic \underline{close to $0$} and is \underline{not clusterable} if the Hopkins Statistic \underline{close to $0.5$}.
    \item \underline{Intuition:}
    \begin{center}\begin{figure}[htbp]
        \centering
        \includegraphics[scale=0.5]{HS.png}
