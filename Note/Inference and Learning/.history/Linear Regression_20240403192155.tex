...
\begin{corollary}[Second Information Equality]\label{cor:asymptotic_normality_of_m_estimators}
    Since $\mathcal{I}(\theta_0)=-G(\theta_0)$, $$\sqrt{n}\left(\hat{\theta}-\theta_0\right) \stackrel{d}{\Rightarrow} N\left(0,\mathcal{I}(\theta_0)^{-1}\right)$$
...
Consider the hypothesis $H_0:\beta=1$ vs. $H_1:\beta\neq 1$ ($\beta>0$). The wald test statistic is $$n\left(\hat{\theta}_\textnormal{MLE}-\theta_0\right)^T \hat{\mathcal{I}} \left(\hat{\theta}-\theta_0\right)$$
An equivalent form, $H_0:\beta^2=1$ vs. $H_1:\beta^2\neq 1$ ($\beta>0$).\\
Using delta method: By first order continuously differentiable,
\begin{equation}
    \begin{aligned}
...
\begin{equation}
    \begin{aligned}
        \sqrt{n}\left(\tau(\hat{\beta})-\tau(1)\right)\sim N(0,\tau'(1)\hat{\mathcal{I}}^{-1}\tau'(1))
    \end{aligned}
    \nonumber
\end{equation}
...
\begin{example}
    \begin{enumerate}
        ...
        \begin{equation}
            ...
            \nonumber
        \end{equation}
        \item Expectational case, $\mathbb{E}[Y\mid X]=\textnormal{exp}(B(X)^T\beta)$
        \begin{equation}
            \begin{aligned}
                Y\mid X\propto \textnormal{Poisson}(\textnormal{exp}(B(X)^T\beta))
            \end{aligned}
            \nonumber
        \end{equation}
    \end{enumerate}
...
\begin{equation}
    \begin{aligned}
        g(Z\mid \beta)=\frac{\partial (Y-\rho(B(X)^T\beta))^2}{\partial \beta}=-2\left[Y-\rho(B(X)^T\beta)\right]\rho'(B(X)^T\beta)B(X)
    \end{aligned}
    \nonumber
\end{equation}
where $\mathbb{E}g(Z\mid \beta)=0$ because $\mathbb{E}[Y|X]=\rho(B(X)^T\beta)$.\\
The Hessian matrix is given by
\begin{equation}
    \begin{aligned}
        G(Z\mid \beta)=\frac{\partial}{\partial \beta^T}g(Z\mid \beta)=
    \end{aligned}
    \nonumber
\end{equation}
