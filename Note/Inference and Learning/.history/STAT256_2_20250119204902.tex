...
\begin{itemize}
    \item Intention-to-treat effect: \(\tau_{ITT} = \Exp[Y \mid Z = 1] - \Exp[Y \mid Z = 0]\)
    \item As-treated estimand: \(\tau_{AT} = \Exp[Y \mid D = 1] - \Exp[Y \mid D = 0]\)
    \item Per protocol estimand: \(\tau_{PP} = \Exp[Y \mid D = Z = 1] - \Exp[Y \mid D = Z = 0]\)
\end{itemize}
...
\begin{align*}
    \tau_{ITT} &= \Exp[Y(Z = 1) \mid Z = 1] - \Exp[Y(Z = 0) \mid Z = 0] \\
    &= \Exp[Y(Z = 1) - Y(Z = 0)]
\end{align*}
...
Recall Fisher's Sharp Null is \(H_0: Y(D = 1) = Y(D = 0)\) (for all units). It is stronger than the ``Neyman's weak null,'' which only asserts $\Exp[Y(D = 1) - Y(D = 0)] = 0$ in expectation. Under Fisher's Sharp Null and assuming binary instrument and treatment,
\begin{align*}
...
\begin{align*}
    \Exp[Y - Y(D = 0)] &= \Exp[Y] - \Exp[Y(D = 0) \mid Z = 0]\\
    &= \Exp[Y] - \Exp[Y \mid Z = 0] \\
    \Exp[Y - Y(D = 0) \mid D = 1] &= \frac{\Exp[Y] - \Exp[Y \mid Z = 0]}{\Pr{D = 1}}
\end{align*}
Where the last equation relies on the fact that \(\Exp[Y - Y(D = 0) \mid D = 0] = 0\). We can estimate these values using various techniques, for example the Regression, Inverse Probability Weighted, and Double Robust estimators.
