...
\begin{proof}
    ...
    \begin{equation}
        \begin{aligned}
            \hat{\phi}_{OLS}=\frac{\sum_{t=2}^TY_{t-1}Y_t}{\sum_{t=2}^T Y_{t-1}^2}=\phi+\frac{\sum_{t=2}^TY_{t-1}\epsilon_t}{\sum_{t=2}^T Y_{t-1}^2},
        \end{aligned}
        \nonumber
    \end{equation}
    where $\epsilon_t=Y_t-\phi Y_{t-1}, t\geq 2$. As we have
    \begin{equation}
        \begin{aligned}
            \mathbb{E}[Y_{t-1}\epsilon_t]=\mathbb{E}[Y_{t-1}(Y_t-\phi Y_{t-1})]
        \end{aligned}
        \nonumber
    \end{equation} $\Rightarrow \mathbb{E}[\epsilon_t\mid Y_1,...,Y_{T-1}]=\epsilon_t\neq 0$ ($2\leq t\leq T-1$)
\end{proof}
