...
Alternatively, in the form of linear model that has a constant,
\begin{equation}
    \begin{aligned}
        Y&=\beta_0+\beta_1^T X+e\\
        &=\beta_0+\beta_1^T(\Gamma_0+\Gamma_1^TZ)+e\\
        &=\beta_0+\beta_1^T\Gamma_0+\beta_1^T\underbrace{(\Gamma_1^TZ)}_{W}+e
    \end{aligned}
    \nonumber
\end{equation}
where $X=\Gamma_0+\Gamma_1^TZ+u$. Then, by the 2SLS,
\begin{enumerate}
    ...
    \begin{equation}
        \begin{aligned}
            \hat{\beta_1}=\frac{\widehat{\textnormal{Cov}(Y,\Gamma_1 Z)}}{\widehat{\textnormal{Var}(\Gamma_1 Z)}}=\frac{\widehat{\textnormal{Cov}(Y,Z)}}{\Gamma_1\widehat{\textnormal{Var}(Z)}}
        \end{aligned}
        \nonumber
    \end{equation}
    By substituting $\hat{\Gamma}_1$, we have
    \begin{equation}
        \begin{aligned}
...
\subsection{Wald Estimator: 2SLS in a Binary Instrument}
...
\begin{equation}
    \begin{aligned}
        Y:&=\beta_0 + \beta_1(\underbrace{(X(1)-X(0))}_{\Gamma_1}Z+X(0))\\
        &=\beta_1\Gamma_1 Z + \beta_0 + \beta_1X(0)
    \end{aligned}
    \nonumber
\end{equation}

In this case, the iv estimator can be written as
\begin{equation}
    \begin{aligned}
        \hat{\beta}_{2SLS}=\frac{\widehat{\textnormal{Cov}(Y,Z)}}{\widehat{\textnormal{Cov}(X,Z)}}=\frac{\widehat{\mathbb{E}[Y|Z=1]}-\widehat{\mathbb{E}[Y|Z=0]}}{\widehat{\mathbb{E}[X|Z=1]}-\widehat{\mathbb{E}[X|Z=0]}}
    \end{aligned}
    \nonumber
\end{equation}
which is called the \textbf{Wald Estimator}.

\begin{theorem}[Convergence with Binary Instrument]\label{thm:2sls_in_potential_outcome_framework}
...
According to Theorem \ref{thm:2sls_in_potential_outcome_framework}, the 2SLS estimator of $Y(1)-Y(0)$ can be given by
\begin{equation}
    \begin{aligned}
        \hat{\beta}_{2SLS}=\frac{\widehat{\textnormal{Cov}(Y,Z)}}{\widehat{\textnormal{Cov}(X,Z)}}\stackrel{P}{\longrightarrow}\frac{\textnormal{Cov}(Y,Z)}{\textnormal{Cov}(X,Z)}=\frac{\mathbb{E}[Y|Z=1]-\mathbb{E}[Y|Z=0]}{\mathbb{E}[X|Z=1]-\mathbb{E}[X|Z=0]}
    \end{aligned}
    \nonumber
\end{equation}
