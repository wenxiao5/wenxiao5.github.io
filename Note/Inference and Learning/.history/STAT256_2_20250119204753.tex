...
\begin{definition}
    ...
    \begin{enumerate}
        ...
        \item \textbf{Exchangeability (Unconfoundedness)}: The IV \(Z\) is independent of the potential outcomes of \(D\) and \(Y\). Specifically, \(Z \indep (D(Z = z), Y(Z = z))\). In the case of a binary IV, this implies \(Z \indep (D(Z = 0), D(Z = 1), Y(Z = 0), Y(Z = 1))\). If covariates \(X\) are included, this condition becomes \(Z \indep (D(Z = z), Y(Z = z)) \given X\).  
    \end{enumerate}
\end{definition}

The \textit{Relevance} assumption has a testable implication: \(\Pr{D = 1 \given Z = 1} - \Pr{D = 1 \given Z = 0} \neq 0\). Note when its value is small but nonzero, \(Z\) is considered to be a weak instrument. However, the other two assumptions are generally untestable.
...
Assuming \( Z \indep Y \mid (X, U) \) and, without loss of generality, that:
\[
...
Under \(Z \indep Y(z) \mid X\), we have:
\[
