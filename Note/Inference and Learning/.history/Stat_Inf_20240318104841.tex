...
\begin{enumerate}
    \item Step 1:
    \begin{enumerate}
        \item[1a.] $Y|X\sim \textnormal{Bern}(F(X^T\beta_0))$
        \item[1b.] Given $X=1$, $Y\sim \textnormal{Bern} (p)$, $p\in (0,1)$.\\
        $\ln \textnormal{Pr}(Y=y|p)=\ln f(Y|X,p)=y\ln p+(1-y)\ln (1-p)$, the score is given by $g(Y|X,p)=\frac{\partial \ln f(Y|X,p)}{\partial p}=\frac{y-p}{p(1-p)}$ which is i.i.d. sample $(Y_l,X_l)_{l=1}^n$, $\hat{p}_{MLE}=\frac{1}{n}\sum_{i=1}^n Y_i$. $\frac{1}{n}\sum_{i=1}^n g(Y_i|X_i,\hat{p}_{MLE})=0$
        \item[1c.] Conditional likelihood with $X$. (1a.) gives 
    \end{enumerate}
\end{enumerate}
\begin{enumerate}[$1$a.]
