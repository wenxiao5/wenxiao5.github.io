...
\begin{corollary}
    If $g(\theta_0):=\mathbb{E}g(z,\theta_0)=0$, then $I(\theta_0)=\mathbb{E}g(z,\theta_0)g(z,\theta_0)^T=G(\theta_0)$. Hence, $$\sqrt{n}\left(\hat{\theta}-\theta_0\right) \stackrel{d}{\Rightarrow} N\left(0,I(\theta_0)^{-1}\right)$$
\end{corollary}
...
\begin{equation}
    ...
    \nonumber
\end{equation}
The sample criterion function is given by $$\hat{Q}_n(\theta)=-\frac{1}{n}\sum_{i=1}^n$$
...
Let the weight of observation $i$ be $w(X_i,\beta):=\frac{F'(X_i^T\beta)}{F(X_i^T\beta)(1-F(X_i^T\beta))}\cdot X_i$. Then, $\sum_{i=1}^n w(X_i,\hat{\beta}_{MLE})\cdot (Y_i-F(X_i^T\hat{\beta}_{MLE}))=0^d$.
\begin{example}[ (Logistic Regression)]
    ...
    which implies that $g(Y\mid X;\beta)=\left(Y-F(X^T\beta)\right)X$. In this case, $w(X_i,\beta)=X_i$ doesn't depend on $\beta$. Therefore, if $F$ is logit, $\sum_{i=1}^n X_i\cdot (Y_i-F(X_i^T\hat{\beta}_{MLE}))=0^d$.
\end{example}
\subsection{Consistency and }
Remind that $\hat{\beta}_{MLE}$ is M-estimator.
\begin{assumption}
