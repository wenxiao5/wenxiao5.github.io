...
\begin{definition}[Covariance Stationary]
    A process $\{Y_t:t\in \mathbb{Z}\}$ is \textbf{covariance stationary} \textit{iff} $\mathbb{E}[Y_{t,i}^2]<\infty$ ($\forall t,i$) and it satisfies \eqref{s} and \eqref{ss}.
    \begin{enumerate}
        \item Same Expectation:
        \begin{equation}
            \begin{aligned}
                \mathbb{E}[Y_t]=\left(\mathbb{E}[Y_{t,1}],...,\mathbb{E}[Y_{t,n}]\right)'=\mu,\ \forall t \textnormal{ (for some constant $\mu$)}
            \end{aligned}
            \label{s}
            \tag{*}
        \end{equation}
        ...
        \begin{equation}
            \begin{aligned}
                \textnormal{Cov}(Y_t,Y_{t-j})=\mathbb{E}[\underbrace{(Y_t-\mu)(Y_{t-j}-\mu)'}_{n\times n}]=\gamma(j),\ \forall t,j \textnormal{ (for some function $\gamma(\cdot)$)}
            \end{aligned}
            \label{ss}
            \tag{**}
        \end{equation}
