...
\begin{enumerate}
    ...
    \item Score: $g(\theta):=\mathbb{E}_{\theta}g(Z,\theta)$. First order condition requires $g(\theta_0)=\mathbb{E}g(Z,\theta_0)$
    \item Information Matrix: $I(\theta):=\mathbb{E}[g(Z,\theta)g(Z,\theta)^T]$
    \item Hessian: $G(\theta):=\mathbb{E}_{\theta}G(Z,\theta)=\textnormal{Var}(g(Z,\theta))$. (We use $G$ denote the true population Hessian, $G:=G(\theta_0)$).
\end{enumerate}
