...
\begin{equation}
    \begin{aligned}
        \underbrace{f(\Gamma,X)}_\textnormal{joint distribution of $\Gamma$ and $X$}:=\underbrace{\textnormal{Pr}(\Gamma=1|X)}_{=F(X^T\beta)}\cdot \underbrace{f_X(X)}_\textnormal{marginal density of $X$ (no parameters of interest)}
    \end{aligned}
    \nonumber
\end{equation}
Then,
\begin{equation}
    \begin{aligned}
        \ln f(\Gamma,X)=\ln F(X^T\beta)+\ln f_X(X)
    \end{aligned}
    \nonumber
\end{equation}
Let
\begin{equation}
    ...
    \nonumber
\end{equation}
Since $\mathbb{E}_{\beta}\ln f_X(X)$ doesn't depend on $\beta$,
\begin{equation}
    \begin{aligned}
