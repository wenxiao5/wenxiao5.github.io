...
\begin{equation}
    ...
    \nonumber
\end{equation}
Since $P(\vec{x}_i\in C_k)$ is either $0$ or $1$ and can equal to $1$ for only one $k=1,...,K$, let $P(\vec{x}_i\in C_{k_1})=1$, we have $\sum_{k=1}^K P\left(\vec{x}_i \in C_k\right) P\left(\vec{x}_i \mid \vec{x}_i \in C_k, \theta_k\right)$ given $\vec{x}_i$
