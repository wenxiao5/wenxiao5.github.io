...
\begin{theorem}[Asymptotic Normality of M-estimators]\label{thm:asymptotic_normality_of_m_estimators}
    ...
    \begin{enumerate}
        ...
        \item[A5.] \underline{Continous convergence of the Hessian}: for any sequence $\tilde{\theta}_n \stackrel{P}{\longrightarrow} \theta_0$, $\hat{G}(\tilde{\theta}_n):=\frac{\partial^2}{\partial \theta\partial \theta^T}\hat{Q}_n(\tilde{\theta}_n) \stackrel{P}{\longrightarrow} B_0$ for some non-stochastic $d\times d$ matrix (usually we check $G_0$).
    \end{enumerate}
    Then,
    \begin{equation}
        \begin{aligned}
...
Consider $Y_i=e^{\theta_0}+\epsilon_i, \mathbb{E}[\epsilon_i]=0$. Suppose $\theta_0\in\Theta\subset \mathbb{R}$, with $\Theta$ compact. Let $\{Y_i\}_{i=1}^N$ a simple random sample with finite second moments and consider
\begin{equation}
    \begin{aligned}
        \hat{Q}_N(\theta)=\frac{1}{N}\sum_{i=1}^N m(Y_i,\theta)
    \end{aligned}
    \nonumber
\end{equation}
where $m(Y_i,\theta)=(Y_i-e^\theta)$. Let $\hat{\theta}=\argmin_{\theta\in\Theta}\hat{Q}_N(\theta)$.

\subsubsection*{Consistency of $\hat{\theta}$}
To show the m-estimator $\hat{\theta} \stackrel{P}{\longrightarrow}\theta_0$, we check the assumptions for consistency being satisfied:
\begin{enumerate}
    \item 
\end{enumerate}
