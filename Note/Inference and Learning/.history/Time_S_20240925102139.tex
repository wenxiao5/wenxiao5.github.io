...
\begin{theorem}[Herglotz's Theorem]
    A function $\gamma: \mathbb{Z}\rightarrow \mathbb{R}$ is positive semi-definite iff
    \begin{equation}
        \begin{aligned}
...
\paragraph*{Symmetry} Suppose $\gamma(j)=\int_{-\pi}^\pi \exp\left(ij\lambda\right)d F(\lambda),j\in \mathbb{Z}$.

If $\gamma(j)\in \mathbb{R},\forall j$, then
\begin{equation}
    \begin{aligned}
