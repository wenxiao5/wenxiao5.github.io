...
\begin{minipage}[t]{0.6\textwidth}
    When we want to do integrals in high dimensions, it can be really challenging.

    For $d$-dimensional integrals, the trapezoidal rule yields an approximation error $|\hat{I}_n - I| \leq \frac{C}{n^{\frac{2}{d}}}$ for some constant $C$. This means that $n$ should scale as $n \sim O\left(\varepsilon^{-\frac{d}{2}}\right)$ in order to achieve a target approximation error of $\varepsilon$. This phenomenon is known as \underline{the curse of dimensionality}.
\end{minipage}%
\hfill
\begin{minipage}[t]{0.4\textwidth}
    \begin{figure}[H]
        \includegraphics[width=\linewidth]{two-dim int.png}
