...
$$
Range of AIC is $(-\infty, \infty)$.\\
\textbf{Interpretation:} The lower the AIC, the better the model is.

\textbf{Note:} AIC dones't work well for small datasets. Use Aic when $\frac{n}{n_p}>40$.

\subsection{Evaluation Metric 2: Bayes Information Criterion (BIC)}
If $n_p$ is the number of parameters that a probabilistic model is trying to learn, \textbf{$n$ is the sample size}, and $L_{o p t}$ is the maximum negative log-likelihood, then the AIC of the model is defined as follows. (Note: the "log" in this metric is "In()".)
$$
