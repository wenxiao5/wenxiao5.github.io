...
\begin{definition}
\normalfont
Suppose the data $Z$ follows true distribution $F$.
\begin{enumerate}
    \item Loss: $Q(\theta):=\mathbb{E}_{F}m(Z,\theta)$.
    \item Score: $g(\theta):=\mathbb{E}_{F}g(Z,\theta)$.
    %\item Information Matrix: $\mathcal{I}(\theta):=\mathbb{E}[g(Z,\theta)g(Z,\theta)^T]$
    \item Hessian: $G(\theta):=\mathbb{E}_{F}G(Z,\theta)= \mathbb{E}_F\left[\frac{\partial^2}{\partial \theta\partial \theta^T}m(Z,\theta)\right]$. (We use $G$ denote the true population Hessian, $G:=G(\theta_0)$).
\end{enumerate}
\end{definition}
...
\begin{theorem}[Asymptotic Normality of M-estimators]\label{thm:asymptotic_normality_of_m_estimators}
    ...
    \begin{enumerate}
        ...
        \item[A3.] $G:=G(\theta_0)=\mathbb{E}\left[\frac{\partial^2}{\partial \theta\partial \theta^T}m(Z,\theta_0)\right]$ is invertible.
    \end{enumerate}
    Then,
    \begin{equation}
        \begin{aligned}
