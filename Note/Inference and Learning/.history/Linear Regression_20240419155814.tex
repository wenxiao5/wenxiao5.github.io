...
\begin{theorem}[Asymptotic Normality of M-estimators]\label{thm:asymptotic_normality_of_m_estimators}
    ...
    \end{enumerate}
    \textbf{Or, assumptions in ECON 715 lecture note}
    \begin{enumerate}
        \item[A3.] $\hat{Q}_n(\theta)$ is twice continuously differentiable in on some neighborhood $\Theta_0\subset \Theta$ of $\theta_0$ (with probability one).
        \item[A4.] $\sqrt{n}\hat{g}(\theta_0):=\sqrt{n}\frac{\partial}{\partial \theta}\hat{Q}_n(\theta) \stackrel{d}{\longrightarrow} N(0,\Omega)$
    \end{enumerate}
    Then,
    \begin{equation}
        \begin{aligned}
