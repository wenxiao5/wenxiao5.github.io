...
\begin{lemma}[Properties of GMM Estimator]
    ...
    \end{enumerate}
    Then, $\sqrt{T}\left(\hat{\theta}_{GMM}-\theta_0\right)\stackrel{d}{\longrightarrow} \mathcal{N}\left(0,\Omega\right)$, where $\Omega:=\left[G'WG\right]^{-1}G'WVWG\left[G'WG\right]^{-1}$, $$\Omega(W)\geq \Omega(V^{-1})=(G'WG)^{-1}$$
\end{lemma}
\begin{remark}
    \begin{enumerate}
...
\begin{proposition}[Newey-West Estimator of $V$]
    $$\hat{V}=\lim_{T\rightarrow \infty}\textnormal{Var}[\sqrt{T}g_T(\theta_0)]=\hat{\Gamma}_n(0)+\sum_{j=1}^\infty\left(1-\frac{j}{b}\right)[\hat{\Gamma}_n(j)+\hat{\Gamma}_n(j)']$$
    where $\hat{\Gamma}_n(j)=\frac{1}{T}\sum_{t=j+1}^Th(X_t,\hat{\theta})h(X_{t-j},\hat{\theta})'$
\end{proposition}
