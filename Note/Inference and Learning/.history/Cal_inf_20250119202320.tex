...
In this case, even if $X \independent Z | Y(0), Y(1)$, $e_n(x)$ is not sufficient for ignorability due to the unobserved confounding factor U. Therefore, adjusting for propensity scores may not remove all the bias in estimating the treatment effect.
...
\textbf{Theorem:}
\begin{center}
    If $Z \independent (Y(1), Y(0)) \thinspace|\thinspace X$
then $Z \independent (Y(1), Y(0)) \thinspace|\thinspace e(X)$
\end{center}
...
We see that $Z \independent (Y(1), Y(0)) \thinspace|\thinspace \hat{e}'(x) = e_k$ approximately holds for $k = 1,2,...,k$\\\\
