...
The first part of the algorithm is to create a \textbf{probability distribution} that represents \textbf{similarities} between neighbors.

Original paper states “\textbf{similarity} of datapoint $x_j$ to datapoint $x_i$ is the conditional probability $p_{j\mid i}$, that $x_i$ would pick $x_j$ as its neighbor“.

When we've picked one of the points from the dataset. Now we have to pick another point and calculate Euclidean Distance between them $\|x_i - x_j\|$.
\begin{enumerate}
    ...
    Similarity between a given point $x_i$ and another point $x_j$ is a function of a normal distribution centered at $x_i$ with a standard deviation $\sigma_i$ that changes based on the point and how many "neighbors" you think each point in the dataset has.
    \begin{equation}
        \begin{aligned}
        \boldsymbol{p}_{i j}= & \frac{1}{\mathbf{2 m}}\left(\boldsymbol{p}_{j \mid i}+\boldsymbol{p}_{i \mid j}\right) \\
        & \bullet p_{j \mid i}=\frac{\exp \left(-\frac{\left\|x_i-x_j\right\|^2}{2 \sigma_i^2}\right)}{\sum_{k \neq i} \exp \left(-\frac{\left\|x_i-x_k\right\|^2}{2 \sigma_i^2}\right)} \\
        & \bullet p_{i \mid j}=\frac{\exp \left(-\frac{\left\|x_i-x_j\right\|^2}{2 \sigma_j^2}\right)}{\sum_{k \neq j} \exp \left(-\frac{\left\|x_j-x_k\right\|^2}{2 \sigma_j^2}\right)}
        \end{aligned}
        \nonumber
    \end{equation}
    \item \textbf{\underline{Creates a Similarity Matrix $Q$ for the Points (i.e., Decision Variables we are Trying to Solve for)} in the Projected Dataset:} Similarity between a given point $y_i$ and another point $y_j$ is a function about $t$-distribution centered at $y_i$.
