...
Use the notations of vectors $\vec{\dot{Y}}_{i}:=\begin{bmatrix}
    ...
    \vdots\\
    \dot{Y}_{i_T}
\end{bmatrix}=\vec{Y}_i-\mathbf{1}\left(\mathbf{1}^T \mathbf{1}\right)^{-1} \mathbf{1}^T \vec{Y}_i=:Q \vec{Y}_i$, where $Q:=I-\mathbf{1}\left(\mathbf{1}^T \mathbf{1}\right)^{-1} \mathbf{1}^T$. Then, the equation $\vec{\dot{Y}}_i=\vec{\dot{X}}_i\beta_0+\vec{\dot{\epsilon}}_i$ can be written as
\begin{equation}
    \begin{aligned}
    ...
    \nonumber
\end{equation}
(Actually, all we want to do is constructing a matrix $Q$ such that $Q\alpha_i=0$.)
...
\begin{enumerate}
    ...
    \begin{equation}
        ...
        \nonumber
    \end{equation}
    The sufficient condition is $\mathbb{E}[\vec{X}^TQ\vec{\epsilon}]=0$.
    \begin{equation}
        \begin{aligned}
            \mathbb{E}[\vec{\dot{X}}^T\vec{\dot{\epsilon}}]=\sum_{t=1}^T \mathbb{E}[\dot{X}_t \dot{\epsilon}_t]=0
        \end{aligned}
        \nonumber
    \end{equation}
    \item 
