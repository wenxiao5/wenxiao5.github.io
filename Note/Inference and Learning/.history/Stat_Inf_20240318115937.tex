...
\begin{example}
    $Q(\theta)=-\mathbf{1}_\{\theta=0\}$ and $Q_n(\theta)=-\mathbf{1}_\{\theta=0\}-2\mathbf{1}_\{\theta=n\}$. $\theta_n \nrightarrow \theta_0$ but $Q_n(\theta)-Q(\theta) \rightarrow 0$.
\end{example}
...
\begin{enumerate}
    ...
    \item Information Matrix: $I(\theta):=\mathbb{E}[g(Z,\theta)g(Z,\theta)^T]$
    \item Hessian: $G(\theta):=\mathbb{E}_{\theta_0}G(Z,\theta)=\textnormal{Var}(g(Z,\theta_0))$. (We use $G$ denote the true population Hessian, $G:=G(\theta_0)$).
\end{enumerate}
\end{definition}
Since $\theta_0$ that $\theta_0=\argmin_{\theta\in\Theta} Q(\theta)$, $g(\theta_0)=[0,...,0]^T$ ($1^{st}$ Information Equality / Alternative definition of $\theta_0$).
...
\begin{proof}
    ...
    \begin{equation}
        \begin{aligned}
            ...
            \textnormal{Var}(\hat{g}(\theta_0))&=\frac{1}{n}\underbrace{\textnormal{Var}(g(Z,\theta_0))}_{:=I(\theta_0)}
        \end{aligned}
        \nonumber
    \end{equation}
    By Taylor,
...
The goal in binary choice analysis is estimation of the \textbf{conditional or response probability} $\textnormal{Pr}[Y = 1 | X]$ given a set of regressors $X$. We may be interested in the response probability or some transformation such as its derivative - the \textbf{marginal effect}.

$\Gamma\in\{0,1\}$, $X\in \mathbb{R}^d$ which affects $\Gamma$ via $X^T\beta_0$, where $\beta_0\in \mathbb{R}^d$. The link is represented by a function $F: \mathbb{R} \rightarrow [0,1]$.
\begin{equation}
    \begin{aligned}
        \textnormal{Pr}(\Gamma=1\mid X)=F(X^T\beta_0)
    \end{aligned}
    \nonumber
\end{equation}
\begin{example}
    The choice of link:
    \begin{enumerate}
        \item LPM: $F(t)=t \mathbf{1}\{t\in[0,1]\}=\left\{\begin{matrix}
            0,&t\leq 0\\
            t,&t\in[0,1]\\
            1,&t\geq 1
        \end{matrix}\right.$ (projection).
        \item Logit: $F(t)=\frac{e^t}{1+e^t}$
        \item Probit: $F(t)=\Phi(t)=\int_{-\infty}^t\frac{1}{\sqrt{2\pi}}e^{-\frac{z^2}{2}}dz$
    \end{enumerate}
\end{example}
Structural motivation for binary choice:
\begin{equation}
    \begin{aligned}
        \Gamma^*(d)=X^Ty_d+\epsilon(d),d\in\{0,1\}
    \end{aligned}
    \nonumber
\end{equation}
where $X^Ty_d$ is the predicted/explained part of utility and $\epsilon(d)$ is the ``taste shock'' unobservable part of utility. The key difference from RCT is the $\Gamma^*$ is not randomly assigned.

Identification:
\begin{equation}
    \begin{aligned}
        \textnormal{Pr}(\Gamma=1|X)&=\textnormal{Pr}(\Gamma^*(1)\geq \Gamma^*(0)|X)\\
        &=\textnormal{Pr}(X^Ty_1+\epsilon(1)\geq X^Ty_0+\epsilon(0))\\
        &=\textnormal{Pr}(\frac{\epsilon(0)-\epsilon(1)}{\sqrt{\textnormal{Var}(\epsilon(0)-\epsilon(1))}})\leq X^T\left(\frac{y_1-y_0}{\sqrt{\textnormal{Var}(\epsilon(0)-\epsilon(1))}}\right)\\
        &=F_{\epsilon(0)-\epsilon(1)}\left(X^T\left(\frac{y_1-y_0}{\sqrt{\textnormal{Var}(\epsilon(0)-\epsilon(1))}}\right)\right)
    \end{aligned}
    \nonumber
\end{equation}
where $\epsilon(1),\epsilon(0)$ are jointly normal. $\epsilon(1)-\epsilon(0)\sim N(0,\sigma^2_{\epsilon(1)-\epsilon(0)})$. It gives probit link function.

$\beta=\frac{\Gamma_1-\Gamma_0}{\sigma_{\epsilon(1)-\epsilon(0)}}\in \mathbb{R}^d$.
...
\begin{equation}
    \begin{aligned}
        \frac{\partial}{\partial X_j}\textnormal{Pr}(\Gamma=1|X=X)=F'(X^T\beta_0)\cdot\beta_j
    \end{aligned}
    \nonumber
\end{equation}
...
\begin{equation}
    ...
    \nonumber
\end{equation}
The marginal effect for an ``average persion'' (MEA) (may not make sense if $X$ is discrete).
\begin{equation}
    \begin{aligned}
...
Denote the joint distribution of $\Gamma$ and $X$
\begin{equation}
    \begin{aligned}
        \underbrace{f(\Gamma,X)}_\textnormal{joint distribution of $\Gamma$ and $X$}:=\underbrace{\textnormal{Pr}(\Gamma=1|X)}_{=F(X^T\beta_0)}\cdot \underbrace{f_X(X)}_\textnormal{marginal density of $X$ (no parameters of interest)}
    \end{aligned}
    \nonumber
\end{equation}
Then,
\begin{equation}
    \begin{aligned}
        \ln f(\Gamma,X)=\ln F(X^T\beta_0)+\ln f_X(X)
    \end{aligned}
    \nonumber
\end{equation}
Let
\begin{equation}
    \begin{aligned}
        Q(\beta):=-\mathbb{E}_{\beta_0}\ln f(\Gamma,X)=-\mathbb{E}_{\beta_0}\ln F(X^T\beta_0)-\mathbb{E}_{\beta_0}\ln f_X(X)
    \end{aligned}
    \nonumber
\end{equation}
Since $\mathbb{E}_{\beta_0}\ln f_X(X)$ doesn't depend on $\beta$,
\begin{equation}
    \begin{aligned}
        \arg\min_{\beta}Q(\beta)\equiv \arg\max_{\beta}\mathbb{E}_{\beta}\ln F(X^T\beta)
    \end{aligned}
    \nonumber
\end{equation}

$F(X^T\beta)=f(Y|X,\beta)$.

Step 1:
\begin{enumerate}[$1$a.]
    \item $\textnormal{Pr}(Y=1|X,\beta)=F(X^T\beta)$.
    \item Log-likelihood $$\ln f(Y|X,\beta)=Y\cdot \ln F(X^T\beta)+(1-Y)\cdot \ln (1-F(X^T\beta))$$
    \item Take the derivative, the score is
    \begin{equation}
        \begin{aligned}
            g(Y|X,\beta):=\frac{\partial \ln f(Y|X,\beta)}{\partial \beta}&=\frac{\partial \ln f(Y|X,\beta)}{\partial F(X^T\beta)}\frac{\partial F(X^T\beta)}{\partial \beta}\\
            &=\left(\frac{Y-F(X^T\beta)}{F(X^T\beta)(1-F(X^T\beta))}\right)\cdot\left(F'(X^T\beta)\cdot X\right)
        \end{aligned}
        \nonumber
    \end{equation}
\end{enumerate}
\paragraph*{MLE, $\hat{\beta}_{MLE}$:}
$(Y_i,X_i)_{i=1}^n$ are i.i.d. samples. $\hat{\beta}_{MLE}$ is given by solving F.O.C.
\begin{equation}
    \begin{aligned}
        \hat{g}(\beta)|_{\beta=\hat{\beta}_{MLE}}=\frac{1}{n}\sum_{i=1}^ng(Y_i|X_i,\beta)\big|_{\beta=\hat{\beta}_{MLE}}=0^d
    \end{aligned}
    \nonumber
\end{equation}
which is a system of (non)linear equations. Let the weight of observation $i$ be
\begin{equation}
    \begin{aligned}
        w(X_i,\beta):=\frac{F'(X_i^T\beta)}{F(X_i^T\beta)(1-F(X_i^T\beta))}\cdot X_i
    \end{aligned}
    \nonumber
\end{equation}
Then,
\begin{equation}
    \begin{aligned}
        \sum_{i=1}^n w(X_i,\hat{\beta}_{MLE})\cdot (Y_i-F(X_i^T\hat{\beta}_{MLE}))=0^d
    \end{aligned}
    \nonumber
\end{equation}
\begin{example}
    Suppose the link function is $F(t)=\frac{e^t}{1+e^t}$. $$F'(t)=\frac{e^t(1+e^t)-e^t\cdot e^t}{(1+e^t)^2}=\frac{e^t}{1+e^t}\cdot\frac{1}{1+e^t}=F(t)\cdot(1-F(t))$$
    In this case, $w(X_i,\beta)=X_i$ doesn't depend on $\beta$. Therefore, if $F$ is logit, $\sum_{i=1}^n X_i\cdot (Y_i-F(X_i^T\hat{\beta}_{MLE}))=0^d$.
\end{example}
\subsection{Consistency}
Is $\hat{\beta}_{MLE}$ consistent (i.e. $=\argmax_\beta\ln f(Y_i|X_i,\beta)$)?
\begin{assumption}
    The consistency theorem requires assumptions:
    \begin{enumerate}[{A}1.]
        \item $Q(\beta)$ is uniquely minimized at $\beta=\beta_0$.
        \item $Q(\beta):\mathbb{R}^d \rightarrow \mathbb{R}$ is continuous on a compact subset of $\mathbb{R}$. ($Q(\beta)$ is continuous if the link $F(\cdot)$ is continuous.)
        \item Uniform Convergence (if $Q(\beta)$ is convex in $\beta$, pointwise convergence is enough, which follows from LLN.)
    \end{enumerate}
\end{assumption}

We want to to find the ``sufficient conditions'' for A1 (to ensure that $Q(\beta)$ is uniquely minimized at $\beta_0$).
\begin{example}
...
\subsection{Asymptotic Normality of $\hat{\beta}_{MLE}$}
$$\ln f(Y|X,\beta)=Y\cdot \ln F(X^T\beta)+(1-Y)\cdot \ln (1-F(X^T\beta))$$
Consider the logistic case, $F(t)=\frac{e^t}{1+e^t}$, the score is
\begin{equation}
    \begin{aligned}
        g(Y|X,\beta)=(Y-F(X^T\beta))\cdot X
    \end{aligned}
    \nonumber
\end{equation}
...
\begin{equation}
    \begin{aligned}
        I(\beta_0)=\textnormal{Var}(g(Y|X,\beta_0))&=\mathbb{E}g(Y|X,\beta)\cdot g(Y|X,\beta)^T\\
        &=\mathbb{E}(Y-F(X^T\beta))^2\cdot X\cdot X^T
    \end{aligned}
    \nonumber
\end{equation}
Since $Y|X\sim \textnormal{Bernoulli}(F(X^T\beta_0))$, $\textnormal{Var}(Y|X)=F(X^T\beta_0)\cdot(1-F(X^T\beta_0))$. Then,
\begin{equation}
    \begin{aligned}
        I(\beta_0)=\mathbb{E}F(X^T\beta_0)\cdot(1-F(X^T\beta_0))\cdot X X^T
    \end{aligned}
    \nonumber
\end{equation}
By the property of logit, $F(X^T\beta_0)\cdot(1-F(X^T\beta_0))=F'(X^T\beta_0)$. Then, the Hessian is $$G(\beta_0)=-I(\beta_0)$$
By Theorem \ref{thm:asymptotic_normality_of_m_estimators},
\begin{equation}
    \begin{aligned}
        \sqrt{n}\left(\theta_{MLE}-\theta_0\right) \stackrel{d}{\Rightarrow} N\left(0,\Omega\right)
    \end{aligned}
    \nonumber
\end{equation}
where
\begin{equation}
    \begin{aligned}
        \Omega=G^{-1}I(\beta_0)G^{-1}=[I(\beta_0)]^{-1}
    \end{aligned}
    \nonumber
\end{equation}
The standard errors
\begin{equation}
    \begin{aligned}
        se(\theta_{MLE})=\textnormal{diagonal}\left(\frac{1}{n}\hat{I}(\theta_{MLE})^{-1}\right)^{\frac{1}{2}}
    \end{aligned}
    \nonumber
\end{equation}
...
\begin{equation}
    \begin{aligned}
        \sqrt{n}\left(\theta_{MLE}-\theta_0\right) \stackrel{d}{\Rightarrow} N\left(0,G^{-1}I(\beta_0)G^{-1}\right)
    \end{aligned}
    \nonumber
\end{equation}
...
\begin{definition}
    ...
    \end{enumerate}
    $\hat{\beta}$ is \textbf{efficient} if for any other $\hat{\beta}_3$, $\Omega_{\hat{\beta}_2}-\Omega_{\hat{\beta}_1}\succeq 0$ in matrix sense.
\end{definition}
