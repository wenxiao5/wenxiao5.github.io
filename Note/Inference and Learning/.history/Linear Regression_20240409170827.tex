...
\subsection{General Information Notations}
Define
\begin{enumerate}[$\circ$]
...
\begin{enumerate}
    ...
    \item Hessian: $G(\theta):=\mathbb{E}_{\theta}G(Z,\theta)= \mathbb{E}\left[\frac{\partial^2}{\partial \theta\partial \theta^T}m(Z,\theta)\right]$. (We use $G$ denote the true population Hessian, $G:=G(\theta_0)$).
\end{enumerate}
\end{definition}
...
\end{example}
Under ``Regularity" condition, $\theta_0=\argmin_{\theta\in\Theta} Q(\theta)$, $g(\theta_0)=0^d$ ($1^{st}$ Information Equality / Alternative definition of $\theta_0$). In this case, $\mathcal{I}(\theta_0)=\textnormal{Var}[g(Z,\theta_0)]$.
\begin{lemma}[Second Information Equality]
    Under ``Regularity" condition, $G=-\mathcal{I}(\theta_0)$.
\end{lemma}
...
\begin{theorem}[Asymptotic Normality of M-estimators]\label{thm:asymptotic_normality_of_m_estimators}
    ...
    \begin{equation}
        \begin{aligned}
            \Omega=\textnormal{Var}(g(Z,\theta_0))
        \end{aligned}
        \nonumber
    \end{equation}
\end{theorem}
...
\begin{corollary}[Second Information Equality]\label{cor:asymptotic_normality_of_m_estimators}
    Under ``Regularity" condition, $\mathcal{I}(\theta_0)=-G(\theta_0)$, $$\sqrt{n}\left(\hat{\theta}-\theta_0\right) \stackrel{d}{\Rightarrow} N\left(0,\mathcal{I}(\theta_0)^{-1}\right)$$
...
\end{definition}
In this case, the second information equality may not hold, $G^{-1}\mathcal{I}(\beta_0)G^{-1}\neq \mathcal{I}(\beta_0)^{-1}$.
\begin{equation}
    \begin{aligned}
...
\begin{enumerate}[(a).]
    ...
    \item Even if $y$ doesn't have PDF $f(y;\theta_0)$ ($f(y;\theta')$ instead), we can still give this result. In this case, the information inequality $\mathbb{E}_{\theta'}[\ln (y;\theta_0)]>\mathbb{E}_{\theta'}[\ln (y;\theta)]$ gives
    \begin{equation}
        \begin{aligned}
            A(\theta_0)+\int f(y ; \theta')B(y)dy+C(\theta_0)\theta'&>A(\theta)+\int f(y ; \theta')B(y)dy+C(\theta)\theta'\\
        \end{aligned}
        \nonumber
    \end{equation}
    \item 
