...
\paragraph*{Symmetry} Suppose $\gamma(j)=\int_{-\pi}^\pi \exp\left(ij\lambda\right)d F(\lambda),j\in \mathbb{Z}$. Then,
\begin{equation}
    \begin{aligned}
    ...
    \nonumber
\end{equation}
If $\gamma(j)\in \mathbb{R},\forall j$, then $\int_{-\pi}^\pi\sin(j\lambda)d F(\lambda)=0.$
