...
\begin{lemma}[Unbiasedness of $\bar{Y}$]
    $\bar{Y}$ is unbiased (i.e., $\mathbb{E}(\bar{Y})=\mu$).
\end{lemma}
\begin{proof}
    Recall that $\mathbb{E}(Y_t)=\mu, \forall t$ because $\epsilon_t\sim \textnormal{WN}(0,\sigma^2)$ and $\sum_{i=0}^\infty \psi_i^2<\infty$. Then,
    \begin{equation}
        \begin{aligned}
