...
\begin{theorem}[Theorem to Check Minimal Sufficient Statistic]
    ...
    \begin{center}
        "for every two sample points $\vec{X}$ and $\vec{Y}$, the ratio $\frac{f(\vec{X}\mid\theta)}{f(\vec{Y}\mid\theta)}$ is constant as a function of $\theta$ \underline{if and only if} $T(\vec{X}) = T(\vec{Y})$".
    \end{center}
