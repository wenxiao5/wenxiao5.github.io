...
\begin{theorem}[Achieving the Cramér-Rao Lower Bound]
    An estimator $\hat{\theta}$ whose variance achieves the Cramér-Rao lower bound, $\textnormal{Var}_\theta[\hat{\theta}(\vec{X})]=\textnormal{CRLB}(\theta)$, \underline{if and only if},
\end{theorem}
