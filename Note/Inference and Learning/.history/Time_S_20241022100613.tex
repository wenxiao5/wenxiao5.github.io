...
Remind that a scalar\footnote process $\{Y_t:t\in \mathbb{Z}\}$ is strictly stationary \textit{if and only if} $\left(Y_t,...,Y_{t+k}\right)\underbrace{\sim}_\textnormal{``is distributed as''} \left(Y_0,...,Y_{k}\right),\ \forall t\in \mathbb{Z},k\geq 0$.
