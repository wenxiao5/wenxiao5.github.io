...
Given $\vec{X}=\{X_1,...,X_N\}$, we want to derive a decision rule $d(\vec{Y})$ given $\vec{Y}$
...
\begin{definition}[Sum of Squared Residual]
    ...
    \begin{equation}
        ...
        \nonumber
    \end{equation}
    where the norm is $\|\vec{X}\|=\left[\sum_{i=1}^N X_i^2\right]^{1/2}$, and $df(\hat{\vec{m}})=\frac{\sum_{i=1}^N \textnormal{Cov}(Y_i,\hat{m_i})}{\sigma^2}$.
\end{definition}
