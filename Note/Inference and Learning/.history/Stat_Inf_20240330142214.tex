...
\begin{example}
    ...
    \begin{enumerate}
        ...
        \item Logit Model: $F(t)=\frac{e^t}{1+e^t}$
        \item Probit Model: $F(t)=\Phi(t)=\int_{-\infty}^t\frac{1}{\sqrt{2\pi}}e^{-\frac{z^2}{2}}dz$
    \end{enumerate}
\end{example}

\subsection{Latent Utility Models}
Structural motivation for binary choice: An agent makes a binary choice $d \in \{0, 1\}$, $\mathbb{E}\epsilon(0)=\mathbb{E}\epsilon(1)=0$. The utility of each choice is given by
\begin{equation}
    \begin{aligned}
    ...
    \nonumber
\end{equation}
where $X^T\gamma_d$ is the predicted/explained part of utility and $\epsilon(d)$ is the ``taste shock'' unobservable part of utility. The key difference from RCT is the $Y^*$ is not randomly assigned.
