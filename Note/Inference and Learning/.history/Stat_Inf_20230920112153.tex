...
\begin{equation}
    ...
    \nonumber
\end{equation}
which is given by $\mathbb{E}[X^2]=\mathbb{E}[X]^2+\textnormal{Var}(X)$. Hence, if $\textnormal{Bias}_\theta(\hat{\theta})=0$, $\textnormal{MSE}_\theta(\hat{\theta})=\textnormal{Var}_\theta(\hat{\theta})$.

\begin{definition}[Optimality]
    \normalfont
    An unbiased estimator $\hat{\theta}$ is a \textbf{uniform minimum variance unbiased} estimator (of $\theta$) iff
    \begin{equation}
        \begin{aligned}
            \textnormal{Var}_\theta(\hat{\theta})\leq \textnormal{Var}_\theta(\tilde{\theta})
        \end{aligned}
        \nonumber
    \end{equation}
