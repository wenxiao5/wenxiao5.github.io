...
\begin{definition}[Simulate samples from $\textnormal{Dirichlet}(\vec{\alpha})$]
    \normalfont
    \begin{enumerate}[1.]
...
\begin{definition}[Sum of Squared Residual]
    \normalfont
    \textbf{Sum of Squared Residual} (SSR) of an estimation $\hat{\boldsymbol{m}}$ is given by
    \begin{equation}
        \begin{aligned}
            \mathbb{E}\left[\|\vec{Y}-\hat{\boldsymbol{m}}\|^2\right]&=\mathbb{E}\left[\|\vec{Y}-\boldsymbol{m}\|^2\right]+\mathbb{E}\left[\|\hat{\boldsymbol{m}}-\boldsymbol{m}\|^2\right]-2 \mathbb{E}\left[(\vec{Y}-\boldsymbol{m})'(\hat{\boldsymbol{m}}-\boldsymbol{m})\right]\\
...
\begin{assumption}
    Suppose the $m(\cdot)$ can be wrritten as a liner combination of bases functions:
...
\begin{equation}
    ...
    \nonumber
\end{equation}
The variance of $k^\textnormal{th}$ item of $\hat{\theta}_{MLE}$, $Z_k\triangleq\frac{1}{N}\sum_{i=1}^N\phi_k(X_i)Y_i\mid X$, is
\begin{equation}
    \begin{aligned}
        \textnormal{Var}(Z_k)=
...
Suppose the class of estimators $\mathcal{L}=\{C\hat{\boldsymbol{\theta}}_{MLE}:C=\{c_1,...,c_K\},c_k\in[0,1]\}$.
\begin{equation}
    \begin{aligned}
...
\begin{equation}
    \begin{aligned}
        \sigma^*_k=\frac{\theta_k^2}{\frac{\sigma^2}{N}+\theta_k^2}
    \end{aligned}
    \nonumber
\end{equation}
...
\begin{equation}
    \begin{aligned}
        \hat{R}_\textnormal{SURE}(\mathcal{Z})=K\sigma^2+2\sigma^2\sum_{k=1}^K \frac{\partial g_k(\mathcal{Z})}{\partial \mathcal{Z}_k}+\underbrace{\sum_{k=1}^K\left(\hat{\theta}_k- \mathcal{Z}_k\right)^2}_{\|g(\hat{\theta})\|_2^2}
    \end{aligned}
    \nonumber
\end{equation}
...
\begin{proof}[for the unbiased property]
    \begin{equation}
        \begin{aligned}
            \mathbb{E}[\|\hat{\theta}-\theta\|^2]=-K\sigma^2+\underbrace{2\sigma^2\sum_{k=1}^K \frac{\textnormal{Cov}(\mathcal{Z}_k,\hat{\theta}_k)}{\sigma^2}}_{df(\hat{\theta})}+\underbrace{\mathbb{E}[\|\hat{\theta}-\mathcal{Z}\|^2]}_{\textnormal{expected sample fit}}
        \end{aligned}
        \nonumber
    \end{equation}
    where
...
\section{James and Stein Estimator}
...
\subsection*{A more general form of estimator $\mathcal{L}=\{CZ:C=\textnormal{diag}\vec{c},\vec{c}\in[0,1]^K\}$}
Consider a new estimator $\hat{\theta}\in \mathcal{L}=\{CZ:C=\textnormal{diag}\vec{c},\vec{c}\in[0,1]^K\}$. The SURE is
\begin{equation}
    \begin{aligned}
        \hat{R}_\textnormal{SURE}\left(\mathcal{Z},\vec{c}\right)=\frac{\sigma^2}{N}\sum_{k=1}^K c_k^2+\sum_{k=1}^K\left(\mathcal{Z}_k^2-\frac{\sigma^2}{N}\right)(1-c_k)^2
    \end{aligned}
    \nonumber
\end{equation}
...
Consider a prediction problem that the distribution $F_{X,Y}$ is known, we observe $X=\begin{pmatrix}
    1\\
    R
\end{pmatrix}\in \mathbb{R}^{K\times 1}$ and predict $Y$. Only linear functions of $X$ are allowed $\mathcal{L}=\{X'b:b\in \mathbb{R}^K\}$. We use square experience loss $(Y-X'b)^2$. We want to minimze Risk (mean squared error) $\mathbb{E}_{X,Y}[(Y-X'b)^2]=\int_{x,y}(y-x'b)^2f_{x,y}(x,y)dxdy$.

Let $\beta_0=\arg\min_{b\in \mathbb{R}^k}\mathbb{E}_{X,Y}[(Y-X'b)^2]$. By the F.O.C.
\begin{equation}
    \begin{aligned}
        \mathbb{E}_{X,Y}[(\underbrace{Y-X'\beta_0}_{\triangleq u})X]=0
    \end{aligned}
    \nonumber
\end{equation}
We assume $\mathbb{E}[u]=0$, then $\mathbb{E}[uR]=0=\textnormal{Cov}(u,R)$. If $u>0$, it is underpredicting and if $u<0$, it is overpredicting.
\begin{equation}
    \begin{aligned}
        \mathbb{E}[(Y-X'\beta_0)X]=0\\
...
\subsection*{Long, Short, Auxilary Regression}
$Y\in \mathbb{R}^{1}$, $X\in \mathbb{R}^{K}$, $K\in \mathbb{R}^{J}$.
\begin{proposition}[Long Regression]
    $\mathbb{E}^*[Y|X,W]=X'\beta_0+W'\gamma_0$, where $\begin{pmatrix}
        \beta_0\\
        \gamma_0
...
\begin{proposition}[Auxiliary Regression]
    $\mathbb{E}^*[W|X]=\Pi_0 X$, which is multivariate regression. For each row $j=1,...,J$, $\mathbb{E}^*[W_j|X]=X'\Pi_{j0}$, where $\Pi_{j0}=\mathbb{E}[XX']^{-1}\mathbb{E}[XW_j]$ and $\Pi_0=\begin{pmatrix}
        \Pi_{10}'\\
        \vdots\\
        \Pi_{J0}'
    \end{pmatrix}$.
...
\begin{proposition}[Short Regression]
    $\mathbb{E}^*[Y|X]=X'b_0$, where $b_0=\beta_0+\Pi_0'\gamma_0$.
\end{proposition}

\subsection*{Frisch-Waugh}
Let the variation in $W$ unexplained by $X$.
\begin{equation}
    \begin{aligned}
        \underbrace{V}_{J\times 1}&=\underbrace{W}_{J\times 1}-\underbrace{\mathbb{E}^*[W|X]}_{J\times 1}\\
        &=W-\Pi_0X
    \end{aligned}
    \nonumber
\end{equation}
