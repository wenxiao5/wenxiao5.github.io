...
\section{Inference}
Suppose we want test $H_0: \theta_0=\Gamma(\beta_0)\neq\hat{\theta}=\Gamma(\hat{\beta})$.
\begin{theorem}
    By using the asymptotic variance of GMM, $V_{{P_0}}$,
    $$m\overbrace{(\hat{\theta}-\theta)^T\underbrace{\left(R(\beta_0)^TV_{P_0}R(\beta_0)\right)^{-1}}_{\triangleq\Omega}(\hat{\theta}-\theta)}^\mathcal{W} \Rightarrow \chi^2_l$$
...
\begin{proof}
    ...
    \begin{equation}
        \begin{aligned}
            \hat{\theta}-\theta_0=\Gamma(\hat{\beta})-\Gamma(\beta_0)=\frac{d\Gamma(\beta_0)}{d\beta}(\hat{\beta}-\beta_0)+o_{P_0}(m^{-\frac{1}{2}})\\
