...
\begin{enumerate}
    \item If $f: \mathbb{R}^n \rightarrow \mathbb{R}$ with $n$ large, then computing $H f(\mathbf{x})$ requires finding many derivatives, which is expensive even when we can take partial derivatives of $f$ easily.
...
Now let's pass to the $n$-dimensional case: finding the solution to a system of equations $\vec{g}(\mathbf{x})=\mathbf{0}$ for a function $\vec{g}: \mathbb{R}^n \rightarrow \mathbb{R}^n$. To approximate Newton's method, we need an estimate of the Jacobian $\nabla \vec{g}$; in other words, a linear approximation of $\vec{g}$.
