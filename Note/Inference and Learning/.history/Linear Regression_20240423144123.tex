...
$\sqrt{n}\left(\hat{\beta}_\textnormal{NLS}-\beta_0\right) \Rightarrow N\left(0,G^{-1}\Omega G^{-1}\right)$, where $g(x_i,\beta)=-(Y-exp\{x_i^T\beta\})exp\{x_i^T\beta\}x_i$, $G=\mathbb{E}\left[\frac{\partial g(x_i,\beta_0)}{\partial \beta}\right]=\mathbb{E}[exp\{2x_i^T\beta_0\}x_ix_i^T]$ and $\Omega=\textnormal{Var}(g(x_i,\beta_0))$
