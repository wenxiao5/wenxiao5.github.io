...
\begin{example}
    Consider the decision rule "Reject $H_0$ iff $\frac{\sum_{i=1}^n X_i}{n}=\bar{X}\geq \frac{1.645}{\sqrt{n}}$", where the critical region is $C=\{(X_1,...,X_n):\frac{\sum_{i=1}^n X_i}{n}\geq \frac{1.645}{\sqrt{n}}\}$. The power function is $\beta(\mu)=P_\mu[\textnormal{Reject $H_0$}]=P_\mu(\bar{X}>\frac{1.645}{\sqrt{n}})$
\end{example}
