...
\begin{theorem}[Asymptotic Normality of M-estimators]\label{thm:asymptotic_normality_of_m_estimators}
    ...
    \begin{enumerate}
        ...
        \item[A5.] for any sequence $\tilde{\theta}_n \stackrel{P}{\longrightarrow} \theta_0$, $\hat{G}(\tilde{\theta}_n):=\frac{\partial^2}{\partial \theta\partial \theta^T}\hat{Q}_n(\tilde{\theta}_n) \stackrel{P}{\longrightarrow} B_0$ for some non-stochastic $d\times d$ matrix (usually we check $G_0$)
    \end{enumerate}
    Then,
    \begin{equation}
        \begin{aligned}
