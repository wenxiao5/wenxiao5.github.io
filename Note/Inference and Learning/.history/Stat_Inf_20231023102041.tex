...
Given $N$ random sample $\{Z_1,...,Z_N\}$ from a Bernoulli distribution with parameter $\theta$ and the sum $\sum_{i=1}^N Z_i=S$. Based on Example \ref{ex:square_loss}, with prior $\theta\sim \textnormal{Beta}(1,1)$ (uniform distribution), $\theta\mid \{Z_1,...,Z_N\}\sim \textnormal{Beta}(S+1,N-S+1)$.
