...
\begin{definition}[Risk]
    \normalfont
    \begin{enumerate}[$\circ$]
        \item \textbf{Risk of decision rule $\delta$ on $\theta$:} $$R(\theta,\delta)=\mathbb{E}_{X\sim \pi(\theta)}\left(l(\delta(X),\theta)\right)$$ where $X$ are random with prob $\pi(\theta)$.
        \item \textbf{Risk of decision rule $\delta$:}
        \begin{equation}
            \begin{aligned}
                R(\delta)&=\mathbb{E}_{\theta\sim P_\theta}[R(\theta,\delta)]\\
...
Given $N$ random sample $\{Z_1,...,Z_N\}$ from a Bernoulli distribution with parameter $\theta$ and the sum $\sum_{i=1}^N Z_i=S$.
