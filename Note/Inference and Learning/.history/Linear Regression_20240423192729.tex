...
$\sqrt{n}\left(\hat{\beta}_\textnormal{NLS}-\beta_0\right) \Rightarrow N\left(0,G^{-1}\Omega G^{-1}\right)$, where $g(z_i,\beta)=-(y_i-\log(x_i^T\beta_0))\frac{1}{x_i^T\beta}x_i$, $G=\mathbb{E}\left[\frac{\partial g(x_i,\beta_0)}{\partial \beta}\right]=\mathbb{E}[exp\{2x_i^T\beta_0\}x_ix_i^T]$ and $\Omega=\textnormal{Var}(g(z_i,\beta_0))=\mathbb{E}[(y_i-exp\{x_i^T\beta\})^2exp\{2x_i^T\beta\}x_ix_i^T]=\mathbb{E}[\sigma^2(x_i)exp\{2x_i^T\beta\}x_ix_i^T]$.
