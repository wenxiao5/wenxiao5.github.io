...
\subsection*{A more general form of estimator $\mathcal{L}=\{C \mathbf{Z}:C=\textnormal{diag}\vec{c},\vec{c}\in[0,1]^K\}$}
Consider a new estimator $\hat{\theta}\in \mathcal{L}=\{C\hat{\boldsymbol{\theta}}_{MLE}:C=\textnormal{diag}\vec{c},\vec{c}\in[0,1]^K\}$. The SURE is
\begin{equation}
    \begin{aligned}
