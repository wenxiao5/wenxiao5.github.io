...
\begin{definition}["Regularity" Condition]
    \normalfont
    "Regularity" condition: If $X$ are continuous, the mean of score function is zero:
    \begin{equation}
        \begin{aligned}
            \mathbb{E}_\theta\left[\frac{\partial}{\partial \theta}\log f(\vec{X}\mid\theta)\right]&=\int_{-\infty}^\infty \left[\frac{\partial}{\partial \theta}\log f(\vec{X}\mid\theta)\right]f(\vec{X}\mid\theta) d\vec{X}\\
            &=\int_{-\infty}^\infty \left[\frac{\partial}{\partial \theta}f(\vec{X}\mid\theta)\right]d\vec{X}\\
            (*)\quad &=\frac{\partial}{\partial \theta}\underbrace{\int_{-\infty}^\infty f(\vec{X}\mid\theta)dx}_{=1}=0
        \end{aligned}
        \nonumber
    \end{equation}
