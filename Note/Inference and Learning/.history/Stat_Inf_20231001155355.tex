...
\begin{theorem}[MLE $\stackrel{n \rightarrow \infty}{\longrightarrow}$ Cramér-Rao Lower Bound]
    An estimator $\hat{\theta}$ whose variance achieves the Cramér-Rao lower bound, $\textnormal{Var}_\theta[\hat{\theta}(\vec{X})]=\textnormal{CRLB}(\theta)$, \underline{if and only if}, its score function can be written as
    \begin{equation}
        \begin{aligned}
            u(\theta,\vec{X})=\frac{\partial}{\partial \theta}\log f_{\vec{X}}(\vec{X}\mid\theta)
        \end{aligned}
        \nonumber
    \end{equation}
\end{theorem}
