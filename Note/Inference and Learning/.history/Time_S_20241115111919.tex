...
\subsection{Strictly Stationary and Covariance Stationary}
The definition of strict stationary and covariance stationary are the same for both scalar and vector time series.
...
\begin{lemma}[Property of Strictly Stationary]
    If $\{Y_t:t\in \mathbb{Z}\}$ is strictly stationary with $\mathbb{E}[Y_{t,i}^2]<\infty$ ($\forall t,i$), then
    \begin{enumerate}
        \item Same Expectation:
        \begin{equation}
            \begin{aligned}
                \mathbb{E}[Y_t]=\mu,\ \forall t \textnormal{ (for some $\mu\in \mathbb{R}^{n\times 1}$)}
            \end{aligned}
            \label{s}
            \tag{*}
        \end{equation}
