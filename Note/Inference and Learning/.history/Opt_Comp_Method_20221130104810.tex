...
\begin{theorem}[Wolfe's theorem]
    Let $\alpha,\beta$ be real numbers satisfying $0 <\alpha < \beta < 1$. Assume $f : \mathbb{R}^n \rightarrow \mathbb{R}$ has continuous $\nabla f$ and is \underline{bounded below}. Starting from a point $x^{(k)}$, whenever $p^{(k)}$ \underline{satisfies Criterion 2} (it is a descent direction), then there
    is a range $[a_k, b_k]$ (with $0 < a_k < b_k$) such that for any $t_k \in [a_k, b_k]$, all four criteria are satisfied.
\end{theorem}
