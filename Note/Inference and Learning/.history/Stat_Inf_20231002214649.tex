...
\begin{example}\quad
    \begin{enumerate}
        \item \textbf{Order statistics sampled from a uniform distribution:} Consider a random sample $U_1,...,U_n$ from the standard uniform distribution. Then,
        \begin{equation}
            \begin{aligned}
        ...
        \item \textbf{The joint distribution of the order statistics of the uniform distribution:}
...
\begin{example}\quad
    ...
    \begin{enumerate}[$\bullet$]
        ...
        Any unbiased estimator given $X_{(n)}$ is UMVU, e.g., $\hat{\theta}_{RB}=\mathbb{E}[\hat{\theta}_{MM}|X_{(n)}]$ is UMVU;
        \item Because $\hat{\theta}_{RB}$ is UMVU and $\mathbb{E}_\theta(X_{(n)})=\frac{n}{n+1}\theta$ $\Rightarrow$ $\hat{\theta}_{RB}=\frac{n+1}{n}X_{(n)}$.
    \end{enumerate}
    \begin{remark}
        The cdf of $X_{(n)}$ is
        \begin{equation}
...
\begin{theorem}[Sufficient and Complete Statistic for Exponential Family]\label{comp_exp_fam}
    If the distribution is a member of the exponential family, that is, $$f(x|\theta)=c(\theta)h(x)e^{\sum_{j=1}^k w_j(\theta) t_j(x)}$$ then $T=(\sum_{i=1}^n t_1(x_i),...,\sum_{i=1}^n t_k(x_i))$ is sufficient and complete if $\{\{w_1(\theta),...,w_k(\theta)\}:\theta\in\Theta\}$ contains a non-empty set.
\end{theorem}
\begin{example}
...
\begin{lemma}[Fisher Information]
    ...
    \begin{equation}
        \begin{aligned}
            ...
            &=\mathbb{E}_\theta\left[\left(\frac{\partial}{\partial \theta}\log f_{\vec{X}}(\vec{X}\mid\theta)\right)^2\right]=n\mathbb{E}_\theta\left[\left(\frac{\partial}{\partial \theta}\log f_{X_i}(X_i\mid\theta)\right)^2\right]\\
            &=-\mathbb{E}_\theta\left[\frac{\partial^2}{\partial \theta^2}\log f_{\vec{X}}(\vec{X}\mid\theta)\right]
        \end{aligned}
        \nonumber
    \end{equation}
\end{lemma}
...
\begin{definition}[Power Function]
    \normalfont
    The \textbf{power function} of a test unit critical region $C\subseteq \mathbb{R}^n$ is the function $\beta:\Theta \rightarrow [0,1]$ given by $\beta(\theta)=P_\theta[\textnormal{Reject $H_0$}]=P_\theta[(X_1,...,X_n)'\in C]$.
\end{definition}
\begin{enumerate}[$\circ$]
...
\begin{enumerate}[(1).]
    \item Choose critical value such that $\beta(\theta)\leq 5\%$ when $\theta\in\Theta_0$;
    \item Choose test statistic such that $\beta(\theta)$ is "maximized" (among tests with $\beta(\theta)\leq 5\%$ when $\theta\in\Theta_0$).
\end{enumerate}


\subsection{Choice of Critical Value: Test Size and Level $\alpha$}
...
\begin{example}
    Suppose $X\sim \textnormal{i.i.d. } N(\mu,1)$, where $\mu\geq 0$ is unknown. Consider the decision rule "Reject $H_0$ iff $\frac{\sum_{i=1}^n X_i}{n}=\bar{X}\geq \frac{1.645}{\sqrt{n}}$". The power function is $\beta(\mu)=P_\mu[\textnormal{Reject $H_0$}]=P_\mu(\bar{X}\geq\frac{1.645}{\sqrt{n}})$\\
...
\subsection{Likelihood Ratio Test: Neyman-Pearson Lemma}
\begin{theorem}[Neyman-Pearson Lemma]
    ...
    \begin{equation}
        ...
        \nonumber
    \end{equation}
    For any $k\geq 0$, the test which rejects $H_0$ iff
    \begin{equation}
        \begin{aligned}
            L(\theta_1\mid X_1,...,X_n)\geq k L(\theta_0\mid X_1,...,X_n)
        \end{aligned}
        \nonumber
    \end{equation}
    ...
    \begin{equation}
        \begin{aligned}
            L(\theta\mid X_1,...,X_n)=\prod_{i=1}^n f(X_i\mid\theta)
        \end{aligned}
        \nonumber
    \end{equation}
\end{theorem}
