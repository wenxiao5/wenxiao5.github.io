...
Reminds that Fixed Effect model has assumption $\mathbb{E}[\vec{\dot{X}}_i \vec{\dot{\epsilon}}_i]=0$, which can be given by ``strict exogeneity'': $\mathbb{E}[X_{i_s}\epsilon_{i_t}]=0m \forall s,t$ ($\epsilon$ is uncorrelated with past, present, and future $X$'s).
