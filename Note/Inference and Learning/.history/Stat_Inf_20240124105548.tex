...
\begin{equation}
    ...
    \nonumber
\end{equation}
Here, the data $\{Z_i\}_i$ is generated from $P_0$, which forms $P_m$.\\ Sometimes, we may study $v(P_0)(t)=\text{Pr}_{P_0}\left(\underbrace{\frac{C(P_m,P_0)}{\text{Var}_{P_0}(\theta(P_m))}}_{T(P_m,P_0)}\leq t\right),\forall t$ instead.
...
\begin{example}
    ...
    \begin{equation}
        ...
        \nonumber
    \end{equation}
    where $Z_i^*\sim_{i.i.d.} P_m$, $Z_i^*\in\{Z_1,...,Z_m\},\forall i\in\{1,...,m\}$. It is equivalent to give a weight $W_{i,m}\sim\{0,1...,m\}$ to each $Z_i$, $\sum_{i=1}^m Z_i^*=\sum_{i=1}^m W_{i,m} Z_i^*$.
\end{example}
