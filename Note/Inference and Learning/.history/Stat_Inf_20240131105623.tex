...
\begin{example}
    \begin{enumerate}
        \item $y=\beta x^*+e$, where $\mathbb{E}[x^*e]=0$. However, we don't have $x^*$ and we only have a noisy variable $x=x^*+v$. Then, $y=\beta(x-v)+e=\beta x+\epsilon$, where $\epsilon:=e-\beta v$. The OLS estimator is
        \begin{equation}
            \begin{aligned}
                \hat{\beta}_{OLS}=\frac{\mathbb{E}_{P_0}[x\epsilon]}{\mathbb{E}_{P_0}[x^2]}
            \end{aligned}
            \nonumber
        \end{equation}
    \end{enumerate}
