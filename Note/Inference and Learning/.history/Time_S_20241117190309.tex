...
\subsection{OLS Estimator is MLE}
...
\begin{proof}
    ...
    \begin{equation}
        \begin{aligned}
            \hat{\phi}_{OLS}=\frac{\sum_{t=2}^TY_{t-1}Y_t}{\sum_{t=2}^T Y_{t-1}^2}=\phi+\frac{\sum_{t=2}^TY_{t-1}\epsilon_t}{\sum_{i=2}^T Y_{i-1}^2},
        \end{aligned}
        \nonumber
    \end{equation}
    where $\epsilon_t=Y_t-\phi Y_{t-1}, t\geq 2$. For every $t$, $\epsilon_t$ is independent of $Y_{t-1}$ but is \underline{not} independent of $\sum_{i=2}^T Y_{i-1}^2$. If $\phi$ is positive, then a positive shock to $\epsilon_t$ raises all $Y_i$ with $i\geq t$.
\end{proof}
