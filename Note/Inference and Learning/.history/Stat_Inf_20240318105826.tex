...
\end{equation}
\begin{example}
    Suppose the link function is $F(t)=\frac{e^t}{1+e^t}$. $F'(t)=\frac{e^t(1+e^t)-e^t\cdot e^t}{(1+e^t)^2}=\frac{e^t}{1+e^t}\cdot\frac{1}{1+e^t}=F(t)\cdot(1-F(t))$. In this case, $w(X_i,\beta)=X_i$ doesn't depend on $\beta$.\\
    Therefore, if $F$ is logit,
    \begin{equation}
        \begin{aligned}
            \sum_{i=1}^n X_i\cdot (Y_i-F(X_i^T\hat{\beta}_{MLE}))=0^d
        \end{aligned}
        \nonumber
    \end{equation}
\end{example}
