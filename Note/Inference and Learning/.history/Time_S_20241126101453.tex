...
\paragraph*{Unit Root Asymptotics:} If $\rho=1$, then
\begin{equation}
    \begin{aligned}
        Y_T(r)&=\frac{y_{\left\lfloor Tr\right\rfloor}}{\sqrt{T}}=\frac{1}{\sqrt{T}}\sum_{s=1}^{\left\lfloor Tr\right\rfloor}\epsilon_s \stackrel{d}{\longrightarrow} \sigma W(r)
    \end{aligned}
    \nonumber
\end{equation}
If $\rho=1$, then
\begin{equation}
    \begin{aligned}
        \sum_{t=1}^T y_{t-1}\epsilon_t=\frac{1}{2}\left(y_T^2-\sum_{t=1}^T\epsilon_t^2\right)
    \end{aligned}
    \nonumber
\end{equation}
where $\frac{1}{T}\sum_{t=1}^T\epsilon_t^2 \stackrel{P}{\longrightarrow} \sigma^2$ by LLN and $\frac{1}{T}y_T^2=\left(\frac{y_T}{\sqrt{T}}\right)^2=Y_T(1)^2\stackrel{d}{\longrightarrow} [\sigma W(r)]^2=\sigma^2W(1)^2$
