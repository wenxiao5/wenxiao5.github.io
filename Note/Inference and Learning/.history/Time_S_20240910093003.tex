...
\begin{definition}[Moving Average Process]
    \begin{enumerate}
        ...
        \begin{claim}
            $\{Y_t\}$ is covariance stationary with nontrivial dynamics: $\mathbb{E}[Y_t]=\mu$ and its autocovariance function is
            \begin{equation}
                \begin{aligned}
                    \rho(j)=\textnormal{Cov}(Y_t,Y_{t-j})=\left\{\begin{matrix}
            ...
            \end{equation}
        \end{claim}
        \item $q^{th}$-order moving average process: $Y_t\sim \textnormal{MA}(q)$ (for some $q\in \mathbb{N}$) iff
        \begin{equation}
            \begin{aligned}
