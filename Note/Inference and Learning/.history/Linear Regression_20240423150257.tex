...
$f(y;x_i^T\beta_0)=(1-exp\{x_i^T\beta_0\})exp\{y\cdot (x_i^T\beta_0)\}$, where $x_i^T\beta<0$ with probability one for all possible values of $\beta$; but the distribution of $x_i$ does not otherwise involve $\beta$. We have $\mathbb{E}[y\mid x_i^T\beta_0]=\frac{exp\{x_i^T\beta_0\}}{1-exp\{x_i^T\beta_0\}}$\\
