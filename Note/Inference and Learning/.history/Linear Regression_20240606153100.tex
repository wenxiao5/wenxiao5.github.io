...
\begin{enumerate}
    ...
    \begin{remark}
        $\hat{p}_{ML}=\bar{X}=\hat{p}_\text{MM}$ is the 'best' estimator of $p$.
    \end{remark}
    ...
    \end{equation}
    \begin{remark}
        $\hat{\theta}_{ML}=X_{(n)}\neq 2\bar{X}=\hat{\theta}_\text{MM}$; $\hat{\theta}_{ML}<X_i$ can't occur, which is good news; $\hat{\theta}_{ML}\leq \theta$ (low) must occur, which is bad news.
    \end{remark}
    \item Suppose $X_i\sim \text{i.i.d.}N(\mu,\sigma^2)$ where $\mu\in \mathbb{R}$ and $\sigma^2>0$ are unknown. Then,
    \begin{equation}
        \begin{aligned}
            \hat{\mu}_{ML}=\hat{\mu}_\text{MM}=\bar{X},\ \hat{\sigma}^2_{ML}=\hat{\sigma}^2_\text{MM}=\frac{1}{n}\sum_{i=1}^n (X_i-\bar{X})^2
        \end{aligned}
        \nonumber
    \end{equation}
\end{enumerate}
...
\begin{enumerate}[(I).]
    \item Recall
    \begin{enumerate}[(i).]
        \item $\bar{X}=\hat{\mu}_\text{MM}=\hat{\mu}_{ML}=\hat{\mu}_{UMVU}$;
