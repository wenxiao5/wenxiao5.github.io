...
\section{Linear GMM}
\begin{assumption}
    Given the true probability of data $P_0$, there exists a unique parameter $\beta$ such that
    \begin{equation}
        \begin{aligned}
...
\begin{definition}[Linear GMM]
    ...
    \begin{equation}
        \begin{aligned}
            \mathbb{E}_{P_0}[\underbrace{Z}_{l\times 1}(\underbrace{Y}_{1\times 1}-\beta^T\underbrace{X}_{k\times 1})]=0
        \end{aligned}
        \nonumber
    \end{equation}
    with $\textnormal{Rank}\left(\mathbb{E}_{P_0}[ZX^T]\right)=k$.
\end{definition}
