...
\paragraph*{B.} Derive an expression for the asymptotic distribution of the ML estimator $\hat{\beta}$, including an expression for its asymptotic covariance matrix.
\paragraph*{Answer of B:} The Hessian of the average log-likelihood is
\begin{equation}
    \begin{aligned}
...
\paragraph*{C.} Suppose that the first component of $x_i$ is one and you want to test the null hypothesis that the intercept of $\beta_0$ is one and the slope coefficient are all zero, i.e.,
\begin{equation}
    \begin{aligned}
        H_0:\beta_0=(1,0,...,0)^T
    \end{aligned}
    \nonumber
\end{equation}
which implies that $x_i^T\beta_0=1$ (w.p.1) and that $y_i$ has a $\textnormal{Unif}(0,1)$ distribution, independent of $x_i$. Derive the form of the Lagrange multiplier or ``Score'' (LM) test statistic for this null hypothesis, and state the form of its asymptotic distribution under $H_0$.%Be as explicit as possible about the form of the estimator, and be sure to use the ``conditional information matrix'' - that is, the sample average of the conditional expectation of the (negative) Hessian, conditional on the regressors, evaluated at the restricted ML - in construction of your statistic.
\paragraph*{Answer of C:} Under $H_0$, the regression function $x_i^T\beta_0=1$,
\begin{equation}
    \begin{aligned}
...
\paragraph*{D.} For general $K\geq 1$, we can use NLLS instead of MLE based on the known parametric form of $\mathbb{E}[y_i\mid x_i]$: $\mathbb{E}[y_i\mid x_i]=\frac{x_i^T\beta_0}{x_i^T\beta_0+1}$.\\
...
\begin{equation}
    \begin{aligned}
        ...
        &=\mathbb{E}_X\left[\textnormal{Var}_{Y|X}\left[-\left(Y-\frac{X^T\beta}{X^T\beta+1}\right)\frac{X}{(X^T\beta+1)^2}\right]\right]+0\\
        &=\mathbb{E}_X\left[\frac{X^T\beta}{X^T\beta+2}\frac{XX^T}{(X^T\beta+1)^4}\right]
    \end{aligned}
    \nonumber
\end{equation}
where $\textnormal{Var}_{Y|X}[Y|X]=\frac{X^T\beta}{X^T\beta+2}$.
