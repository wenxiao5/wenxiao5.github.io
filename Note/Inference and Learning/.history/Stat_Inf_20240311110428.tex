...
\section{}
\paragraph*{Extremum estimation/identification}
...
Suppose $\theta_0\in\argmin_{\theta\in\Theta} Q(\theta)$, where $Q: \Theta \rightarrow \mathbb{R}$ is objective function (unknown) and $\hat{Q}: \Theta \rightarrow \mathbb{R}$ is a sample version. $\hat{\theta}$ is called \textbf{M-estimator} of $\theta$ if $\hat{\theta}\in\argmin_{\theta\in\Theta} \hat{Q}(\theta)$
