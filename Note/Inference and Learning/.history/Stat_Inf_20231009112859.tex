...
\end{proposition}
Hence, the covariance matrix can be estimated as $(\mathcal{I}(\hat{\theta}))^{-1}$, where $\mathcal{I}(\hat{\theta})$ is the (sample) information matrix given by $$\mathcal{I}(\hat{\theta})=-\frac{\partial^2}{\partial \theta^2}\log f_{\vec{X}}\left(X_1,...,X_n\mid\hat{\theta}\right)$$
Similarly, \textit{SE} is estimated by $\sqrt{(\mathcal{I}(\hat{\theta}))^{-1}}$.
