...
\begin{enumerate}
    ...
    \begin{equation}
        \begin{aligned}
            \vec{g}(\vec{x}^{(k)})+D_{k+1}(\vec{x}^{(k+1)}-\vec{x}^{(k)})=\vec{0}
        \end{aligned}
        \tag{2}
    \end{equation}
    \item We should make the same predictions in every direction orthogonal to $\vec{x}^{(k+1)}-\vec{x}^{(k)}$. Because we didn't go in those directions, we didn't learn anyting new.
\end{enumerate}
