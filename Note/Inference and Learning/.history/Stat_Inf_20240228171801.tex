...
\textbf{Potential Outcome Model}

$Z:=(Y,D,X)$, where $Y$ is the outcome, $D\in\{0,1\}$ is the treatment, and $X$ is the covariates.
\begin{enumerate}[$\circ$]
...
\begin{example}
    \begin{enumerate}
        ...
        \item \underline{Linear Instrumental Variable Model}: $\mathbb{E}_P[(Y-\beta^T_0X)|W]=0$, where $W$ is the instrumental variable. Look at $\mathbb{E}_{P_0}[(Y-\beta^TX)W]=0$. Suppose $\hat{\beta}\neq\beta_0$,
        \begin{equation}
            \begin{aligned}
            ...
            \nonumber
        \end{equation}
        $\mathbb{E}_{P_0}[(\hat{\beta}-\beta_0)^TXW]=\underbrace{(\hat{\beta}-\beta_0)^T}_{1\times m}\underbrace{\mathbb{E}_{P_0}[XW]}_{m\times k}=0$, which holds iff $\hat{\beta}=\beta_0$ given $\mathbb{E}_{P_0}[XW]$ has full rank.
    \end{enumerate}
\end{example}
...
\begin{equation}
    ...
    \nonumber
\end{equation}
Here, the data $\{Z_i\}_i$ is generated from $P_0$, which forms $P_m$.\\ Sometimes, we may study $v_A(P_0)(t)=\textnormal{Pr}_{P_0}\left(\underbrace{\frac{C(P_m,P_0)}{\sqrt{\text{Var}_{P_0}(\theta(P_m))}}}_{T(P_m,P_0)}\leq t\right),\forall t$ instead.
...
\begin{equation}
    \begin{aligned}
        v(P_m)=\textnormal{Pr}_{P_m}\left(\theta(P_m^*)-\theta(P_m)\leq t\right)
    \end{aligned}
    \nonumber
\end{equation}
...
\begin{example}
    $\theta(P_0)=\mathbb{E}_{P_0}(Z)$, $\theta(P_m)=\bar{Z}_m=\frac{1}{m}\sum_{i=1}^m Z_i$. $v(P_0)(t)=\textnormal{Pr}_{P_0}\left(\frac{1}{m}\sum_{i=1}^m (Z_i-\mathbb{E}_{P_0}(Z))\leq t\right)$. The Bootstrap estimator is given by
    \begin{equation}
        \begin{aligned}
            v(P_m)=\textnormal{Pr}_{P_0}\left(\frac{1}{m}\sum_{i=1}^m (Z_i^*-\underbrace{\mathbb{E}_{P_m}(Z)}_{\bar{Z}_m})\leq t\right)
        \end{aligned}
        \nonumber
    \end{equation}
    or
    \begin{equation}
        \begin{aligned}
            v_A(P_m)=\textnormal{Pr}_{P_0}\left(\sqrt{m}\frac{\frac{1}{m}\sum_{i=1}^m (Z_i^*-\underbrace{\mathbb{E}_{P_m}(Z)}_{\bar{Z}_m})}{\sqrt{\text{Var}_{P_0}(\theta(P_m))}}\leq t\right)
        \end{aligned}
        \nonumber
    \end{equation}
    ...
    \begin{equation}
        \begin{aligned}
            v(P_m)=\textnormal{Pr}_{P_0}\left(\frac{1}{m}\sum_{i=1}^m (W_{i,m}-1)Z_i\leq t\right)
        \end{aligned}
        \nonumber
    \end{equation}
    ...
    \begin{equation}
        \begin{aligned}
            v_A(P_m)=\textnormal{Pr}_{P_0}\left(\sqrt{m}\frac{\bar{Z}_m-\mathbb{E}_{P_0}[Z]}{S_Z}\leq t\right)
        \end{aligned}
        \nonumber
    \end{equation}
\end{example}
...
\paragraph*{Sub-Sampling/$k$-out-of $m$ Bootstrap}
...
\begin{assumption}
    We assume $\mathbb{E}[\vec{X}^T Q\vec{\epsilon}]=0$, which is equivalent to $\mathbb{E}[\vec{\dot{X}}_i \vec{\dot{\epsilon}}_i]=0$.
\end{assumption}
...
\begin{assumption}
    $\textnormal{Cov}(\alpha_i X_{i_t})=0$
\end{assumption}
\begin{equation}
    \begin{aligned}
...
\begin{equation}
    \begin{aligned}
        ...
        (\textnormal{assuming homosecticity and Cov}(\epsilon_s,\epsilon_t)=0)\quad &=\sigma_\alpha^2 \mathbf{1}\mathbf{1}^T+\sigma_\epsilon^2 I\\
        &:=\Omega
    \end{aligned}
    \nonumber
\end{equation}
...
\begin{enumerate}
    ...
    \item $Z$: the intended treatment of an agent.
\end{enumerate}
...
\begin{enumerate}
    \item Never Treated: $X(0)=X(1)=0$.
    \item Always Treated: $X(0)=X(1)=1$.
    \item Complies: $X(0)=0,X(1)=1$.
    \item Defiers: $X(0)=1,X(1)=0$.
\end{enumerate}
