...
\begin{example}
    Consider $$Y=X^T\gamma_0+\epsilon$$, where $\epsilon$ is independent of $X$ (not $\mathbb{E}[\epsilon|X]=0$, which is too weak). This assumption gives
    \begin{equation}
        \begin{aligned}
            Q_{\epsilon|X}(\tau|X)=Q_\epsilon(\tau)
        \end{aligned}
        \nonumber
    \end{equation}
\end{example}
