...
Let $\hat{\Lambda}=\hat{\Gamma}^{-1}\hat{\Omega}\hat{\Gamma}^{-1}$, $\sqrt{N}(\hat{\beta}-\beta_0) \xrightarrow{D} N\left(0,\hat{\Lambda}\right)$. Hence,
\begin{equation}
    \begin{aligned}
        T_N\triangleq \hat{\Lambda}^{-\frac{1}{2}}\left(\hat{\beta}-\beta_0\right)\xrightarrow{D} N(0,1)
    \end{aligned}
    \nonumber
\end{equation}

Consider the event $A=\mathbf{1}\left\{|T_N|\leq 1.96\right\}$
