...
\begin{example}
    Let's consider a linear regression model, $\mathbb{E}_{P_0}[Y|X]=\beta^T_0 X$. We solve $\beta$ by $\min_\beta \mathbb{E}_{P_0}[(y-\beta^Tx)^2]$. The F.O.C. gives $\mathbb{E}_{P_0}[YX^T]=\beta^T \mathbb{E}_{P_0}[XX^T]$.
\end{example}
