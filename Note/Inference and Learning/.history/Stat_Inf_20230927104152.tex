...
\begin{equation}
    ...
    \nonumber
\end{equation}
for everty unbiased estimator $\hat{\theta}$, where $\mathcal{I}(\theta)=n \mathbb{E}_\theta[\left(\frac{\partial}{\partial \theta}\log f(x|\theta)\right)^2]$
\end{proposition}
\begin{remark}
...
\begin{enumerate}[(1).]
    ...
    \begin{enumerate}
        \item Estimating $\sigma^2$ when $X\sim \textnormal{i.i.d. } N(\mu,\sigma^2)$: $\textnormal{Var}(S^2)=\frac{2\sigma^4}{n-1}$
    \end{enumerate}
\end{enumerate}
