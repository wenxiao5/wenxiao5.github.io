...
\begin{theorem}[The Sherman-Morrison formula]
...
\begin{enumerate}
    ...
    \item In terms of the vectors $\vec{b}_k=\vec{x}^{(k+1)}-\vec{x}^{(k)}=-D_k^{-1}\vec{g}(\vec{x}^{(k)})$ and $\vec{c}_k=D_k^{-1}\vec{g}(\vec{x}^{(k+1)})$, set
    \begin{equation}
        \begin{aligned}
            D_{k+1}^{-1}=D_{k}^{-1}-\frac{\vec{c}_k\left((\vec{b}_k)^T D_k^{-1}\right)}{\vec{b}_k\cdot (\vec{b}_k+\vec{c}_k)}
        \end{aligned}
        \nonumber
    \end{equation}
\end{enumerate}
