...
\begin{equation}
    ...
    \nonumber
\end{equation}
We can find the second information equality doesn't hold, $G^{-1}\Omega G^{-1}\neq G^{-1}$.


\subsection{Efficient NLS}
In binary case, $m(Z\mid\beta)=\frac{1}{2}(Y-\rho(B(X)^T\beta))^2$ is the simplest criterion but $G\neq \Omega \Rightarrow$ NLS may not be efficient. The inefficiency can be fixed by
\begin{equation}
    \begin{aligned}
...
\begin{enumerate}
    ...
    \item Estimate $\hat{\beta}_\textnormal{2SNLS}$ by
    \begin{equation}
        \begin{aligned}
            \hat{\beta}_\textnormal{2SNLS}=\argmin \sum_{i=1}^n \frac{(Y-\rho(B(X)^T\beta))^2}{\rho(B(X)^T\beta)(1-\rho(B(X)^T\beta))}
        \end{aligned}
        \nonumber
    \end{equation}
\end{enumerate}
