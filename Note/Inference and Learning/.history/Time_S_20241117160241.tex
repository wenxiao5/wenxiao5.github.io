...
\begin{proof}
    ...
    \begin{equation}
        \begin{aligned}
            \textnormal{Var}(\bar{Y})&=\textnormal{Cov}\left(\frac{1}{T}\sum_{t}Y_t, \frac{1}{T}\sum_{s}Y_s\right)=\frac{1}{T^2}\sum_{t}\sum_{s}\textnormal{Cov}\left(Y_t,Y_s\right)\\
            &=\frac{1}{T^2}\sum_{t}\sum_{s}\gamma(t-s)=\frac{1}{T^2}\sum_{j=1-T}^{T-1}(T-|j|)\gamma(j)=\frac{1}{T}\sum_{j=1-T}^{T-1}(1-\frac{|j|}{T})\gamma(j)\\
            &\leq \frac{1}{T}\sum_{j=1-T}^{T-1} |\gamma(j)|\leq\frac{1}{T}\sum_{j=-\infty}^\infty |\gamma(j)|
        \end{aligned}
        \nonumber
    \end{equation}
