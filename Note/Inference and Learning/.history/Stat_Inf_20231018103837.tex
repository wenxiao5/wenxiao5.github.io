...
\begin{example}[ (Square Loss)]
    ...
    \begin{equation}
        \begin{aligned}
            R(t,Z)&=\mathbb{E}_\theta[(t-Z)^2]\\
            &=(t-\theta)^2+\theta(1-\theta)
        \end{aligned}
        \nonumber
    \end{equation}
    Then, the optimal estimation is $t^*=\theta$. And we say the minimial risk is \textbf{oracle risk} $$R(t^*,Z)=\theta(1-\theta)$$\\
    However, we don't have $\theta$. What if we use the MLE $\hat{\theta}_\textnormal{MLE}=\bar{Z}=\frac{1}{n}\sum_{i=1}{n}Z_i$ instead?
    \begin{equation}
        \begin{aligned}
            R(\bar{Z},Z)&=\mathbb{E}_\theta[(\bar{Z}-Z)^2]\\
            &=\mathbb{E}[(\bar{Z}-\theta)^2]+\theta(1-\theta)\\
            &=\frac{1+n}{n}\theta(1-\theta)
        \end{aligned}
        \nonumber
    \end{equation}
\end{example}
