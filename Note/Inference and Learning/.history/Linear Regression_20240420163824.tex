...
\begin{equation}
    \begin{aligned}
        \frac{\partial^2 L_N(\hat{\beta})}{\partial \beta\partial \beta^T}=-\frac{1}{N}\sum_{i=1}^N \left(\frac{1}{x_i^T\beta}\right)^2 x_ix_i^T
    \end{aligned}
    \nonumber
\end{equation}
The second information equality holds in MLE, $I_0=-\mathbb{E}\left[\left(\frac{1}{x_i^T\beta}\right)^2 x_ix_i^T\right]$. $\sqrt{N}(\hat{\beta}_\textnormal{ML}-\beta_0) \stackrel{a}{\longrightarrow} N(0,I_0^{-1})$.
