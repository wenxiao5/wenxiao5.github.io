...
\begin{enumerate}[$\bullet$]
    \item \textbf{\underline{Input:}} $W=[w_{ii'}]$ is a $m\times m$ distance matrix that measures the \underline{distance} between each pair of objects in $X=\{\vec{x}_1,...,\vec{x}_m\}$, where the objects in $X$ can be numerical, categorical, or mixed-data type.
    \item 
\end{enumerate}
