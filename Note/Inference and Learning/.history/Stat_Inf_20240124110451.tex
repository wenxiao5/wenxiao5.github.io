...
\begin{equation}
    ...
    \nonumber
\end{equation}
Here, the data $\{Z_i\}_i$ is generated from $P_0$, which forms $P_m$.\\ Sometimes, we may study $v_A(P_0)(t)=\text{Pr}_{P_0}\left(\underbrace{\frac{C(P_m,P_0)}{\text{Var}_{P_0}(\theta(P_m))}}_{T(P_m,P_0)}\leq t\right),\forall t$ instead.
...
\begin{example}
    ...
    \begin{equation}
        \begin{aligned}
            v_A(P_m)=\text{Pr}_{P_0}\left(\sqrt{m}\frac{\frac{1}{m}\sum_{i=1}^m (Z_i^*-\underbrace{\mathbb{E}_{P_m}(Z)}_{\bar{Z}_m})}{\text{Var}_{P_0}(\theta(P_m))}\leq t\right)
        \end{aligned}
        \nonumber
    \end{equation}
