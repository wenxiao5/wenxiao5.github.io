...
Consider a prediction problem that the distribution $F_{X,Y}$ is known, we observe $X\in \mathbb{R}^{K\times N}$ and predict $Y$. Only linear functions of $X$ are allowed $\mathcal{L}=\{X'b:b\in \mathbb{R}^K\}$. We use square experience loss $(Y-X'b)^2$. We want to minimze Risk (mean squared error) $\mathbb{E}_{X,Y}[(Y-X'b)^2]=\int_{x,y}(y-x'b)^2f_{x,y}(x,y)dxdy$.

Let $\beta_0=\arg\min_{b\in \mathbb{R}^k}\mathbb{E}_{X,Y}[(Y-X'b)^2]$. By the F.O.C. $\mathbb{E}_{X,Y}[(Y-X'\beta_0)X]=0$
