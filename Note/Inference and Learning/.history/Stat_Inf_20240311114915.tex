...
Suppose $\theta_0=\argmin_{\theta\in\Theta} Q(\theta)$, where $Q: \Theta \rightarrow \mathbb{R}$ is criterion (objective) function (unknown) and $\hat{Q}: \Theta \rightarrow \mathbb{R}$ is a sample version. $\hat{\theta}$ is called \textbf{extremum estimator} of $\theta$ if $\hat{\theta}=\argmin_{\theta\in\Theta} \hat{Q}(\theta)$
...
\begin{example}[ Nonlinear Least Squares]
    ...
    \begin{equation}
        \begin{aligned}
            Q(\theta)=\mathbb{E}[Y-g(X,\theta_0)]^2
        \end{aligned}
        \nonumber
    \end{equation}
    and the sample 
