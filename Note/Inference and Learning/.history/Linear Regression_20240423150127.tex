...
$\sqrt{n}\left(\hat{\beta}_\textnormal{NLS}-\beta_0\right) \Rightarrow N\left(0,G^{-1}\Omega G^{-1}\right)$, where $g(x_i,\beta)=-(y_i-exp\{x_i^T\beta\})exp\{x_i^T\beta\}x_i$, $G=\mathbb{E}\left[\frac{\partial g(x_i,\beta_0)}{\partial \beta}\right]=\mathbb{E}[exp\{2x_i^T\beta_0\}x_ix_i^T]$ and $\Omega=\textnormal{Var}(g(x_i,\beta_0))=\mathbb{E}[(y_i-exp\{x_i^T\beta\})^2exp\{2x_i^T\beta\}x_ix_i^T]=\mathbb{E}[\sigma^2(x_i)exp\{2x_i^T\beta\}x_ix_i^T]$.

\subsection{$Geometric(exp\{x_i^T\beta_0\})$ Distribution: $f(y;\pi)=(1-exp\{x_i^T\beta_0\})exp\{y\cdot (x_i^T\beta_0)\}$}
$f(y;x_i^T\beta_0)=(1-exp\{x_i^T\beta_0\})exp\{y\cdot (x_i^T\beta_0)\}$, where $x_i^T\beta<0$ with probability one for all possible values of $\beta$; but the distribution of $x_i$ does not otherwise involve $\beta$.\\
...
\begin{equation}
    \begin{aligned}
        \mathbb{E}[u_i(\beta_0)\mid x_i]=\mathbb{E}[y_i\mid x_i]-\frac{exp\{x_i^T\beta_0\}}{1-exp\{x_i^T\beta_0\}}=\frac{1}{x_i^T\beta_0}-\frac{1}{x_i^T\beta_0}=0
    \end{aligned}
    \nonumber
\end{equation}
