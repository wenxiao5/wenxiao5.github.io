...
\end{definition}
The MA(1) process, $\{Y_t\}$, is covariance stationary:
\begin{enumerate}
...
\begin{definition}[MA(p)]
    ...
    where $\epsilon_t\sim \textnormal{WN}(0,\sigma^2)$.
\end{definition}
The MA(p) process, $\{Y_t\}$, is covariance stationary:
\begin{enumerate}
    \item $\mathbb{E}[Y_t]=\mu$ and
    \item the autocovariance function is $$\gamma(j):=\textnormal{Cov}(Y_t,Y_{t-j})=\left\{\begin{matrix}
...
\end{definition}
\begin{lemma}
    The $\textnormal{MA}(\infty)$ process, $\{Y_t\}$, is covariance stationary: i.e.,
    \begin{enumerate}
        \item $\mathbb{E}[Y_t]=\mu$ and
