...
\chapter{}
Consider $Z\in \mathbb{Z}=\{z_1,....,z_J\}$ with parameter $\theta=\{\theta_1,...,\theta_J\}\in \Theta= S^{J-1}=\{\theta\in \mathbb{R}^J:\sum_{j=1}^J\theta_j=1,\theta_j\geq 0, j=1,...,J\}$ such that $P(Z=z_j\mid\theta)=\theta_j$.

\underline{Goal:} Assign probability mass to $\Theta$. -- Dirichlet distribution
...
\begin{example}[ (Square Loss)]
    ...
    \begin{equation}
        ...
        \nonumber
    \end{equation}
    The averge risk with prior belief $P(\theta)=1_{\{\theta\in[0,1]\}}$ is
    \begin{equation}
        \begin{aligned}
...
Consider a series of Gamma random variable $w_i^{(t)}\sim \textnormal{Gamma}(1,1)$ from time $t=1,...,T$. Given $N$ random sample $\{Z_1,...,Z_N\}$ from a Bernoulli distribution with parameter $\theta$ and the sum $\sum_{i=1}^N Z_i=S$. Then, we have
\begin{equation}
    \begin{aligned}
    ...
    \nonumber
\end{equation}
Define $v_i^{(t)}=\frac{w_i^{(t)}}{\sum_{j=1}^N w_j^{(t)}}$ and $\hat{\theta}^{(t)}=\sum_{i=1}^N v_i^{(t)}Z_i$. Based on the property of Gamma distribution, we have $\mathbb{E}[w_i^{(t)}]=\textnormal{Var}[w_i^{(t)}]=1$ and $\mathbb{E}[v_i^{(t)}]=\frac{1}{N}$.
