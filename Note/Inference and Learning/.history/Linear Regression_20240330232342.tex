...
The MLE is given by solving F.O.C.
\begin{equation}
    \begin{aligned}
        \hat{g}(\beta)|_{\beta=\hat{\beta}_{MLE}}=\frac{1}{n}\sum_{i=1}^ng(Y_i\mid X_i;\beta)\big|_{\beta=\hat{\beta}_{MLE}}=0^d
    \end{aligned}
    \nonumber
\end{equation}
...
\begin{example}[ (Logistic Regression)]
    ...
    \begin{equation}
        \begin{aligned}
            g(Y\mid X;\beta)=\frac{Y-F(X^T\beta)}{F(X^T\beta)(1-F(X^T\beta))}\cdot\left(F'(X^T\beta)\cdot X\right)
        \end{aligned}
        \nonumber
    \end{equation}
