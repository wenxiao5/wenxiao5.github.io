...
$$
\begin{aligned}
\max _{\Theta, \mathrm{W}} P\left(x_1, x_2, \ldots, x_m \mid \Theta\right)=\max _{\Theta, \mathrm{W}} \prod_{i=1}^m P\left(x_i \mid \Theta\right) \\
=\max _{\Theta, \mathrm{W}} \prod_{i=1}^m \sum_{k=1}^K P\left(x_i \in C_k\right) P\left(x_i \mid x_i \in C_k, \theta_k\right)
\end{aligned}
$$

where $W=\left\{P\left(x \in C_1\right), \ldots, P\left(x \in C_K\right)\right\}$ and $\sum_{k=1}^K P\left(x \in C_k\right)=1$
Solution: We can use an iterative search based algorithm to attempt to find the optimal parameters $\Theta, W$ that maximize this function $P(X \mid \Theta)$.
