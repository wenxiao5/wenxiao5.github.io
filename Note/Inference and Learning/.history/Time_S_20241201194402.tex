...
\begin{enumerate}[$\circ$]
    ...
    \item The $AR(1)$ model admits an $MA(\infty)$ solution
    \begin{equation}
        \begin{aligned}
...
\begin{definition}[Stability of $AR(p)$]
    The $AR(p)$ model is \textbf{stable} if \begin{equation}
        ...
        \nonumber
    \end{equation}
    (All solutions' absolute values are greater than 1).
\end{definition}
...
\end{definition}
\begin{lemma}[Equivalence of Stability]\label{lemma:eqv_stability}
...
\begin{lemma}[Asymptotic Normality]\label{lemma:asymptotic_normality}
    Suppose that
    \begin{enumerate}[(i).]
        \item $\frac{1}{n}\sum_{i=1}^n x_ix_i' \stackrel{P}{\longrightarrow} Q$ for some $Q\succ 0$.
...
\begin{proof}
    ...
    \begin{equation}
        ...
        \nonumber
    \end{equation}
    where $\frac{1}{\sqrt{T}}\tilde{\psi}(L)(\epsilon_T-\epsilon_0) =o_p(1)$ is proved by
    \begin{equation}
        \begin{aligned}
            \mathbb{E}\left[\frac{1}{\sqrt{T}}\tilde{\psi}(L)(\epsilon_T-\epsilon_0)\right]&=0\\
            \textnormal{Var}\left[\frac{1}{\sqrt{T}}\tilde{\psi}(L)(\epsilon_T-\epsilon_0)\right]&=\frac{1}{T}\mathbb{E}\left[(\tilde{\psi}(L)\epsilon_T-\tilde{\psi}(L)\epsilon_0)^2\right]\\
...
\begin{corollary}
    If $\epsilon_t\sim \textnormal{i.i.d.} \mathcal{N}(0,\sigma^2)$ and if $(\epsilon_2,\epsilon_3,...)\perp Y_1$, then, under ``regularity'' conditions,
    \begin{equation}
        \begin{aligned}
            \sqrt{T}\left(\hat{\phi}_{OLS}-\phi\right) \stackrel{d}{\longrightarrow} \mathcal{N}\left(0,\mathcal{I}(\theta)^{-1}\right)
        \end{aligned}
        \nonumber
    \end{equation}
    where $\mathcal{I}(\theta)^{-1}$ is the Cramér-Rao Lower Bound.
\end{corollary}
...
\begin{claim}[$\hat{\phi}_{OLS}$ is Biased]
...
\begin{claim}[$\hat{\phi}_{OLS}$ is Consistent]
...
Recall that if $\{X_t\}$ is non-random and bounded and if $r_t \rightarrow \infty$, $\frac{X_t}{r_t} \rightarrow 0$.
\begin{enumerate}
...
\end{definition}
\begin{lemma}[$X_t\sim i.i.d$ with $\mathbb{E}[X_t]=0$ $\Rightarrow$ $L^1-$mixingale]
...
\begin{lemma}[$L^1-$mixingale $\Rightarrow$ $\mathbb{E}[X_t]=\mathbb{E}\left(\mathbb{E}[X_t\mid Z_{t-m},Z_{t-m-1},...]\right)=0$]
...
\begin{example}[ ($X_t=Z\sim \mathcal{N}(0,1),\forall t$ is not an $L^1-$mixingale)]
...
\begin{remark}
    ...
    \end{example}
\end{remark}
\subsection{Uniformly Integrality}
...
\subsection{OLS Estimator has Asymptotic Normality}
Usual template, i.e., the Lemma \ref{lemma:asymptotic_normality}. The estimator $\hat{\phi}$ satisfies asymptotic normality if
\begin{enumerate}[(i).]
...
\begin{claim}[Asymptotic Normality of OLS]
...
\subsection{Estimation of Variance $1-\phi^2$}
To be estimated:
\begin{equation}
    \begin{aligned}
...
\begin{lemma}[Properties of GMM Estimator]
    ...
    \end{enumerate}
    Then, $$\sqrt{T}\left(\hat{\theta}_{GMM}-\theta_0\right)\stackrel{d}{\longrightarrow} \mathcal{N}\left(0,\Omega\right)$$
    where $\Omega:=\left[G'WG\right]^{-1}G'WVWG\left[G'WG\right]^{-1}$ and $\Omega(W)\geq \Omega(V^{-1})=(G'V^{-1}G)^{-1}$.
\end{lemma}
\begin{remark}
    \begin{enumerate}
        ...
        \begin{equation}
            ...
            \nonumber
        \end{equation}
        \item $G_T(\theta_0)=\frac{1}{T}\sum_{t=1}^T \frac{\partial }{\partial \theta'}h(X_t,\theta_0)$. Thus, if $\frac{\partial }{\partial \theta'}h(X_t,\theta_0)$ satisfies LLN, then (ii) holds and $$G=\mathbb{E}\left[\frac{\partial }{\partial \theta'}h(X_t,\theta_0)\right].$$
        ...
        \begin{enumerate}
            ...
            \begin{enumerate}
                ...
                \item $g_T(\theta)= g_T(\theta_0)+G_T(\theta)\left(\theta-\theta_0\right),\forall \theta$
            \end{enumerate}
...
\begin{enumerate}[$\cdot$]
    ...
    \begin{theorem}[Helly-Bray Theorem]
...
\begin{theorem}[Donsker's Theorem (FCLT)]
...
\begin{equation}
    ...
    \nonumber
\end{equation}
Thus, $T\cdot(\hat{\rho}-1)=O_P(1) \Rightarrow \hat{\rho}$ is ``super-consistent.''

Note that the $T\cdot(\hat{\rho}-1)$ is not symmetric distributed around 0, so $T\cdot(\hat{\rho}-1)\stackrel{d}{\nrightarrow} \mathcal{N}(0,\cdot)$.
...
\begin{remark}
    ...
    \begin{enumerate}
        ...
        \item $T\cdot(\hat{\rho}-1)$ is also asymptotically pivotal.
        \item $5\%$ critical value for $t(1)$ is $-1.95<-1.645$.
    \end{enumerate}
\end{remark}
