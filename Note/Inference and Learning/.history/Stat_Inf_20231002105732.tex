...
\begin{definition}[Test Size and Level $\alpha$]
    \normalfont
    \begin{enumerate}
        \item[] The \textbf{size} of a test (with power function $\beta$) is $\sup_{\theta\in\Theta_0}\beta(\theta)$.
        \item[] A test is of \textbf{level $\alpha$ ($\in[0,1]$)} \underline{if and only if} its size is $\leq \alpha$. (Standard choice $\alpha=0.05$).
    \end{enumerate}
\end{definition}
