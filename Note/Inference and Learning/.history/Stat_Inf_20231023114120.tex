...
Consider a series of Gamma random variable $w_i^{(t)}\sim \textnormal{Gamma}(\alpha_i,1), i=1,...,J$ from time $t=1,...,T$. Then, we have
\begin{equation}
    \begin{aligned}
        \sum_{i=1}^N w_i^{(t)} \mathbf{1}_{\{Z_i=1\}}&\sim \textnormal{Gamma}(S,1)\\
        \sum_{i=1}^N w_i^{(t)} \mathbf{1}_{\{Z_i=0\}}&\sim \textnormal{Gamma}(N-S,1)
...
$\mathbb{E}[w_i^{(t)}]=\textnormal{Var}[w_i^{(t)}]=1$ and $\mathbb{E}[v_i^{(t)}]=\frac{1}{N}$.
