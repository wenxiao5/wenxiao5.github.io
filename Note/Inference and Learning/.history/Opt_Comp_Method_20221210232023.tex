...
Now suppose that instead of computing the Jacobian $\nabla \vec{g}\left(\vec{x}^{(k)}\right)$, we somehow found an approximation $D_k$ : an $n \times n$ matrix that's our best guess at the partial derivatives of $\vec{g}$. Just as with Newton's
