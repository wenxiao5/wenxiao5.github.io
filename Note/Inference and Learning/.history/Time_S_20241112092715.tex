...
\begin{proposition}[Newey-West Estimator of $V$]
    $$\hat{V}=\lim_{T\rightarrow \infty}\textnormal{Var}[\sqrt{T}g_T(\theta_0)]=\hat{\Gamma}_n(0)+\sum_{j=1}^\infty\left(1-\frac{j}{n}\right)[\hat{\Gamma}_n(j)+\hat{\Gamma}_n(j)']$$
    where $\hat{\Gamma}_n(j)=\frac{1}{T}\sum_{t+j}^Th(X_t,\hat{\theta})h(X_{t-j},\hat{\theta})'$
\end{proposition}
