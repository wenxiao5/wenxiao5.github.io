...
\begin{enumerate}[$\bullet$]
    ...
    \end{example}
    \item Testing Procedure:
    \subitem Consider the problem of testing $H_0:\theta\in \Theta_0$ vs. $H_1:\theta\in \Theta_1$.
    \subitem A \underline{testing procedure} is a (data-based) rule for choosing between $H_0$ and $H_1$.
    \subitem The rule: $$\textnormal{"Reject $H_0$ iff $(X_1,...,X_n)\in C$" (for some $C\in \mathbb{R}^n$)}$$ is a testing procedure with \underline{critical region} $C$.
\end{enumerate}
