...
Define $\vec{m}\triangleq \left[m(X_1),...,m(X_N)\right]'$, its estimation is denoted by $\hat{\vec{m}}$, which is based on the decision rule.

\begin{definition}[Sum of Squared Residual]
    \normalfont
    \textbf{Sum of Squared Residual} (SSR) of an estimation $\hat{\vec{m}}$ is given by
    \begin{equation}
        \begin{aligned}
            \mathbb{E}\left[\|\vec{Y}-\hat{\vec{m}}\|^2\right]&=\mathbb{E}\left[\|Y-m\|^2\right]+\mathbb{E}\left[\|\hat{\vec{m}}-\vec{m}\|^2\right]-2 \mathbb{E}\left[(\vec{Y}-\vec{m})'(\hat{\vec{m}}-\vec{m})\right]\\
            &=N\sigma^2+\sum_{i=1}^N\left(\hat{m}(X_i)-m(X_i)\right)^2-2\sigma^2 df(\hat{\vec{m}})
        \end{aligned}
        \tag{SSR}
        \label{SSR}
    \end{equation}
    where the norm is $\|\vec{X}\|=\left[\sum_{i=1}^N X_i^2\right]^{1/2}$, and the degree of freedom $df(\hat{\vec{m}})=\frac{\sum_{i=1}^N \textnormal{Cov}(Y_i,\hat{m_i})}{\sigma^2}$.
\end{definition}
The second equality is because
\begin{equation}
    \begin{aligned}
        \mathbb{E}\left[(\vec{Y}-\vec{m})'(\hat{\vec{m}}-\vec{m})\right]&=\sum_{i=1}^N \mathbb{E}\left[(Y_i-m_i)(\hat{m_i}-m_i)\right]\\
...
We can represent the \underline{risk} of estimation $\hat{\vec{m}}$ by rewriting \ref{SSR}
\begin{equation}
    \begin{aligned}
        \mathbb{E}\left[\|\hat{\vec{m}}-\vec{m}\|^2\right]=\mathbb{E}\left[\|\vec{Y}-\hat{\vec{m}}\|^2\right]-N\sigma^2+2\sigma^2 df(\hat{\vec{m}})
    \end{aligned}
    \nonumber
\end{equation}

\section{Slein's Unbiased Risk Estimate (SURE)}
\begin{assumption}
    Suppose the $m(\cdot)$ can be wrritten as a liner combination of bases functions:
    $$m(X)=\sum_{k=1}^K \alpha_k g_k(X)$$
\end{assumption}

\begin{definition}[Gram-Schmidt Orthonormalization]
    \normalfont
    $\phi_{k}(X), k=1,...,K$ such that $\frac{1}{N}\sum_{i=1}^N\phi_{k}^2(X_i)=1$ and $\frac{1}{N}\sum_{i=1}^N\phi_{k}(X_i)\phi_{k}(X_j)=0$.
\end{definition}
Suppose the $m(\cdot)$ is a liner combination of Gram-Schmidt orthonormalizations:
\begin{equation}
    \begin{aligned}
        m(X)&=\sum_{k=1}^K \theta_k \phi_{k}(X)\\
        &\triangleq W \boldsymbol{\theta}
    \end{aligned}
    \nonumber
\end{equation}
where $W=\left(w(X_1),...,w(X_N)\right)^T\in \mathbb{R}^{N\times K}$, $w(X_i)=\left(\phi_1(X_i),...,\phi_K(X_i)\right)^T$, and $\boldsymbol{\theta}=\left(\theta_1,...,\theta_K\right)^T$.
