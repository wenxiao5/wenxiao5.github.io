...
\begin{definition}[Risk]
    \normalfont
    \textbf{Risk of decision rule $\delta$ on $\theta$:} $$R(\delta,\theta)=\mathbb{E}_{X\sim \pi(\theta)}\left[l(\delta(X),\theta)\right]$$ where $X$ are random with prob $\pi(\theta)$.\\
...
Consider $Z\in \mathbb{Z}=\{z_1,....,z_J\}$ with parameter $\theta=\{\theta_1,...,\theta_J\}\in \Theta= \mathbb{S}^{J-1}=\{\theta\in \mathbb{R}^J:\sum_{j=1}^J\theta_j=1,\theta_j\geq 0, j=1,...,J\}$ such that $P(Z=z_j\mid\theta)=\theta_j$.
