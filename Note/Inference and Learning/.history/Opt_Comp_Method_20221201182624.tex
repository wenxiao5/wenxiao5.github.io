...
\begin{theorem}
    Suppose that we are using Newton's method to solve $g(\vec{x}) = \vec{0}$ for some $g : \mathbb{R}^n → \mathbb{R}^n$ (with continuous first partial derivatives). If the sequence of points $\vec{x}_1,\vec{x}_2,...$ that we get converges to some limit $\vec{x}^*$, then $g(\vec{x}^*) = \vec{0}$.
