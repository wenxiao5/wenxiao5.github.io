...
Specifically, if $\hat{Q}$ is in the form of $\mathbb{E}m(\textnormal{data},\theta)$ (i.e., $\sum_{i=1}^n \rho(x_i,\theta)$ for some function $\rho$), we call the $\hat{\theta}\in\argmin_{\theta\in\Theta} \hat{Q}(\theta)$ be the \textbf{M-estimator} of $\theta$.
\begin{example}
    $\hat{\theta}\in\argmin_{\theta\in\Theta} \mathbb{E}\log f(x|\theta)$
\end{example}
