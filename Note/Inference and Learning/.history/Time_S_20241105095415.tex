...
\begin{lemma}[Properties of GMM Estimator]
    ...
    Suppose
    \begin{enumerate}[(i).]
        \item $\sqrt{T}\left(\hat{\theta}_{GMM}-\theta_0\right)\approx -\left[G_T(\theta_0)'W_TG_T(\theta_0)\right]^{-1}G_T(\theta_0)'W_T\sqrt{T}g_T(\theta_0)$.
    ...
    \end{enumerate}
    Then, $\sqrt{T}\left(\hat{\theta}_{GMM}-\theta_0\right)\stackrel{d}{\longrightarrow} \mathcal{N}\left(0,\Omega\right)$, where $\Omega:=\left[G'WG\right]^{-1}G'WVWG\left[G'WG\right]^{-1}$.
\end{lemma}
