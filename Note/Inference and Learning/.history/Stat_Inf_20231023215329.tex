...
\section{Non-Parametric / Bayesian (Weighted) Bootstrap}
This is \textbf{non-parametric}! It may also called \textit{Posterior Simulation}.
\begin{assumption}
...
\begin{enumerate}[(1).]
    ...
    \item Draw samples $\vec{\theta}^{(t)},t=1,...,T$ from $\pi(\vec{\theta}\mid \vec{Z})$ and compute $$\frac{1}{T}\sum_{t=1}^T\hat{\theta}^{(t)}\approx \mathbb{E}[\theta^{(t)}|\{Z_1,...,Z_n\}]$$
\end{enumerate}
\end{definition}
