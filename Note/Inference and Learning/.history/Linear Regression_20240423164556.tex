...
Under $H_0:\beta^T\beta_0-1=0$. Define  a new score function $g(\beta_0)=\beta_0^T\beta_0-1$. The corresponding variance (information matrix) of $g(\hat{\beta})$ is $\hat{\mathcal{I}}=\mathbb{E}\left[\frac{\partial g(\hat{\beta})}{\partial \beta}\right]=$ (Note that $\hat{\beta}$ is derived from MLE, so $g(\hat{\beta})$ is also a MLE of some distributions.)
