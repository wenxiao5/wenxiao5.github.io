...
\begin{enumerate}[(1).]
    \item Choose critical value such that $\beta(\theta)\leq 5\%$ when $\theta\in\Theta_0$ (i.e., $\sup_{\theta\in\Theta_0}\beta(\theta)\leq 5\%$);
    \item Choose test statistic such that $\beta(\theta)$ is "maximized" (among tests with $\sup_{\theta\in\Theta_0}\beta(\theta)\leq 5\%$).
\end{enumerate}


\subsection{Choice of Critical Value: Uniformly Most Powerful (UMP) Level $\alpha$ Test}
...
\subsubsection*{Likelihood Ratio Test: Neyman-Pearson Lemma}
