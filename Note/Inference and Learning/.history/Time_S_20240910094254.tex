...
\begin{definition}[Moving Average Process]
    \begin{enumerate}
        ...
        where $\epsilon_t\sim \textnormal{WN}(0,\sigma^2)$
        \begin{claim}
            Conjecture: $\{Y_t\}$ is covariance stationary; $\mathbb{E}[Y_t]=\mu$ and its autocovariance function is $$\gamma(j):=\textnormal{Cov}(Y_t,Y_{t-j})=(\sum_{i=0}^{\infty}\psi_i\psi_{i+j})\sigma^2,\forall j\geq 0.$$
        \end{claim}
    \end{enumerate}
\end{definition}
