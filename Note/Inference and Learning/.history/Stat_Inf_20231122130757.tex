...
\begin{proof}[for the unbiased property]
    Similar to \eqref{SSR},
    \begin{equation}
        \begin{aligned}
            \mathbb{E}[\|\hat{\theta}-\theta\|^2]&=\mathbb{E}[\|(\hat{\theta}-Z)+(Z-\theta)\|^2]\\
            &=\mathbb{E}[\|\hat{\theta}-Z\|^2]+\mathbb{E}[\|\theta-Z\|^2]-2\mathbb{E}[(\hat{\theta}-Z)(\theta-Z)]\\
            &=\\
            &=-K\sigma^2+\underbrace{2\sigma^2\sum_{k=1}^K \frac{\textnormal{Cov}(\mathcal{Z}_k,\hat{\theta}_k)}{\sigma^2}}_{df(\hat{\theta})}+\underbrace{\mathbb{E}[\|\hat{\theta}-\mathcal{Z}\|^2]}_{\textnormal{expected sample fit}}
        \end{aligned}
        \nonumber
    \end{equation}
    where
