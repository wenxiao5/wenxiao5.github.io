...
Warning: Chi-squared approximation can be poor (later).

\begin{example}[ Poisson Case]
    For a loglinear model $\log\eta_i=\alpha+\sum_{k}\beta_{k}x_{ik}$, we can show $$D(\boldsymbol{y} ; \hat{\boldsymbol{\mu}})=2\sum_i \left(y_i\ln\frac{y_i}{\hat{\mu}_i}-y_i+\hat{\mu}_i\right)$$
\end{example}
