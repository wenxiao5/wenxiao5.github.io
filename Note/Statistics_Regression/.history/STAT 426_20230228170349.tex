...
$$
\subsubsection*{Interpretation $\beta$}
...
\subsubsection*{Interpretation $\alpha$}
...
Assume independently-sampled data pairs $(x_i,y_i), i=1,...,N$. Let $\hat{\alpha},\hat{\beta}$ be the MLEs. We still use three testing methods: 1. Wald; 2. Likelihood ratio; 3. Score (“Rao”).
...
is
$$
\frac{\hat{\beta}}{S E(\hat{\beta})} \underset{H_0}{\dot{\sim}} N(0,1)
$$
where the square of $S E$ comes from the estimated asymptotic covariance matrix (estimated inverse information matrix).

Similarly, we can form a Wald $\mathrm{Cl}$ for $\beta$: $(L,U)$. Then, odds ratio $e^\beta$ for increasing $x$ by one unit is estimated by $e^{\hat{\beta}}$.
