...
\begin{definition}[Wald Test]
    ...
    So we reject the $H_0$ if $|z_W|\geq z_{\frac{\alpha}{2}}$ for a two-sided level $\alpha$ test.

    The $(1-\alpha)100\%$ Wald (confidence) interval is $$\{\beta_0:|z_W|=\frac{|\hat{\beta}-\beta_0|}{SE}<z_{\frac{\alpha}{2}}\}=(\hat{\beta}-z_\frac{\alpha}{2}SE,\hat{\beta}+z_\frac{\alpha}{2}SE)$$

    The Wald test also has a \underline{chi-squared form}, using
    \begin{equation}
        \begin{aligned}
...
\begin{definition}[Likelihood Ratio Test]
    ...
    Thus, reject $H_0$ if $$-2\ln\Lambda\geq\chi_1^2(\alpha)$$
    The $(1-\alpha)100\%$ likelihood-ratio (confidence) interval is $$\{\beta_0:-2\ln\Lambda=-2\left(L(\beta_0)-L(\hat{\beta})\right)<\chi_1^2(\alpha)\}=(\hat{\beta}-z_\frac{\alpha}{2}SE,\hat{\beta}+z_\frac{\alpha}{2}SE)$$
\end{definition}
...
\begin{definition}[Score Test]
    ...
    Thus, reject $H_0$ if $|z_S|\geq z_{\frac{\alpha}{2}}$.\\
