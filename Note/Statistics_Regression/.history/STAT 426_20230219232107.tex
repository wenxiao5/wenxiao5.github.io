...
in vector form: $$\mathbf{\eta}=\mathbf{X}\mathbf{\beta}$$
where $\mathbf{\eta}=\left[\eta_1,\cdots,\eta_N\right]^T$, $\mathbf{\beta}=\left[\alpha,\beta_1,\cdots, \beta_p\right]^T$, and the model matrix $\mathbf{X}$ has $i^\textnormal{th}$ row $[1,x_{i1},\cdots,x_{ip}]$.

Let the MLE of $\mathbf{\beta}$
