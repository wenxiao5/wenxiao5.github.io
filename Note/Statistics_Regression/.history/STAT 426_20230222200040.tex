...
\begin{definition}[Deviance]
    ...
    $$
    Note: $D(\boldsymbol{y} ; \hat{\boldsymbol{\mu}})$ is the \underline{likelihood-ratio test (LRT) chi-squared statistic} for
    \begin{enumerate}[-]
...
Warning: Chi-squared approximation can be poor.
...
\begin{example}[ Poisson Case]
    ...
    \textbf{Remark:}
    \begin{enumerate}[$\bullet$]
        \item The chi-squared approximation (under H0) is adequate if all $\mu_i$ are sufficiently large.
        \item These formulas also apply to loglinear rate models (with rate variable $t_i$), for which $\mu_i=\lambda_i t_i$, $\hat{\mu}_i=\hat{\lambda}_i t_i$.
    \end{enumerate}
\end{example}
