...
in vector form: $$\vec{\eta}=\mathbf{X}\vec{\beta}$$
where $\vec{\eta}=\left[\eta_1,\cdots,\eta_N\right]^T$, $\vec{\beta}=\left[\alpha,\beta_1,\cdots, \beta_p\right]^T$, and the model matrix $\mathbf{X}$ has $i^\textnormal{th}$ row $[1,x_{i1},\cdots,x_{ip}]$.

Let the MLE of $\vec{\beta}$ be $$\hat{\vec{\beta}}=\left[\hat{\alpha},\hat{\beta}_1,\cdots,\hat{\beta}_p \right]^T$$

\subsection{(Fisher) Information Matrix}
\begin{definition}[(Fisher) Information Matrix]
    The (Fisher) information matrix for $\vec{\beta}\in \mathbb{R}^{p+1}$ is a $(p+1)\times(p+1)$ matrix $$\mathcal{J}$$ with element $(h,j)$ being $$\mathbb{E}\left(-\frac{\partial^2 L(\vec{\beta})}{\partial \beta_h \partial \beta_j}\right)$$
\end{definition}
For a GLM, the information matrix becomes $$\mathcal{J}=\mathbf{X}^T \mathbf{W} \mathbf{X}$$
...
Under regularity conditions, as $N \rightarrow \infty$, the distribution of $\hat{\boldsymbol{\beta}}$ is approximately multivariate normal with mean vector $\boldsymbol{\beta}$ and covariance matrix $\mathcal{J}^{-1}$ :
$$
\hat{\boldsymbol{\beta}} \dot{\sim} N\left(\boldsymbol{\beta}, \mathcal{J}^{-1}\right)
$$
So the asymptotic covariance of $\hat{\boldsymbol{\beta}}$ is
$$
\mathcal{J}^{-1}=\left(\boldsymbol{X}^T \boldsymbol{W} \boldsymbol{X}\right)^{-1}
$$
