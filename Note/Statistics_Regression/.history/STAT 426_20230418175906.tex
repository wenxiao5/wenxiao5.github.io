...
\begin{enumerate}[(1)]
    \item Independent Poisson: $N_{i j k}$ independently distributed as Poisson $\left(\mu_{i j k}\right)$; the counts in the table have no fixed totals, and the overall total $N$ for the table is random.
    \item Multinomial: $N_{i j k}$ jointly distributed as $\operatorname{Multinomial}(n, \boldsymbol{\pi})$, where $n$ is the fixed total for the table, and $\pi$ is the vector of all $I J K$ cell probabilities $\pi_{i j k}$ in some particular order.
\end{enumerate}
