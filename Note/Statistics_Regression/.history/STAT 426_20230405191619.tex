...
\end{array}
$$
If $\boldsymbol{X}$ has full rank, then existence of the $\boldsymbol{b}$ above implies that an MLE does not exist. There is no maximum. The MLE fails to exist because the likelihood keeps increasing as one or more parameters become infinite.
...
\end{definition}
One common situation can cause quasi-complete separation: One category of a nominal explanatory variable has only $Y =0$ cases or only $Y =1$ cases.
