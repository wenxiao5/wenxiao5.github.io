...
\title{\textbf{Statistics: Regression}}
\author{Wenxiao Yang}
...
\begin{definition}[Assumptions (errors $\varepsilon$ form)]
...
$Cov(\varepsilon_1,\varepsilon_j)=\sigma^2\delta_{ij}$ where $\delta_{ij}=\left\{\begin{matrix}
    0&i\neq j\\
    1&i=j
\end{matrix}\right.$

\begin{definition}[Assumptions ($Y|X$ form)]
    Based on the SLR model moment assumptions on the error terms, we have the following \textbf{equivalent} assumptions for the moments of $Y $conditioning on $X$:
    \begin{enumerate}[-]
        \item $E(y_i|X_i)=\beta_0+\beta_1x_i$
        \item $Var(y_i|X_i)=\sigma^2$
        \item $Cov(y_i,y_j|X_i,X_j)=0,\ i\neq j$
    \end{enumerate}
\end{definition}

\begin{claim}[Interpretation of $\beta_1$, $\beta_0$]
    $\beta_1$ is the \textbf{change in the mean} of the probability distribution function of $y$ per unit change in $x$.\\
    $\beta_0$ is the \textbf{mean} of the probability distribution function of $y$(at $x=0$) when $X=\vec{0}$, otherwise $\beta_0$ \textbf{has no particular meaning}.
\end{claim}

\subsection{Least Squares Estimations}
Suppose the \textbf{Residual Sum of Squares (RSS)} is
$RSS=\sum_{i=1}^n(y_i-\beta_0-\beta_1 x_i)^2$. We want to find estimates of $\beta_0$, $\beta_1$ to minimize the RSS:
$$(\hat{\beta_0},\hat{\beta_1})=\argmin_{(\beta_0,\beta_1)}RSS$$

\begin{proposition}[LS Estimators]
    \begin{equation}
        \begin{aligned}
            \hat{\beta}_{0}&=\bar{y}-\hat{\beta}_{1} \bar{x};\\ \hat{\beta}_{1}&=\frac{\sum_{i=1}^{n} x_{i} y_{i}-n \bar{x} \bar{y}}{\sum_{i=1}^{n} x_{i}^{2}-n \bar{x}^{2}}=\frac{\sum_{i=1}^{n}\left(x_{i}-\bar{x}\right)\left(y_{i}-\bar{y}\right)}{\sum_{i=1}^{n}\left(x_{i}-\bar{x}\right)^{2}}=\frac{\sum_{i=1}^{n}\left(x_{i}-\bar{x}\right)y_{i}}{\sum_{i=1}^{n}\left(x_{i}-\bar{x}\right)^{2}}
        \end{aligned}
        \nonumber
    \end{equation}
    
        Alternative Representation of $\hat{\beta_1}$ is
        ...
        where $S_{xy}=\sum_{i}\left(x_{i}-\bar{x}\right)\left(y_{i}-\bar{y}\right)$, $S_{xx}=\sum_{i}\left(x_{i}-\bar{x}\right)^{2}$, $S_{yy}=\sum_{i}\left(y_{i}-\bar{y}\right)^{2}$, and $r_{xy}=\frac{\sum_{i}\left(x_{i}-\bar{x}\right)\left(y_{i}-\bar{y}\right)}{\sqrt{\sum_{i}\left(x_{i}-\bar{x}\right)^{2}\sum_{i}\left(y_{i}-\bar{y}\right)^{2}}}=\frac{S_{xy}}{\sqrt{S_{xx}S_{yy}}}$.
\end{proposition}
\begin{proof}
...
\begin{lemma}[Unbiased Estimators]
    $\mathbb{E}\hat{\beta}_{0}=\beta_0$, $\mathbb{E}\hat{\beta}_{1}=\beta_1$.
\end{lemma}
\begin{proof}
    $\mathbb{E}\hat{\beta}_{1}=\mathbb{E}\left[\frac{\sum_{i=1}^{n}\left(x_{i}-\bar{x}\right)(\beta_0+\beta_1x_i+\varepsilon_i)}{\sum_{i=1}^{n}\left(x_{i}-\bar{x}\right)^{2}}\right]=\mathbb{E}\left[\frac{\sum_{i=1}^{n}\left(x_{i}-\bar{x}\right)\beta_1x_i}{\sum_{i=1}^{n}\left(x_{i}-\bar{x}\right)^{2}}\right]=\beta_1$. $\mathbb{E}\hat{\beta}_{0}=\mathbb{E}\left[\bar{y}-\hat{\beta}_{1} \bar{x}\right]=\beta_0$.
\end{proof}
...
\begin{definition}[Fitted Values]
    The \underline{\textit{Prediction of $y_i$}} or the \underline{\textit{fitted value at $x_i$}} is $$\hat{y_i}=\hat{\beta_0}+\hat{\beta_1}x_i$$
\end{definition}
\begin{equation}
    \begin{aligned}
        \hat{y_i}&=\hat{\beta_0}+\hat{\beta_1}x_i\\
        &=\bar{y}-\hat{\beta}_{1} \bar{x}+ \hat{\beta_1}x_i\\
        &=\bar{y}+\frac{\sum_{i=1}^{n}\left(x_{i}-\bar{x}\right)\left(y_{i}-\bar{y}\right)}{\sum_{i=1}^{n}\left(x_{i}-\bar{x}\right)^{2}}(x_i-\bar{x})
    \end{aligned}
    \nonumber
\end{equation}
\begin{definition}
    The \underline{\textit{$i^{th}$ residual}} is $r_i=y_i-\hat{y_i}$.
\end{definition}

\begin{proposition}[Properties of residuals]
    \begin{enumerate}
        \item $\sum_i r_i=0$.
        \item $RSS=\sum_i r_i^2$ is minimized.
        \item $\sum_{i=1}y_i=\sum_{i=1}\hat{y_i}$.
        \item $\sum_ix_ir_i=0$ prove by first-order condition (proof: $\sum_ix_ir_i=\sum_ix_i(y_i-\bar{y}-\hat{\beta_1}(x_i-\bar{x}))=\sum_ix_iy_i-n\bar{x}\bar{y}-\frac{\sum_{i=1}^{n} x_{i} y_{i}-n \bar{x} \bar{y}}{\sum_{i=1}^{n} x_{i}^{2}-n \bar{x}^{2}}(\sum_ix_i^2-n\bar{x}^2)=0$) \textcolor{red}{(We can interpret Least Square elimates $Y$'s values in $X$'s direction, the residual should be perpendicular to $X$.)}
        \item $\sum_i \hat{y_i} r_i=0$ (inferred from 4).
...
\section{Parameter Estimation $\hat{\beta}$}
...
\begin{equation}
    \begin{aligned}
        ...
        \mathbf{X}^{T}(y-\mathbf{X} \beta)&=\mathbf{0} \longrightarrow\text{ Normal Equations}\\
        \left(\mathbf{X}^{T} \mathbf{X}\right) \beta&=\mathbf{X}^{T} y\\
        \hat{\beta}&=\left(\mathbf{X}^{T} \mathbf{X}\right)^{-1} \mathbf{X}^{T} y \rightarrow \text{ LS Estimators}
    \end{aligned}
    \nonumber
\end{equation}
\textbf{Remarks}\\
...
\subsection{Hat Matrix $\mathbf{H}_{n \times n}=\mathbf{X}\left(\mathbf{X}^{T} \mathbf{X}\right)^{-1} \mathbf{X}^{\top}$}
...
\section{Residuals}
\subsection{Residuals: $\mathbf{r}=(\mathbf{I}-\mathbf{H}) y$}
...
$$
\subsection{Error Variance: $\hat{\sigma}^2=\frac{\mathbf{r}^T \mathbf{r}}{n-p}$}
...
\subsection{Orthogonal to column of data $\mathbf{X}^{T} \mathbb{r}=0$}
1. The cross-products between the residual vector $r$ and each column of $\mathbf{X}$ are zero, i.e.
$$
\begin{aligned}
\mathbf{X}^{T} \mathbf{r} &=\mathbf{X}^{T}(\mathbf{y}-\mathbf{X} \hat{\mathbf{\beta}})=\mathbf{X}^{T} \mathbf{y}-\mathbf{X}^{T} \mathbf{X} \hat{\beta} \\
&=\mathbf{X}^{T} \mathbf{y}-\left(\mathbf{X}^{T} \mathbf{X}\right)\left(\mathbf{X}^{T} \mathbf{X}\right)^{-1} \mathbf{X}^{T} \mathbf{y}=0
\end{aligned}
$$
\subsection{Orthogonal to fitted value $\hat{y}^{T} \mathbb{r}=0$}
...
\subsection{Variance-Covariance Matrix of $\hat{\beta}$: $\textnormal{Cov}(\hat{\beta})=\sigma^{2}\left(\mathbf{X}^{T} \mathbf{X}\right)^{-1}$}
...
\subsection{Properties of $\hat{y}$: $\mathbb{E}(\hat{y})=\mathbf{X}\beta$, $Cov(\hat{y})=\sigma^2 \mathbf{H}$}
\begin{equation}
    \begin{aligned}
    ...
    \nonumber
\end{equation}
\subsection{Properties of $\mathbf{r}$: $\mathbb{E}(\mathbf{r})=0$, $Cov(\mathbf{r})=\sigma^2 (\mathbf{I}_n-\mathbf{H})$}
\begin{equation}
    \begin{aligned}
    ...
    \nonumber
\end{equation}
\subsection{Unbiased Variance Estimation $\mathbb{E}(\hat{\sigma}^2)=\sigma^2$}
$$\mathbb{E}(\hat{\sigma}^2)=\frac{1}{n-p}\mathbb{E}(\mathbf{r}^T \mathbf{r})=\frac{1}{n-p}\sigma^2(n-p)=\sigma^2$$
\textit{$n-p$: there are $p$ elements in the diagonal of $H$ are $1$ and others are $0$, so the diagonal of $\mathbf{I}_n-\mathbf{H}$ has $n-p$ terms with value $1$. Then there are only $n-p$ terms with value $r_i^2=\sigma^2$.}
...
\subsection{Assumption $\mathbf{y} \sim \mathbf{N}_{n}\left(\mathbf{X} \beta, \sigma^{2} \mathbf{I}\right)$}
...
\subsection{Estimations $\hat{\beta}\sim \mathbf{N}_{\mathbf{p}}\left(\beta, \sigma^{2}\left(\mathbf{X}^{\top} \mathbf{X}\right)^{-1}\right)$, $\hat{\mathbf{y}}\sim\mathbf{N}_{\mathbf{n}}\left(\mathbf{X} \beta, \sigma^{2} \mathbf{H}\right)$, $\hat{\mathbf{r}}\sim \mathbf{N}_{\mathbf{n}}\left(\mathbf{0}, \sigma^{2}\left(\mathbf{I}_{\mathbf{n}}-\mathbf{H}\right)\right)$}
...
\end{gathered}
$$
\section{$\mathbf{r}$ Residuals' Properties}
\subsection{$n-p$ dimensions: $\mathbf{r}\in \mathbb{R}^{n-p}$}
Although $\mathbf{r}$ is a vector of dimension $n$, it always lies in a subspace of dimension $(n-p)$.
\subsection{Distribution: $\hat{\sigma}^{2}\sim\sigma^{2} \frac{\chi_{n-p}^{2}}{n-p}$}
$\mathbf{r}$ behaves like a random vector with a distribution $\mathbf{N}_{n-p}\left(\mathbf{0}, \sigma^{2} \mathbf{I}_{n-p}\right)$, so we have:
$$
\hat{\sigma}^{2}=\frac{\|\mathbf{r}\|^{2}}{n-p} \sim \sigma^{2} \frac{\chi_{n-p}^{2}}{n-p}
$$
\subsection{$\hat{\mathbf{y}}$ and $\mathbf{r}$ are independent}
...
$$
\begin{aligned}
h_{i} &=\mathbf{x}_{\mathbf{i}}^{\mathbf{T}}\left(\mathbf{X}^{\top} \mathbf{X}\right)^{-1} \mathbf{x}_{\mathbf{i}} \\
&=\frac{1}{n}+\frac{1}{n-1}\left(\mathbf{z}_{i}-\overline{\mathbf{z}}\right)^{T} \hat{\mathbf{\Sigma}}^{-1}\left(\mathbf{z}_{i}-\overline{\mathbf{z}}\right)
\end{aligned}
$$
where
$$
...
$$
is the sample covariance of the $(p-1)$ predictor variables. The second term in the right hand side ($\frac{1}{n-1}\left(\mathbf{z}_{i}-\overline{\mathbf{z}}\right)^{T} \hat{\mathbf{\Sigma}}^{-1}\left(\mathbf{z}_{i}-\overline{\mathbf{z}}\right)$) is the so-called \textbf{Mahalanobis distance} from $\mathbf{z}_{i}$ to the data center $\overline{\mathbf{z}}$
...
&\quad=-\frac{1}{2}(\mathbf{y}-\mathbf{X} \beta)^{\top} \mathbf{\Sigma}^{-1}(\mathbf{y}-\mathbf{X} \beta)+\text { Constant. }
\end{aligned}
$$
...
$$
\operatorname{minimize}(y-X \mathbf{\beta})^{\top}(y-X \mathbf{\beta})+\lambda \sum_{j} \mathbf{\beta}_{j}^{2}
$$
...
\begin{equation}
    \begin{aligned}
        (y-X\beta)^T(y-X\beta)\text{ subject to }\sum_{j} \mathbf{\beta}_{j}^{2}\leq t^2
    \end{aligned}
    \nonumber
\end{equation}
- The parameter $\lambda$ (or $t$) should be chosen to have stable estimates of $\beta$.\\

- Note that when $\lambda=0$ the ridge regression estimation problem reduces to the standard LS problem, while when $\lambda \rightarrow \infty, \hat{\mathbf{\beta}} \rightarrow 0$\\
...
- In this case the estimated $\hat{\mathbf{\beta}}$ minimizes:
$$
\operatorname{minimize}(y-X \mathbf{\beta})^{\top}(y-X \mathbf{\beta})+\lambda \sum_{j}\left|\beta_{j}\right|
$$
