...
\end{array}\right)
$$
- In this notation, $\operatorname{logit}(\pi(\mathbf{x}))=\mathbf{x}^T \boldsymbol{\beta}$
- Adding $\boldsymbol{\delta}$ to $\mathrm{x}$ adds $\boldsymbol{\delta}^T \boldsymbol{\beta}$ to the logit, so
$$
e^{\boldsymbol{\delta}^T \boldsymbol{\beta}}=\text { odds ratio at } \mathrm{x}+\boldsymbol{\delta} \text { relative to } \mathrm{x}
$$
