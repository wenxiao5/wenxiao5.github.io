...
which is also equivalent to maximizing the logarithm of the likelihood function $L(\beta,\vec{x}) = \log(l(\beta,\vec{x}))$, $$\hat{\beta}=\arg\max_{\beta}L(\beta,\vec{x})$$

\begin{definition}[score function]
    The score function is $$u(\beta,\vec{x})=\frac{\partial L(\beta,\vec{x})}{\partial \beta}=\frac{\nabla_{\beta} l(\beta,\vec{x})}{l(\beta)}$$
\end{definition}
\begin{lemma}
    $$\mathbb{E}_{\vec{x}}u(\beta,\vec{x})$$
\end{lemma}
