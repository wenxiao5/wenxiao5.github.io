...
\begin{enumerate}[(1)]
    \item \underline{Random component:} density of $Y_i$ from a \textbf{natural exponential family} $$f(y_i;\theta_i)=a(\theta_i)b(y_i)\textnormal{exp}(y_i Q(\theta_i))$$
    where $Q(\theta_i)$ is the \textbf{natural parameter}.
    \item \underline{Systematic component:} the \textbf{linear predictor}
    ...
    \item \underline{Link function:} monotonic, differentiable $g$ such that
    $$g(\mu_i)=\eta_i\quad \textnormal{ where }\mu_i=\mathbb{E}(Y_i)$$
    (Note: Ordinary linear models use the identity link: $g(\mu)=\mu$)
