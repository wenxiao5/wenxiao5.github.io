...
\begin{enumerate}[$\bullet$]
    ...
    \begin{equation}
        \begin{aligned}
            L(\boldsymbol{\mu})=n\lambda + \sum_{i}n_{i+}\lambda_i^X+\sum_j n_{+j}\lambda_j^Y + \sum_{ij}n_{ij}\lambda_{ij}^{XY}-\sum_{ij}e^{\lambda+\lambda_i^X+\lambda_j^Y+\lambda_{ij}^{XY}}
        \end{aligned}
        \nonumber
    \end{equation}
    \item 
