...
\begin{enumerate}[(1)]
    ...
    (Note: Ordinary linear models use the identity link: $g(\mu)=\mu$, which means $\mu=\alpha+\beta_1 x_{i1}+\cdots +\beta_p x_{ip}$.)
...
\begin{example}[ (Binary Regression)]
    ...
    Note:
    $$
    \mu_i=E\left(Y_i\right)=\pi_i
    $$
    Canonical link:
    $$
    g(\pi)=Q(\pi)=\operatorname{logit}(\pi)
    $$
    which leads to \textbf{logistic regression}.
\end{example}
