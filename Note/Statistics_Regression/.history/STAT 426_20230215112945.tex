...
If several observations have the same $\boldsymbol{x}$ ("replications"), then they have the same $\pi(\boldsymbol{x})$.

Summing binary (0/1) observations with the same $\boldsymbol{x}$ gives grouped data:
$$
Y_i \sim \operatorname{binomial}\left(n_i, \pi\left(\boldsymbol{x}_i\right)\right)
$$
where " $i$ " now refers to the $i$ th group (of $n_i$ binary obs.).
Note: Both $Y_i$ and $n_i\left(\right.$ or $\left.n_i-Y_i\right)$ must be included in the data.
