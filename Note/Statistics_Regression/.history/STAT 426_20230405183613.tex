...
Technically, the logistic regression MLE exists and is unique if and only if there does not exist $\boldsymbol{b} \neq \mathbf{0}$ such that, for all $i$,
$$
\begin{array}{rlr}
b^T \boldsymbol{x}_i>0 & \Rightarrow & y_i=1 \\
\boldsymbol{b}^T \boldsymbol{x}_i<0 & \Rightarrow & y_i=0
\end{array}
$$
If $\boldsymbol{X}$ has full rank, then existence of the $\boldsymbol{b}$ above implies that an MLE does not exist. There is no maximum.
