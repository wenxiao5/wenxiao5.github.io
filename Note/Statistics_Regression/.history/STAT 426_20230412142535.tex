...
\begin{enumerate}[$\bullet$]
    \item Nominal. e.g. favorite soft drink (Coke, Pepsi, other)
    \item Ordinal. e.g. customer rating of service (good, neutral, bad)
\end{enumerate}
...
\begin{enumerate}[$\bullet$]
    \item Ungrouped codes: $y_i=j$, $j\in\{1,...,J\}$.
    \item Ungrouped indicator vectors: $\boldsymbol{y}_i=(y_{i1},...,y_{iJ})$ where $y_{ij}=\left\{\begin{matrix}
        1,&\textnormal{ if obs. $i$ in category $j$}\\
        0,&\textnormal{ otherwise}
    \end{matrix}\right.$\\
    Then $\boldsymbol{Y}_i\sim \textnormal{multinomial}(1,\boldsymbol{\pi}(\boldsymbol{x}_i))$ where $\boldsymbol{\pi}(\boldsymbol{x})=\left(\pi_1(\boldsymbol{x}),...,\pi_J(\boldsymbol{x})\right)$.
    \item Grouped: Assume observation $i$ represents $n_i$ independent responses having the same explanatory vector $\boldsymbol{x}_i$ (i.e., replicates).
\end{enumerate}
