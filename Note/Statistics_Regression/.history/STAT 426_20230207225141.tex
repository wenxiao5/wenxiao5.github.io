...
\begin{definition}[Pearson $\chi^2$ Test (Score Test)]
    $X^2=\sum_{ij}\frac{(n_{ij}-\hat{\mu}_{ij})^2}{\hat{\mu}_{ij}}$
    $$
X^2=\sum_{i j} \frac{\left(n_{i j}-\hat{\mu}_{i j}\right)^2}{\hat{\mu}_{i j}} \underset{H_0}{\dot{\sim}} \chi_{(I-1)(J-1)}^2
$$
Note:
$$
\begin{aligned}
...
\begin{equation}
    \begin{aligned}
        \sqrt{n}\frac{g(T_n)-g(\theta)}{|g'(T_n)|\sigma} \dot{\sim}  N(0,1)
    \end{aligned}
    \nonumber
\end{equation}
