...
\begin{enumerate}[$\bullet$]
    ...
    \item \textbf{Ungrouped indicator vectors:} $\boldsymbol{y}_i=(y_{i1},...,y_{iJ})$ where $$y_{ij}=\left\{\begin{matrix}
        ...
        0,&\textnormal{ otherwise}
    \end{matrix}\right.$$
    Then $\summ_{j=1}^J y_{ij}=1,\forall i$ and $\boldsymbol{Y}_i\sim \textnormal{multinomial}(1,\boldsymbol{\pi}(\boldsymbol{x}_i))$ where $\boldsymbol{\pi}(\boldsymbol{x})=\left(\pi_1(\boldsymbol{x}),...,\pi_J(\boldsymbol{x})\right)$.
    ...
    Then $\summ_{j=1}^J y_{ij}=1,\forall i$ and 
\end{enumerate}
