...
\begin{definition}[Pearson Residuals]
    ...
    where
    $$
    \nu(\mu)=\operatorname{var}(Y) \text { when } E(Y)=\mu
    $$
    Eg: Poisson
    $$
...
\begin{definition}[Deviance Residuals]
    \normalfont
    The deviance can be written in terms of the sum of contributions from each observation:
    $$
    D(\boldsymbol{y} ; \hat{\boldsymbol{\mu}})=\sum_{i=1}^N d_i
    $$
    where
    $$
    d_i=-2\left(L\left(\hat{\mu}_i ; y_i\right)-L\left(y_i ; y_i\right)\right)
    $$
    is non-negative.
    The $i^\textnormal{th}$ deviance residual is
    $$
    \operatorname{sign}\left(y_i-\hat{\mu}_i\right) \cdot \sqrt{d_i}
    $$
\end{definition}
