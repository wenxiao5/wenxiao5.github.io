...
\textbf{Note:} The total number of (scalar) free parameters is $(J-1)(1+p)$, where $\boldsymbol{x}$ has $p$ elements.\\
Substituting the MLEs gives estimated response functions $\hat{\pi}_j(\boldsymbol{x}),\forall j$, "fitted values" $\hat{\pi}_{ij}=\hat{\pi}_j(\boldsymbol{x}_i),\forall i,j$, and estimated logits, such as $\ln\left(\frac{\hat{\pi}_{j}(\boldsymbol{x})}{\hat{\pi}_J(\boldsymbol{x})}\right)=\hat{\alpha}_j+\hat{\boldsymbol{\beta}}_j^T$
