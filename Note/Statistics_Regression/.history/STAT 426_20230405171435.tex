...
$$
Note: If $\theta_{X Y(k)}>1$ then $n_{11 k}-\mu_{11 k}$ tends to be positive.

If this is true for all $k, \mathrm{CMH}$ will tend to be larger.

(Likewise if $\theta_{X Y(k)}<1$ for all $k$.)
