...
\begin{definition}[Gittins Index]
    ...
    \begin{equation}
        ...
        \nonumber
    \end{equation}
    where $\tau$ is a measurable stopping time. The function $\mathcal{V}^i(x^i)$ is called \textbf{Gittins index} of bandit process $i$ at state $x^i$. The optimal $\tau$ is called the \underline{optimal stoppoing time at $x^i$}
\end{definition}
