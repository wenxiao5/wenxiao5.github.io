...
$$
\mathcal{R}(z):=\max _{p \geq 0} \sum_{j \in N}\left(p_{j}-z_{j}\right) \pi_{j}(N, p)
$$
For many choice models, if $p_{j}=\infty$ then, $\pi_{j}(N, p)=0$. Thus optimizing over prices could implicitly select an assortment.
...
\begin{center}
    \fcolorbox{black}{gray!10}{\parbox{.9\linewidth}{Add a random noise component to the utilities of the products: $$U_i =u_i +\varepsilon_i,i \in N.$$
