...
\end{definition}
\begin{enumerate}[$\bullet$]
    \item State never changes except when pulled.
    \item When pulled, state evolution is a \underline{Markov chain}. (Transition probabilities given by Bayes' Law.)
    \item \underline{State can be expected reward} when pulled is governed by state.
\end{enumerate}
