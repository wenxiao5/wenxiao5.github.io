...
We follow \cite{berry1995automobile} in assuming that each $\epsilon_{ijt}$ is an i.i.d. draw from a standard type-1 extreme value (Gumbel) distribution. The most common assumption is that these random variables are jointly normally distributed. Let $\bar{\alpha}$ and $\bar{\beta}$ be the average of $\alpha_{it}$ and $\beta_{it}$ that represent \underline{all} consumers' tastes. (In previous typical forms, $\bar{\alpha}=e^{\alpha_0}$ and $\bar{\beta}=\beta_0$.) In this case, define $\delta_{jt}:=x_{jt}\bar{\beta}-\bar{\alpha}p_{jt}+\xi_{jt}$.
