...
\begin{enumerate}
    ...
    \item $\epsilon_{ijt}$ is the \textit{error term}. It is most often specified as an i.i.d. draw from a standard type-1 extreme value distribution, yielding a \textit{mixed multinomial logit model (Mixed-MNL)}. In this case, the choice probabilities in the population (i.e., the market shares) are given by
    \begin{equation}
        \begin{aligned}
            s_{jt}=\int \frac{e^{x_{jt}\beta_{it}-\alpha_{it}p_{jt}+\xi_{jt}}}{}
        \end{aligned}
        \nonumber
    \end{equation}
    \item 
