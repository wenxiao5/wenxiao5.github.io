...
\begin{enumerate}
    ...
    \begin{enumerate}
        ...
        \begin{equation}
            \begin{aligned}
                \alpha_{it}=\alpha_0+\alpha_y y_{it}+\alpha_v v_{it}^{(0)}
            \end{aligned}
            \label{eq:alpha_it}
        \end{equation}
...
\begin{definition}
    ...
    \begin{enumerate}
        ...
        \begin{equation}
            \begin{aligned}
                \min_{\alpha,\beta}\frac{1}{J}\sum_{j=1}^{J_t}\left(\hat{\delta}_{jt}-x_{jt}\beta+\alpha p_{jt}\right)z_{jt}
            \end{aligned}
            \nonumber
        \end{equation}
...
We follow \cite{berry1995automobile} in assuming that each $\epsilon_{ijt}$ is an i.i.d. draw from a standard type-1 extreme value (Gumbel) distribution and that each $v_{it}^{(k)}$ in \eqref{eq:beta_it} is an i.i.d. draw from a standard normal distribution. Let $\bar{\alpha}$ and $\bar{\beta}$ be the average of $\alpha_{it}$ and $\beta_{it}$ that represent \underline{all} consumers' tastes. (In previous typical forms, $\bar{\alpha}=e^{\alpha_0}$ and $\bar{\beta}=\beta_0$.) In this case, define $\delta_{jt}:=x_{jt}\bar{\beta}-\bar{\alpha}p_{jt}+\xi_{jt}$.

Take the typical forms of $\alpha_{it}$ and $\beta_{it}$. Let $$\theta:=(\alpha_0,\beta_0,\beta_d,\beta_v)$$
...
\begin{equation}
    ...
    \nonumber
\end{equation}
Note that $\theta$ contains the information of all $\alpha_{it}$ and $\beta_{it}$.
...
\begin{equation}
    \begin{aligned}
        ...
        &:=\hat{s}_{jt}^{RC}\left(\delta_{1t},...,\delta_{J_tt};\theta\right)
    \end{aligned}
    \nonumber
\end{equation}
...
\begin{enumerate}
    ...
    \item Then, for each market $t$, the inversion step described before can be used. We ``invert'' the demand model at the observed market shares $\tilde{s}_{jt}$ to find $\hat{\delta}_{jt}(\theta):=\delta_{jt}(\tilde{s}_{1t},...,\tilde{s}_{J_tt};\theta)$ as \eqref{eq:inversion}. The corresponding unobserved characteristics are $\xi_{jt}(\theta):=\hat{\delta}_{jt}(\theta)-x_{jt}\beta+\alpha p_{jt}$
