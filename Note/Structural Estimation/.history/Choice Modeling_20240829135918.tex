...
2SLS can work in linear models, $\hat{\alpha}_{1,0}=\frac{\widehat{\textnormal{Cov}(q_t,z_t)}}{\widehat{\textnormal{Cov}(p_t,z_t)}}\stackrel{P}{\longrightarrow}\frac{\mathbb{E}[q_t|z_t=1]-\mathbb{E}[q_t|z_t=0]}{\mathbb{E}[p_t|z_t=1]-\mathbb{E}[p_t|z_t=0]}$, but it is not an estimator of a structural parameter.
...
\begin{assumption}
    \begin{enumerate}
        ...
        \item For each value of $z$ and $t$, there is a unique market clearing price,
        \begin{equation}
            \begin{aligned}
                \tilde{p}(z,t) \textnormal{ s.t. }q^d_t(\tilde{p}(z,t))=q^s_t(\tilde{p}(z,t),z)
            \end{aligned}
            \nonumber
        \end{equation}
    \end{enumerate}
