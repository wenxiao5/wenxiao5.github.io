...
\begin{equation}
    ...
    \nonumber
\end{equation}
The diversion ratio is given by
\begin{equation}
    \begin{aligned}
        D_{j \rightarrow k}=\bigg|\frac{\frac{\partial s_{k}}{\partial p_j}}{\frac{\partial s_{j}}{\partial p_j}}\bigg|=\bigg|\frac{\frac{\partial s_{k}}{\partial \delta_j}\frac{\partial \delta_j}{\partial p_j}}{\frac{\partial s_{j}}{\partial \delta_j}\frac{\partial \delta_{j}}{\partial p_j}}\bigg|=
        \left\{\begin{matrix}
...
\begin{assumption}\label{BH2014_Ass1}
...
\begin{definition}
    ...
    \begin{enumerate}
        \item $\sigma_k(\delta_t,p_t)$ is nonincreasing in $\lambda_{jt}$ for all $j>0$, $k\neq j$, $(\delta_t,p_t)\in \mathbb{R}^{2J}$; and
        \item at each $(\delta_t,p_t)\in \textnormal{supp}(\delta_t,p_t)$, for any nonempty $\mathcal{K}\subseteq\{1,...,J\}$ there exist $k\in \mathcal{K}$ and $l\notin \mathcal{K}$ such that $\sigma_l(\delta_t,p_t)$ is strictly decreasing in $\lambda_{kt}$.
    \end{enumerate}
\end{definition}

\begin{assumption}\label{BH2014_Ass2}
    Goods $(0,1,...,J)$ are connected substitutes in $-p_t$ and in $\delta_t$.
\end{assumption}

\paragraph*{Observables} Let $z_t$ denote a vector of instruments excluded from the demand model. In the market data environment, the observables include $\left(M_t,x_t,p_t,s_t,z_t\right)$.


\subsection{Identification of Demand}
Now, we identify the demand system, that is,
\begin{equation}
    \begin{aligned}
        \sigma(\cdot)=\left(\sigma_1(\cdot),..., \sigma_J(\cdot)\right)
    \end{aligned}
    \nonumber
\end{equation}

\begin{lemma}[\cite{berry2013connected}]
    Consider any price vector $p$ and any market share vector $s=(s_1,...,s_J)$ such that $s_j>0$ for all $j$ and $\sum_{j=1}^J s_j<1$. Under Assumptions \ref{BH2014_Ass1} and \ref{BH2014_Ass2}, there is at most one vector $\delta$ such that $\sigma_j(\delta,p)=s_j$ for all $j$.
\end{lemma}
Based on this result, for any $(s_t,p_t)$ in their support we can write
\begin{equation}
    \begin{aligned}
        \sigma_j^{-1}(s_t,p_t)=\delta_{jt}:=x_{jt}+\xi_{jt},\ j=1,...,J,
    \end{aligned}
    \nonumber
\end{equation}
In this section, we choose the normalization $\mathbb{E}[\xi_{jt}]=0,\forall j$.

\begin{assumption}[Assumptions for IV.]\label{BH2014_Ass34}
    \begin{enumerate}
        \item For all $j = 1,...,J$, $\mathbb{E}[\xi_{jt}\mid z_t,x_t]=0$ almost surely.
        \item For all functions $B(s_t,p_t)$ with finite expectation, if $\mathbb{E}[B(s_t,p_t)\mid z_t,x_t]=0$ almost surely, then $B(s_t,p_t)=0$ almost surely.
    \end{enumerate}
\end{assumption}

\begin{theorem}
    Under Assumptions \ref{BH2014_Ass1}, \ref{BH2014_Ass2}, and \ref{BH2014_Ass34}, for all $j=1,...,J$,
    \begin{enumerate}
        \item $\xi_{jt}$ is identified with probability $1$ for all $t$,
        \item and the function $\sigma_j(\chi)$ is identified on $\mathcal{X}$.
    \end{enumerate}
\end{theorem}

\subsection{Identification of Changes in Consumer Welfare}
\begin{assumption}\label{BH2014_Ass5}
    $v_{ijt}=\mu_{ijt}-p_{jt}$, where $(\mu_{i1t},...,\mu_{iJt})$ are independent of $p_t$ conditional on $(x_t,\xi_t)$.
\end{assumption}
Under Assumptions \ref{BH2014_Ass1} and \ref{BH2014_Ass5}, the joint distribution $F_{v}(\cdot\mid \chi_t)$ can be replaced by $F_{\mu}(\cdot\mid\delta_t)$, which is an element of $\mathcal{F}$ satisfying
\begin{enumerate}
    \item $\sigma_0(\delta_t,p_t)=F_{\mu}(p_t\mid\delta_t)$,
    \item $\sigma_j(\delta_t,p_t)=\int \mathbf{1}\left\{\Gamma_j m\leq\Gamma_j p_t\right\}d F_\mu(m\mid \delta_t), j=1,...,J$.
\end{enumerate}
