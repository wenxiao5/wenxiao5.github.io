...
\begin{equation}
    ...
    \nonumber
\end{equation}
Let $\bar{\alpha}$ and $\bar{\beta}$ be the average of $\alpha_{it}$ and $\beta_{it}$ that represent \underline{all} consumers' tastes. (In previous typical forms, $\bar{\alpha}=e^{\alpha_0}$ and $\bar{\beta}=\beta_0$.) In this case, define $\delta_{jt}:=x_{jt}\bar{\beta}-\bar{\alpha}p_{jt}+\xi_{jt}$.
