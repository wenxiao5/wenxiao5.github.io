...
\begin{definition}
    ...
    \begin{enumerate}
        ...
        \begin{equation}
            ...
            \nonumber
        \end{equation}
        Now, using estimated $\hat{\delta}_{jt}$ to calculate sample moment, and find $\alpha$ and $\beta$ by minimizing the sample moment
        \begin{equation}
            \begin{aligned}
...
\begin{enumerate}
    ...
    \item Then, for each market $t$, the inversion step described before can be used. We ``invert'' the demand model at the observed market shares $\tilde{s}_{jt}$ to find $\hat{\delta}_{jt}:=\delta_{jt}(\tilde{s}_{1t},...,\tilde{s}_{J_tt})$ as \eqref{eq:inversion}.
\end{enumerate}
