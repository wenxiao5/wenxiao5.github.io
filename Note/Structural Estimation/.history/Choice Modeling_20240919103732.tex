...
\begin{definition}[Gumbel Distribution]
    \textbf{Gumbel distribution} is also called \textbf{type-I generalized extreme value distribution}, denoted by $\textnormal{Gumbel}(\mu,\beta)$. Its cdf is
    \begin{equation}
        \begin{aligned}
...
\begin{lemma}
    ...
    \begin{enumerate}
        \item The mean is $\mathbb{E}[X]=\mu+\gamma\beta$;
        \item The median is $\mu-\beta\ln (\ln 2)$;
        \item The variance is $\frac{\pi^2}{6}\beta^2$;
        \item If $G_1,...,G_k$ are i.i.d. Gumbel random variables with parameters $(\mu,\beta)$, its maximum is also a Gumbel random variable, $$\max\{G_1,...,G_k\}\sim \textnormal{Gumbel}(\mu+\beta\ln k,\beta)$$
    \end{enumerate}
\end{lemma}
...
\begin{equation}
    ...
    \nonumber
\end{equation}
where $\mathbb{E}[\max_{j\in g}u_{ij}]=\log\left(\sum_{k\in h}\exp\left(\frac{\delta_{ik}}{1-\rho}\right)\right)$.
