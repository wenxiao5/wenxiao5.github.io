...
\begin{equation}
    ...
    \nonumber
\end{equation}
Let $\theta\equiv(\alpha,\beta_0,\beta_v)$, which is partitioned into ``linear parameters'', $\theta_1=(\alpha,\beta_0)$, and ``nonlinear parameters'', $\theta_2=\beta_v$. According to \cite{berry1994estimating}, given any $\theta_2$, there is a $\delta_t=\sigma_{jt}^{-1}\left(s_{jt}; x_t,p_t,\theta_2\right)$ such that $\sigma_{jt}\left(\delta_t,x_t,p_t,\beta_v\right)=s_{jt}$, where $s_{jt}$ is the observed market share.

\paragraph*{Computation of $\delta_t=\sigma_{jt}^{-1}\left(s_{jt}; x_t,p_t,\theta_2\right)$ given $\theta_2$}
The $\delta_t=\sigma_{jt}^{-1}\left(s_{jt}; x_t,p_t,\theta_2\right)$ such that $\sigma_{jt}\left(\delta_t,x_t,p_t,\beta_v\right)=s_{jt}$ is hard to compute. \cite{berry1995automobile} propose to do this iterating:
\begin{enumerate}
