...
\paragraph*{Step 1: Computation of $\delta_t(\theta_2)=\sigma_{t}^{-1}\left(s_t; x_t,p_t,\theta_2\right)$ given $\theta_2$}
The $\delta_t=\sigma_{t}^{-1}\left(s_t; x_t,p_t,\theta_2\right)$ such that $\sigma_{jt}\left(\delta_t,x_t,p_t,\theta_2\right)=s_{jt}$ for all $j$ is hard to compute. \cite{berry1995automobile} propose to do this iterating:
\begin{enumerate}
...
\paragraph*{Step 2: Estimation of $\theta$} Use the $\delta_t$ and estimate $\theta_1$ by linear-GMM (or 2SLS) using the instruments $Z$:
\begin{equation}
    \begin{aligned}
