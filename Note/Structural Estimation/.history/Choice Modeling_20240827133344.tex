...
We follow Berry, Levinsohn, and Pakes (1995) in assuming that each $\epsilon_{ijt}$ is an i.i.d. draw from a standard type-1 extreme value (Gumbel) distribution and that each $v_{it}^{(k)}$ in \eqref{eq:mixed_multinomial_logit} is an i.i.d. draw from a standard normal distribution.
