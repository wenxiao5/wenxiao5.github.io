...
\end{definition}
\begin{claim}[Do symmetry and transitivity imply reflexivity?]
    No. It is tempting to think so, but there may be \underline{elements unrelated to any element}. However if we add the assumption that every element is related to some other element, then reflexivity is redundant.
\end{claim}
...
\subsection{Theorem 1.2.7: any equivalence class forms a unique partition; any partition forms a unqiue equivalence class \small{(@ Lec 01 of ECON 204)}}
...
\begin{theorem}[Properties of Ring]
