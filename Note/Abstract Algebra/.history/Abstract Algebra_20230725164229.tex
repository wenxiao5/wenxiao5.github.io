...
\section{Equivalence Relations (rational equivalence in micro)}
\begin{definition}
\normalfont
    A binary relation $R$ from $X$ to $Y$ is a subset $R \subseteq X\times Y$. We write $xR y$ if $(x, y) \in R$ and “not $xRy$” if $(x, y) \notin R$. $R \subseteq X \times X$ is a binary relation on $X$.
\end{definition}
Given a set $X$, a relation on X is a subset of $R\subset X\times X$. We write $a R b$.\\
A relation $R$ is said to be
\begin{enumerate}
    \item \textit{Reflexive} if $\forall x\in X$, we have $x\sim x$
    \item \textit{Symmetric} if $\forall x,y\in X$, $x\sim y\Rightarrow y\sim x$
    \item \textit{Transitive} if $\forall x,y,z\in X$, $x\sim y, y\sim z\Rightarrow x\sim z$
\end{enumerate}
The $\sim$ is called \textbf{equivalence relation} if it is \textit{reflexive, symmetric} and \textit{transitive}. (which is also the definition of rational equivalence in microeconomics).
...
\section{Partition (separate a set into disjoint sets with no element left)}
...
\section{Relationship of \underline{\textit{Equivalence relation}}, \underline{\textit{Set of equivalence classes}} and \underline{\textit{Partitions}}}
...
\subsection{Theorem 1.2.7: Equivalence relation $\sim$ $\Leftrightarrow$ Set of equivalence classes $X/\sim$; $\{$all Sets of equivalence classes$\}=\{$all Partitions$\}$}
...
\end{example}
\subsection{Proposition 1.2.12: use $X/\sim=\{[x]|x\in X\}$ to infer $\sim_\pi$ equals to $\sim$.}
