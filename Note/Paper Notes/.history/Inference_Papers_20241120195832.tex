...
A decision maker (DM) makes a binary choice $c\in\{0,1\}$ for each individual, which is summarized by characteristics $x\in \mathcal{X}$ and an unknown outcome $y^*\in \mathcal{Y}$ (observable when $c=1$). These three variables are summarized by a joint distribution, $(X,C,Y^*)\sim P(\cdot)$
