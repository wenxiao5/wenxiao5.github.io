...
$$
\vec{\lambda}^*(k)=\left(2 k g_1^{+}\left(\mathbf{x}^*(k)\right), 2 k g_2^{+}\left(\mathbf{x}^*(k)\right), \ldots, 2 k g_m^{+}\left(\mathbf{x}^*(k)\right)\right) .
$$
This is the vector chosen so that we have
$$
\nabla F_k\left(\mathbf{x}^*(k)\right)=\nabla f\left(\mathbf{x}^*(k)\right)+\sum_{j=1}^m \vec{\lambda}^*(k) \nabla g_j\left(\mathbf{x}^*(k)\right)=\nabla_{\mathbf{x}} L\left(\mathbf{x}^*(k), \vec{\lambda}^*(k)\right)
$$
where $L$ is the Lagrangian $L(\mathbf{x}, \vec{\lambda})=f(\mathbf{x})+\vec{\lambda} \cdot \mathbf{g}(\mathbf{x})$.
Because we assumed that $P$ is a convex program, the Lagrangian is a convex function of $\mathbf{x}$ (when $\vec{\lambda}$ is fixed). The equation we've just written down tells us that $\mathbf{x}^*(k)$ is a critical point of $L\left(\mathbf{x}, \vec{\lambda}^*(k)\right)$, and therefore that it is a global minimizer of $L\left(\mathbf{x}, \vec{\lambda}^*(k)\right)$.

In other words, $h\left(\vec{\lambda}^*(k)\right)=L\left(\mathbf{x}^*(k), \vec{\lambda}^*(k)\right)$, where $h$ is the dual objective function mentioned earlier.

This is the "KKT side of things". From the "penalty method side of things", we know that we have this unbounded sequence of penalty factors $k_1, k_2, k_3$ with $\mathbf{x}^*\left(k_i\right)$ converging to the optimal solution $\vec{x}^*$ as $k_i \rightarrow \infty$. For each $k_i$ in the sequence, we have
$$
\begin{aligned}
...
&=f\left(\mathbf{x}^*\left(k_i\right)+\sum_{j=1}^m \vec{\lambda}^*\left(k_i\right) g_j\left(\mathbf{x}^*\left(k_i\right)\right)\right.\\
&=L\left(\mathbf{x}^*\left(k_i\right), \vec{\lambda}^*\left(k_i\right)\right)=h\left(\vec{\lambda}^*\left(k_i\right)\right) .
\end{aligned}
$$
Now we have conflicting information about this value. On the one hand, from the KKT side of things, we have $h\left(\vec{\lambda}^*\left(k_i\right)\right) \leq M D$, because $M D$ is an upper bound on $h$ and $h\left(\vec{\lambda}^*\left(k_i\right)\right)$ is just some value of $h$. On the other hand, from the penalty method side of things, we know that $f\left(\mathbf{x}^*\left(k_i\right)\right)$ converges to $f\left(\vec{x}^*\right)=M P$ as $k_i \rightarrow \infty$.
