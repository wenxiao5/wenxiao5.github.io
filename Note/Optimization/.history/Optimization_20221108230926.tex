...
\begin{proof}
    ...
    Let $\vec{y}$ be a point satisfying $\mathbf{g}(\vec{y}) \leq \mathbf{0}$. Then all the global minimizers $\vec{x}^*(k)$ are contained in the sublevel set $S=\{\vec{x}: f(\vec{x}) \leq f(\vec{y})\}$, because points outside $S$ will have a value of $F_k$ bigger than $\vec{y}$ does.
