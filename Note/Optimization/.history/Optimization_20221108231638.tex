...
\end{theorem}
\begin{proof}
    Let $S$ be the set $\left\{\mathbf{x} \in \mathbb{R}^n:\|\mathbf{x}\|=1\right\} . S$ is closed and bounded, so $f(\mathbf{x})$ has a global minimizer $\mathbf{x}^*$ on $S$. Let $\alpha=f\left(\mathbf{x}^*\right)$.

    Fun fact: actually $\alpha$ is the smallest eigenvalue of $A$. But we don't need to know that. All we need to know is that $\alpha=\mathbf{x}^{* \top} A \mathbf{x}^*>0$, because $A$ is positive definite, and that for all $\mathbf{x}$ with $\|\mathbf{x}\|=1$, $f(\mathbf{x}) \geq \alpha$.
    Now take an arbitrary $\mathbf{x} \in \mathbb{R}^n, \mathbf{x} \neq \mathbf{0}$. We have
    $$
    f(\mathbf{x})=\mathbf{x}^{\top} A \mathbf{x}=\left(\frac{\mathbf{x}}{\|\mathbf{x}\|}\right)^{\top} A\left(\frac{\mathbf{x}}{\|\mathbf{x}\|}\right) \cdot\|\mathbf{x}\|^2 \geq \alpha \cdot\|\mathbf{x}\|^2 .
    $$
    We can show that $\alpha\|\mathbf{x}\|^2$ is a coercive function, because the sublevel set $\left\{\mathbf{x} \in \mathbb{R}^n: \alpha\|\mathbf{x}\|^2 \leq c\right\}$ is the disk around $\mathbf{0}$ of radius $\sqrt{\frac{c}{\alpha}}$, which is bounded.
    
    Since $f(\mathbf{x}) \geq \alpha\|\mathbf{x}\|^2$ for all $\mathbf{x}$ (including $\mathbf{0}$, because both functions are 0 there), we know that $f$ is a coercive function as well.
    
    In fact, this condition goes both ways: if $A$ is a symmetric matrix, then $\mathbf{x}^{\top} A \mathbf{x}$ is only coercive when $A$ is positive definite. If not, then we can find a nonzero $\mathbf{y}$ for which $\mathbf{y}^{\top} A \mathbf{y} \leq 0$; for any scalar $t$, we'll have $(t \mathbf{y})^{\top} A(t \mathbf{y}) \leq 0$, so the sublevel set $\left\{\mathbf{x}: \mathbf{x}^{\top} A \mathbf{x}\right\}$ will contain an entire unbounded line $\{t \mathbf{y}: t \in \mathbb{R}\}$.
\end{proof}
