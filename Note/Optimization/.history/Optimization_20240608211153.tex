...
\begin{definition}[Convex Set]
    \underline{\textbf{Convex set}} $C: x, y \in C$ implies $\lambda x+(1-\lambda) y \in C$, for any $\lambda \in[0,1]$.\\
...
\subsection{Jensen's Inequality: $f\left(\frac{\sum_{i=1}^k\lambda_ix_i}{\sum_{j}\lambda_j}\right)\leq \frac{\sum_{i=1}^k\lambda_if(x_i)}{\sum_{j}\lambda_j}$ $\bigg/$ $f(\mathbb{E}[x])\leq \mathbb{E}[f(x)]$}
...
\subsection{AM-GM inequality: $\frac{x_1+x_2+...+x_n}{n}\geq \sqrt[n]{x_1x_2...x_n}$}
...
\subsection{Weighted AM-GM inequality: $\sum_{i=1}^n\delta_ix_i\geq \prod_{i=1}^nx_i^{\delta_i}$}
\begin{theorem}[Weighted AM-GM inequality]
    For any $x_1,x_2,...,x_n\geq 0$ with $\delta_1,\delta_2,\cdots,\delta_n>0$ with $\delta_1+\cdots+\delta_n=1$, $$\delta_1x_1+\delta_2x_2+\cdots+\delta_nx_n\geq x_1^{\delta_1}x_2^{\delta_2}\cdots x_n^{\delta_n}$$
...
\chapter{The Closest Point: Projection}
\section{Set Theory Basis}
...
\section{\underline{Unique} projection $[z]^\&$ on \underline{closed convex} subset of $\mathbb{R}^n$}
\begin{proposition}
...
\section{Obtuse Angle Criterion: $[z]^\&$ is projection $\Leftrightarrow$ $(z-[z]^\&)^T(y-[z]^\&)\leq 0, \forall y\in\&$}
...
\begin{theorem}[Obtuse angle criterion, Necessary and Sufficient Condition for Projection]
    Let $C$ be a convex set and let $\vec{y}$ be a point outside $C$. $\vec{x}^*$ is the closest point (\textbf{projection}) in $C$ to $\vec{y}$ \underline{if and only if} $$(\vec{y}-\vec{x}^*)\cdot (\vec{x}-\vec{x}^*)\leq 0,\quad \forall \vec{x}\in C$$
\end{theorem}
\begin{center}\begin{figure}[htbp]
    \centering
...
\end{figure}\end{center}
\begin{proof}
    $\vec{x}^* = \argmin_{\vec{x}\in\ C}g(\vec{x})$, with $g(\vec{x})=\|\vec{x}-\vec{y}\|^2$ (which is strictly convex), $\nabla g(\vec{x})=2(\vec{x}-\vec{y})$.

    By the optimality conditions,
    \begin{equation}
        \begin{aligned}
            &\vec{x}^*\text{ is the minimizer of $g(\vec{x})$ over $C$}\\
            \Leftrightarrow	& \nabla g(\vec{x}^*)^T(\vec{x}-\vec{x}^*)\geq 0,\quad \forall \vec{x}\in C\\
            \Leftrightarrow & (\vec{y}-\vec{x}^*)\cdot (\vec{x}-\vec{x}^*)\leq 0,\quad \forall \vec{x}\in C
        \end{aligned}
        \nonumber
    \end{equation}
\end{proof}
...
\subsection{Special Case (Linear subspace): Orthogonality Principle $(z-[z]^\&)^Tx= 0,\forall x\in\&$}
...
\section{Prop: Projection is Non-expansive $\|[x]^\&-[z]^\&\|\leq \|x-z\|,\forall x,z\in \mathbb{R}^n$}
\begin{proposition}
...
\section{Separating Hyperplane Theorem \small{(Lec 13 @ Econ 204)}}
\begin{theorem}[Separation Theorem]
    If $C\subseteq \mathbb{R}^n$ is a closed convex set and $\vec{y}\notin C$, then there is a linear inequality separating $\vec{y}$ from $C$:
    \begin{enumerate}[$\bullet$]
        \item $\exists$ $\vec{a}\in \mathbb{R}^n, b\in \mathbb{R}$ s.t. $\vec{a}\cdot \vec{x}\leq b$ for all $\vec{x}\in C$, but $\vec{a}\cdot \vec{y}> b$
        \item (Equivalent:) $\exists$ $\vec{a}\in \mathbb{R}^n$ s.t. $\vec{a}\cdot \vec{x}< \vec{a}\cdot \vec{y},\ \forall \vec{x}\in C$
    \end{enumerate}
\end{theorem}
...
\begin{theorem}[Separating Hyperplane Theorem]
    Let $X, Y \subseteq \mathbb{R}^n$ be nonempty, disjoint, convex sets. Then there exists a nonzero vector $\vec{p}\in \mathbb{R}^n$ such that
    \begin{equation}
        \begin{aligned}
            \vec{p}\cdot \vec{x}\leq \vec{p}\cdot \vec{y},\ \forall \vec{x}\in X,\vec{y}\in Y
        \end{aligned}
        \nonumber
    \end{equation}
\end{theorem}

\begin{theorem}[Strict Separating Hyperplane Theorem]
    Let $X, Y \subseteq \mathbb{R}^n$ be nonempty, disjoint, convex sets with $X$ closed and $Y$ compact. Then there exists a nonzero vector $\vec{p}\in \mathbb{R}^n$ such that
    \begin{equation}
        \begin{aligned}
            \vec{p}\cdot \vec{x}< \vec{p}\cdot \vec{y},\ \forall \vec{x}\in X, \vec{y}\in Y
        \end{aligned}
        \nonumber
    \end{equation}
\end{theorem}
...
\subsection{Extreme Value Theorem: continuous $f$, compact set $\rightarrow \mathbb{R}$ has global-min}
\begin{corollary}[Extreme value theorem]
...
\subsection{Support Theorem: $z\in bd(C)$, $\exists \vec{u}$ s.t. $\vec{u}\cdot \vec{x}\leq \vec{u}\cdot \vec{z}, \forall \vec{x}\in C$}
...
\begin{definition}[General Form of Constrained Optimization]
    A nonlinear program $P$
    \begin{equation}
        \begin{aligned}
...
\begin{definition}[Convex Program]
    A nonlinear program $P$ is a \textbf{convex program} if
    \begin{enumerate}
        \item The set $S$ is convex.
        \item All the function $f,g_1,...,g_m$ are convex.
    \end{enumerate}
\end{definition}

\section{Perturbation of Constraints}
...
\subsection{Theorem: convex program $\Rightarrow$ $MP(\vec{z})$ is a convex function on convex domain}
\begin{theorem}
...
\subsection{Slater's condition: $P$ is super-consistent if $\exists \vec{x}^*\in S$ s.t. $\vec{g}(\vec{x}^*)<\vec{0}$}
...
\subsection{Lemma: Convex $P$ is super-consistent $\Rightarrow$ $\exists$ sensitivity vector $\vec{\lambda}\geq 0$ s.t. $MP(\vec{z})\geq MP(\vec{0})-\vec{\lambda}\cdot \vec{z},\forall \vec{z}$}
\begin{definition}[Sensitivity Vector]
    A vector $\vec{\lambda}\in \mathbb{R}^m$ with $\vec{\lambda}\geq \vec{0}$ satisfying $$MP(\vec{z})\geq MP(\vec{0})-\vec{\lambda}\cdot \vec{z},\quad \forall \vec{z}$$ is called a \textbf{sensitivity vector}: it measures the sensitivity of $P$ to changes in the constraints.
\end{definition}
\begin{lemma}[Sensitivity vector lemma]
        Let $P$ be a convex program that is \textbf{super-consistent}, then there exists \textbf{sensitivity vector} $\vec{\lambda}\in \mathbb{R}^m$.
\end{lemma}
\begin{proof}
...
\subsection{Goalposts Lemma: $\vec{x}^{(0)}$ is feasible for $P(\vec{g}(\vec{x}^{(0)}))$ and $MP(\vec{g}(\vec{x}^{(0)}))\leq f(\vec{x}^{(0)})$}
\begin{lemma}[“Moving the goalposts” lemma]
...
\subsection{KKT Theorem: Saddle Point Form}
\begin{definition}
...
\begin{theorem}[Karush-Kuhn-Tucker theorem, saddle point form]
...
\subsection{KKT Theorem: Gradient Form}
\begin{theorem}[Karush-Kuhn-Tucker theorem, gradient form]
        Let P be any nonlinear program where $f$ and $g_1,...,g_m$ have continuous first partial derivatives. Suppose that $\vec{x}^* \in int (S)$ is an optimal solution of $P$, and $\vec{\lambda}^* \geq 0$ is a sensitivity vector. Then:$\Rightarrow$
        \begin{enumerate}[(1)]
...
\subsection{Relationship between KKT and Optimal Solution}
\begin{center}\begin{figure}[htbp]
    \centering
...
\subsection{KKT Duality: If $P$ has optimal solution $\vec{x}^*$ and sensitivity vector $\vec{\lambda}^*$ $\Rightarrow h(\vec{\lambda^*})=f(\vec{x}^*)$}
\begin{definition}[Dual Objective Function]
    $h(\vec{\lambda})=\inf_{\vec{x}\in S}\{L(\vec{x},\vec{\lambda})\}$
\end{definition}
...
\begin{definition}
    \begin{enumerate}
        ...
        \item When we \underline{do} have a sensitivity vector, then \textbf{strong duality} holds, and we have the equality $f(\vec{x})=h(\vec{\lambda})$ for some primal feasible $\vec{x}$ and dual feasible $\vec{\lambda}$ - such $\vec{x}$ and $\vec{\lambda}$ are optimal.
    \end{enumerate}
\end{definition}
...
\begin{definition}
    ...
    \end{equation}
\end{definition}
\subsection{KKT with equality constraints}
...
\subsection{Penalty method with equality constraints}
\begin{equation}
    \begin{aligned}(P')\quad
        \left\{\begin{matrix}
        \min_{\vec{x}\in \mathbb{R}^n}& f(\vec{x})\\
...
\subsection{Super-consistent with equality constraintes}
\begin{definition}[Super-consistent with equality constraintes]
...
\chapter{Constrained Geometric Programming}
\section{Standard Form of Geometric Programming}
\begin{equation}
...
\section{Dual Geometric Program}
...
\begin{proposition}[Dual Geometric Program]
    The \textbf{dual objective function} $v(\vec{\delta})$ is the product of:
    \begin{enumerate}[$(1)$]
    ...
    \end{enumerate}
    The dual problem has the following \textbf{constraints}:
    \begin{enumerate}[$(1)$]
...
\begin{proposition}
    \begin{enumerate}[(1)]
        \item Terms in the \textbf{objective function.} Given $\vec{\delta}^*$ we can solve $\vec{t}^*$ by solving the system of equations:
        \begin{equation}
            \begin{aligned}
            ...
            \nonumber
        \end{equation}
        \item Terms appearing in \textbf{active constraints.} Every primal constraint whose \underline{dual variables are positive} is an active constraint: the value of the left-hand side is not just at most 1 but equal to 1. For all such constraints, we have
        $$
...
\section{Equality constraints in geometric programming}
We can replace one equation by two inequalities.
\begin{example}
    One more trick that can make our geometric programs more flexible,
    \begin{equation}
        \begin{aligned}
            \min_{x,y\in \mathbb{R}}\quad& \sqrt{x^2+y^2}+xy\\
            s.t.\quad &\frac{1}{x}+\frac{1}{y}\leq 1\\
            &x,y>0
        \end{aligned}
        \nonumber
    \end{equation}
    We can replace $\sqrt{x^2+y^2}$ by $z$. We can ask $z\geq \sqrt{x^2+y^2}$ because $z$ will achieve its lower bound when minimizing.
    \begin{equation}
        \begin{aligned}
            \min_{x,y\in \mathbb{R}}\quad& z+xy\\
            s.t.\quad &\frac{1}{x}+\frac{1}{y}\leq 1\\
...
\section{The Courant-Beltrami penalty function}
...
\subsection{Guarantees assuming convergence}
...
\begin{theorem}[If convergence, it is optimal]
...
\subsection{Guaranteeing convergence: (1)feasible $P$; (2)continuous $g_i$; (3)coercive $f$}
\begin{theorem}[For some problems, exists optimal]
...
\section{The Penalty Method and KKT Duality}
\subsection{Thm: (1)feasible $P$; (2)continuous $g_i$; (3)coercive $f$ $\Rightarrow MP=MD$}
\begin{theorem}
...
\chapter{Optimization with Equality Constraints}
\section{Basic}
\begin{align*}
    &\min\quad f(x)\\
    &\begin{array}{r@{\quad}r@{}l@{\quad}l}
...
\section{Lagrange Mutiplier Theorem}
\subsection{First-order necessary condition: $\exists\lambda, \nabla f(x^*)+\sum_{i=1}^m\lambda_i \nabla h_i(x^*)=0$}
...
\subsection{Second-order necessary condition: $z^T\left(\nabla^2 f(x^*)+\sum_{i=1}^m\lambda_i \nabla^2 h_i(x^*)\right)z\geq 0,\forall z\in V(x^*)$}
\begin{theorem}
...
\subsection{Sensitivity Analysis $f(x^*(u))=f(x^*)-\lambda^Tu+O(\|u\|)$}
...
\chapter{Optimization with Inequality Constraints}
\section{Basic}
...
\begin{claim}
    If $x^*$ is a local min for ICP, then $x^*$ is also a local min for ECP:
    \begin{align*}
        &\min\quad f(x)\\
        &\begin{array}{r@{\quad}r@{}l@{\quad}l}
        s.t.
...
\section{Karush–Kuhn–Tucker (KKT) Necessary Conditions}
...
\section{Karush–Kuhn–Tucker (KKT) Sufficient Conditions}
\begin{proposition}[KKT]
...
\section{General Sufficiency Condition}
...
\section{An Example Using KKT or Barrier}
\begin{example}
\begin{equation}
    \begin{aligned}
...
\begin{equation}
    ...
    \nonumber
\end{equation}
\subsection*{\underline{Dual Function}}
...
\section{Weak Duality Theorem: $\max_{(\lambda,\mu)\in G}D(\lambda,\mu)\leq \min_{x\in F}f(x)$}
...
\section{Strong Duality Theorem: under some conditions, $\max_{(\lambda,\mu)\in G}D(\lambda,\mu)= \min_{x\in F}f(x)$}
...
\subsection{Slater's sufficient condition for strong duality}
...
\section{Dual of Linear Program}
\begin{center}
...
\end{equation}
\begin{center}
    \fcolorbox{black}{gray!10}{\parbox{.9\linewidth}{Hence, the dual problem is:\begin{equation}
        \begin{aligned}
            &\min\ \bar{x}^Tb\\
...
\chapter{Strongly Convexity}
\section{$\mu$-Strongly Convex: $
...
\section{Lemma: Strongly convexity $\Rightarrow$ Strictly convexity}
\begin{lemma}
...
\chapter{Lipschitz Gradient ($L$-Smooth)}
...
\section{Descent Lemma: $f$ is $L$-smooth $\Rightarrow$ $f(y)\leq f(x)+\nabla f(x)^T(y-x)+\frac{L}{2}\|y-x\|^2$}
...
\section{Co-coercivity Condition: $(\nabla f(x)-\nabla f(y))^T(x-y)\geq \frac{1}{L}\|\nabla f(x)-\nabla f(y)\|^2$}
