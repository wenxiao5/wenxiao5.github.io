...
\subsubsection{Thm: If $P$ has optimal solution $\vec{x}^*$ and sensitivity vector $\vec{\lambda}^*$, $h(\vec{\lambda})=\inf_{\vec{x}\in S}\{L(\vec{x},\vec{\lambda})\}\leq h(\vec{\lambda^*})=f(\vec{x}^*)$}
