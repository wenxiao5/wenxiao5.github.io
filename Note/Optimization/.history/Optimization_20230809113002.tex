...
\begin{theorem}[Separating Hyperplane Theorem]
    Let $A, B \subseteq \mathbb{R}^n$ be nonempty, disjoint convex sets. Then there exists a nonzero vector $\vec{p}\in \mathbb{R}^n$ such that
    \begin{equation}
        \begin{aligned}
            \vec{p}\cdot \vec{a}\leq \vec{p}\cdot \vec{b},\ \forall \vec{a}\in A,\vec{b}\in B
        \end{aligned}
        \nonumber
    \end{equation}
\end{theorem}
