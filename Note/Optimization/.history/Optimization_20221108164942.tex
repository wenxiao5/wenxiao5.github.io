...
In fact, when the penalty method (with Courant-Beltrami penalty function) works, it works exactly that way - it give us the right answer in the limit, i.e., as $k \rightarrow \infty$, we'll get $\vec{x}_k^* \rightarrow \vec{x}^*$ for some feasible $\vec{x}^* \in \mathbb{R}^n$.
