...
\begin{theorem}
    Suppose that $P$ is feasible, $g_1, g_2, \ldots, g_m$ are continuous, and $f$ is coercive. Then there is some unbounded sequence $k_1<k_2<k_3<\cdots$ with global minimizers $\vec{x}^*\left(k_1\right), \vec{x}^*\left(k_2\right), \ldots$ that converge to $\vec{x}^*$ (which is an optimal solution of $P$ by Theorem 2.1).
\end{theorem}
