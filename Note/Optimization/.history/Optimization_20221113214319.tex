...
\begin{lemma}[Sensitivity vector lemma]
        Let $P$ be a convex program that is super-consistent, then there exists some a \textbf{sensitivity vector} $\vec{\lambda}\in \mathbb{R}^m$.
\end{lemma}
\begin{proof}
...
\subsection{Thm: If $P$ has optimal solution $\vec{x}^*$ and sensitivity vector $\vec{\lambda}^*$, $h(\vec{\lambda})=\inf_{\vec{x}\in S}\{L(\vec{x},\vec{\lambda})\}\leq h(\vec{\lambda^*})=f(\vec{x}^*), \forall \vec{\lambda}$}
