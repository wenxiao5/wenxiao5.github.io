...
\begin{center}
    \fcolorbox{black}{gray!10}{\parbox{.9\linewidth}{There is some flexibility here. For any $C>0$, we can deal with the constraint $g(\mathbf{t}) \leq C$, by rewriting it as $C^{-1} g(\mathbf{t}) \leq 1$

    But this cannot be done with a negative $C$, because the coefficients in a posynomial cannot be positive. Similarly, we cannot add the constraint $g(\mathbf{t}) \geq 1$ to a geometric program: there is no way to put that constraint in standard form. (We can't rewrite it as $-g(\mathbf{t}) \leq-1$ : this is not a posynomial constraint!) Equality constraints are equally impossible.
    
    There is one exception. Suppose that $g(\mathbf{t})$ is a posynomial with only one term. (A posymonomial??) Then we can write down the constraint $g(\mathbf{t}) \geq 1$, by taking the reciprocal of both sides and writing down $\frac{1}{g(\mathbf{t})} \leq 1$. And together, the two constraints $\frac{g(\mathbf{t})}{C} \leq 1$ and $\frac{C}{g(\mathbf{t})} \leq 1$ encode an equality constraint $g(\mathbf{t})=C$.}}
\end{center}
