...
$$
F_k(\mathbf{x})=f(\mathbf{x})+k \sum_{j=1}^m\left[g_j^{+}(\mathbf{x})\right]^2=f(\mathbf{x})+k \sum_{j=1}^m\left[\max \left\{0, g_j(\mathbf{x})\right\}\right]^2
$$
we know that for all $k>0, F_k(\mathbf{x})$ has an global minimizer $\mathbf{x}^*(k)$, and that there is an unbounded sequence $k_1<k_2<k_3$ so that as $k_i \rightarrow \infty, \mathbf{x}^*\left(k_i\right) \rightarrow \mathbf{x}^*$ for some $\mathbf{x}^*$.
(That $\mathrm{x}^*$ turns out to be the optimal solution of $P$.)
...
In particular, if $F_k$ has a global minimizer, it has a critical point. At $\mathbf{x}^*(k)$, we have $\nabla F_k\left(\mathbf{x}^*(k)\right)=0$. That is,
$$
\nabla F_k\left(\mathbf{x}^*(k)\right)=\nabla f\left(\mathbf{x}^*(k)\right)+\sum_{j=1}^m 2 k g_j^{+}\left(\mathbf{x}^*(k)\right) \nabla g_j\left(\mathbf{x}^*(k)\right)=\mathbf{0} .
$$
Relating this back to the KKT theorem: let
$$
\boldsymbol{\lambda}^*(k)=\left(2 k g_1^{+}\left(\mathbf{x}^*(k)\right), 2 k g_2^{+}\left(\mathbf{x}^*(k)\right), \ldots, 2 k g_m^{+}\left(\mathbf{x}^*(k)\right)\right) .
$$
This is the vector chosen so that we have
$$
\nabla F_k\left(\mathbf{x}^*(k)\right)=\nabla f\left(\mathbf{x}^*(k)\right)+\sum_{j=1}^m \boldsymbol{\lambda}^*(k) \nabla g_j\left(\mathbf{x}^*(k)\right)=\nabla_{\mathbf{x}} L\left(\mathbf{x}^*(k), \boldsymbol{\lambda}^*(k)\right)
$$
where $L$ is the Lagrangian $L(\mathbf{x}, \boldsymbol{\lambda})=f(\mathbf{x})+\boldsymbol{\lambda} \cdot \mathbf{g}(\mathbf{x})$.
Because we assumed that $P$ is a convex program, the Lagrangian is a convex function of $\mathbf{x}$ (when $\boldsymbol{\lambda}$ is fixed). The equation we've just written down tells us that $\mathbf{x}^*(k)$ is a critical point of $L\left(\mathbf{x}, \boldsymbol{\lambda}^*(k)\right)$, and therefore that it is a global minimizer of $L\left(\mathbf{x}, \boldsymbol{\lambda}^*(k)\right)$.

In other words, $h\left(\boldsymbol{\lambda}^*(k)\right)=L\left(\mathbf{x}^*(k), \boldsymbol{\lambda}^*(k)\right)$, where $h$ is the dual objective function mentioned earlier.

This is the "KKT side of things". From the "penalty method side of things", we know that we have this unbounded sequence of penalty factors $k_1, k_2, k_3$ with $\mathbf{x}^*\left(k_i\right)$ converging to the optimal solution $\mathrm{x}^*$ as $k_i \rightarrow \infty$. For each $k_i$ in the sequence, we have
$$
\begin{aligned}
f\left(\mathbf{x}^*\left(k_i\right)\right) & \leq f\left(\mathbf{x}^*\left(k_i\right)\right)+2 k_i \sum_{j=1}^m\left[g_j^{+}\left(\mathbf{x}^*\left(k_i\right)\right)\right]^2 \\
&=f\left(\mathbf{x}^*\left(k_i\right)\right)+\sum_{j=1}^m 2 k_i g_i^{+}\left(\mathbf{x}^*\left(k_i\right)\right) g_j\left(\mathbf{x}^*\left(k_i\right)\right) \\
&=f\left(\mathbf{x}^*\left(k_i\right)+\sum_{j=1}^m \boldsymbol{\lambda}^*\left(k_i\right) g_j\left(\mathbf{x}^*\left(k_i\right)\right)\right.\\
&=L\left(\mathbf{x}^*\left(k_i\right), \boldsymbol{\lambda}^*\left(k_i\right)\right)=h\left(\boldsymbol{\lambda}^*\left(k_i\right)\right) .
\end{aligned}
$$
Now we have conflicting information about this value. On the one hand, from the KKT side of things, we have $h\left(\boldsymbol{\lambda}^*\left(k_i\right)\right) \leq M D$, because $M D$ is an upper bound on $h$ and $h\left(\boldsymbol{\lambda}^*\left(k_i\right)\right)$ is just some value of $h$. On the other hand, from the penalty method side of things, we know that $f\left(\mathbf{x}^*\left(k_i\right)\right)$ converges to $f\left(\mathrm{x}^*\right)=M P$ as $k_i \rightarrow \infty$.
