...
\end{figure}\end{center}
\begin{proof}
    $\vec{x}^* = \argmin_{\vec{y}\in\ C}g(\vec{y})$, with $g(\vec{y})=\|\vec{x}-\vec{y}\|^2$ (which is strictly convex), $\nabla g(\vec{y})=2(\vec{y}-\vec{x})$.

    By the optimality conditions,
    \begin{equation}
        \begin{aligned}
            &\vec{x}^*\text{ is the minimizer of $g(\vec{y})$ over $C$}\\
            \Leftrightarrow	& \nabla g(\vec{x}^*)^T(\vec{y}-\vec{x}^*)\geq 0\quad \forall \vec{y}\in\&\\
