...
\begin{center}
    \fcolorbox{black}{blue!10}{\parbox{.9\linewidth}{}}
\end{center}
\begin{theorem}
    ...
    Additionally, suppose that as $k \rightarrow \infty$, $\vec{x}^*(k) \rightarrow \vec{x}^*$ for some $\vec{x}^*\in \mathbb{R}^n$. (If there are multiple global minimizers of $F_k$, we assume that $\vec{x}^*(k)$ is one of them, chosen to have this convergence.)
\end{theorem}
