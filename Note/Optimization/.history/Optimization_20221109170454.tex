...
\begin{definition}
Suppose $P$ is convex: $S$ is a convex set; the functions $f,g_1,...,g_m$ are all convex; and the functions $h_1,...,h_l$ are all linear (potential with constant terms). Then $P$ is superconsistent if there exists $\vec{x}\in S$ with $g_i(\vec{x})<0$ for all $i=1,...,m$.
\end{definition}
