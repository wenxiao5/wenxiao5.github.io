...
The dual objective function $v(\vec{\delta})$ is the product of:
...
- There is a positivity constraint $\vec{\delta}>\mathbf{0}$. It has an exception: for each constraint, we are allowed to set all dual variables from that constraint to 0 simultaneously. (For the purposes of evaluating $v(\vec{\delta})$, we assume that $0^0=1$ in this case.)
