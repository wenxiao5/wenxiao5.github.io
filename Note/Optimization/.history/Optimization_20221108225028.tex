...
\subsubsection{Lemma: sum of coercive functions is a coercive function}
\begin{lemma}
    If $f_1,f_2,...,f_n$ are coercive functions $\mathbb{R} \rightarrow  \mathbb{R}$, then $f(\vec{x})=\sum_{i=1}^n f_i(x_i)$ is coercive.
\end{lemma}

\subsubsection{Lemma: sum of coercive function and bounded below function is a coercive function}
\begin{lemma}
...
\begin{theorem}
    Suppose that $P$ is feasible, $g_1, g_2, \ldots, g_m$ are continuous, and $f$ is coercive. Then there is some unbounded sequence $k_1<k_2<k_3<\cdots$ with global minimizers $\vec{x}^*\left(k_1\right), \vec{x}^*\left(k_2\right), \ldots$ that converge to $\vec{x}^*$ (which is an optimal solution of $P$ by former theorem).
\end{theorem}
