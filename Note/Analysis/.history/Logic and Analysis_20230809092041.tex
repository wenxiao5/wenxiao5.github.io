...
\begin{theorem}
    Let $X \subseteq \mathbb{R}^n$ and $A \subseteq \mathbb{R}^p$ be open, and $f : X \times A \rightarrow \mathbb{R}^m$ be $C^r$ with $r \geq 1 + max\{0, n - m\}$. Suppose that 0 is a regular value of f. Then there is a set A0 ⊆ A such that A \ A0 has Lebesgue measure zero and for all a ∈ A0, 0 is a regular value of fa = f(·, a).
\end{theorem}
