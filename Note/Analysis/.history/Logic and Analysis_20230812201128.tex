...
\begin{proof}
    ...
    \begin{enumerate}
        ...
        \item Assume $C\subseteq \Psi(C)$ doesn't hold, that is $\exists y\in C$ s.t. $y\notin \Psi(C)$. Because $y\notin \Psi(C)$, $y\notin \Psi(c),\forall c\in C$. Because $C=\cap_{i=0}^\infty C_i$, $C\subseteq C_n$ for all $n$.
        \item $\forall y\in C$, $y\in C_n$ for all $n$. Because $y\in C$ and $C_0\supseteq C_1\supseteq \cdots C_n\supseteq \cdots$, for all $n$, there exists $k\in C_n$ s.t. $y\in\Psi(k)$.
    \end{enumerate}
\end{proof}
