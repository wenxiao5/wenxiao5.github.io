...
\begin{theorem}[Inverse Function Theorem]
    ...
    \begin{equation}
        \begin{aligned}
            ...
            f^{-1}:V &\rightarrow U \textnormal{ is }C^1\\
            Df^{-1}(f(x_0))&=[Df(x_0)]^{-1}
        \end{aligned}
        \nonumber
    \end{equation}
...
\begin{theorem}[Brouwer's Fixed Point Theorem]
    Let $X \subseteq \mathbb{R}^n$ be nonempty, compact, and convex, and let $f : X \rightarrow X$ be continuous. Then $f$ has a fixed point.
\end{theorem}
\begin{proof}
    \normalfont
    Using a fact that "Let $B$ be the unit ball in $\mathbb{R}^n$. Then there is no continuous function $h : B \rightarrow \partial B$ such that $h(x_0) = x_0$ for every $x_0 \in \partial B$", which is hard to prove. (See \textit{J. Franklin, Methods of Mathematical Economics}, for an elementary (but long) proof.)
\end{proof}
