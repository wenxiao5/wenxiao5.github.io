...
\subsection{Contraction of modulus $\beta$: $d(T(x), T(y)) \leq \beta d(x, y)$}
\begin{definition}
    \normalfont
...
\subsection{$\bigstar$ Contraction Mapping Theorem: contraction with modulus $\beta < 1$ $\Rightarrow$ exist fixed point}
...
\subsection{$\bigstar$ Brouwer's Fixed Point Theorem: continuous function over compact, convex set $\rightarrow$ exist fixed point}
