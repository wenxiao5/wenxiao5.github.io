...
\end{theorem}
\begin{proof}
    Instead, we look for a weaker type of approximation. Let $X \subseteq \mathbb{R}^n$ be a non-empty, compact, convex set, and let $\psi : X \rightarrow 2^X$ be an uhc correspondence with non-empty, compact, convex values. For every $\varepsilon > 0$, define the $\varepsilon$ ball about graph $\psi$ to be
\end{proof}
