...
\begin{theorem}
    ...
    y^{\prime}=\left(y-y_s\right)^{\prime}=M\left(y-y_s\right)
    $$
    where M is a real $n\times n$ matrix. Suppose that M can be diagonalized over the complex field $\mathbb{C}$. Let $U$ be the standard basis of Rn and $V = \{v1, . . . , vn\}$ be a basis of (complex) eigenvectors corresponding to the eigenvalues $\lambda_1, . . . , \lambda_n \in \mathbb{C}$. Then the solution of the initial value problem is given by
    \begin{equation}
        \begin{aligned}
...
We can determine the form of the real solutions once we know the eigenvalues. In an important special case, we can solve for the solution of the initial value problem without calculating the diagonalization, as in Equation .
