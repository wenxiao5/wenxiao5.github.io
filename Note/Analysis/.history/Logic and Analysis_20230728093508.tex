...
\begin{proof}
    ...
    \begin{enumerate}[$\circ$]
        \item If $\inf_{x\in\mathbb{X}} f(x)$ is finite, let $\inf_{x\in\mathbb{X}} f(x) \leq f\left(\sigma_{k}\right) \leq \inf _{x \in \mathbb{X}} f(x)+\frac{1}{k}$.
        \item If $\inf_{x\in\mathbb{X}} f(x)$ is infinite, let $f\left(\sigma_{k}\right) \leq -k$. By the Bolzano-Weierstrass Theorem (\ref{B-W}), $\{\sigma_k\}$ contains a subsequence $\{\sigma_{k_n}\}$ s.t. $\lim _{n \rightarrow \infty} f\left(\sigma_{k_n}\right)=\inf _{x \in \mathbb{X}} f(x}\}$
    \end{enumerate}
