...
\begin{definition}
    \normalfont
    A collection of sets $\mathcal{U} = \{U_\lambda : \lambda \in \Lambda\}$ in a metric space $(X, d)$ is an open cover of $A\subseteq X$ if $U_\lambda$ is open for all $\lambda \in \Lambda$ and $A\subseteq \cup_{\lambda\in\Lambda}U_\lambda$
\end{definition}
