...
\begin{theorem}[Continuous Images of Compact Set is Compact]
...
\subsection{Weierstrass' Theorem (Extreme value Theorem) \small{(@ Lec 05/06 of ECON 204)}}
\begin{theorem}
    [Weierstrass' Theorem (Extreme value Theorem)]
    Given a \textbf{compact set} $C$ in a metric space $(X, d)$ and a \textbf{continuous} function $f: C \rightarrow \mathbb{R}$, then $f$ is bounded on $C$ and attains its min and max on $C$ i.e.,
    $$
    \begin{aligned}
    \exists x_1 \in C \text { s.t. } f\left(x_{1}\right) &=\inf _{x \in C} f(x) \\
    \exists x_{2} \in C \text { s.t. } f\left(x_{2}\right) &=\sup _{x \in C} f(x)
    \end{aligned}
    $$
\end{theorem}
\begin{proof}
