...
\subsection{}
\begin{theorem}
    Let $X \subseteq \mathbb{E}^n$, $Y \subseteq \mathbb{E}^m$ and $f : X \rightarrow Y$. Let $\Psi : X \rightarrow  2^Y$ be defined by $\Psi(x) = \{f(x)\}$ for all x ∈ X. Then Ψ is uhc if and only if f is continuous.
\end{theorem}
