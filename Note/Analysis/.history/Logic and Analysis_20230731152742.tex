...
\begin{proof}
    ...
    Let $U_c=B_{\delta(c)}(c)$, then $\{U_c:c\in C\}$ is an open cover of $C$. Since $C$ is compact, there is a finite subcover $\{U_{c_1},...,U_{c_n}\}=\{B_{\delta(c_1)}(c_1),...,B_{\delta(c_n)}(c_n)\}$.

    Let $\delta=\min\{\delta(c_1),...,\delta(c_n)\}$. Given $x,y\in C$ with $d(x,y)<\delta$, note that $x\in U_{c_i}$ for some $i\in\{1,...,n\}$, so $d(x,c_i)<\delta(c_i)$. $d(y,c_i)\leq d(y,x)+d(x,c_i)$
\end{proof}
