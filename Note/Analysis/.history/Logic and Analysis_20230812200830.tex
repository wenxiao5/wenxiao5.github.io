...
\begin{proof}
    ...
    \begin{enumerate}
        ...
        \item Assume $C\subseteq \Psi(C)$ doesn't hold, that is $\exists y\in C$ s.t. $y\notin \Psi(C)$. Because $y\notin \Psi(C)$, $y\notin \Psi(c),\forall c\in C$. Because $C=\cap_{i=0}^\infty C_i$, there exists $N$ s.t. $C\subseteq C_n, \forall n\geq N$.
        \item $\forall y\in C$, $y\in C_n$ for all $n$. We construct a sequence $\{y_n\}$ such that $y_n\in C_n$ and $y_*\in\Psi(y_n)$ for all $n$
    \end{enumerate}
\end{proof}
