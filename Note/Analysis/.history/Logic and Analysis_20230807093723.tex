...
Using this notation, note that $f$ is differentiable at $x$ $\Leftrightarrow$ $\exists T_x\in L(\mathbb{R}^n,\mathbb{R}^m)$ such that $$f(x+h)=f(x)+T_x(h)+o(|h|)\textnormal{ as }h \rightarrow 0$$


- $d f_x$ is the linear transformation $T_x$
- $D f(x)$ is the matrix of $d f_x$ with respect to the standard basis. This is called the Jacobian or Jacobian matrix of $f$ at $x$
- $E_f(h)=f(x+h)-\left(f(x)+d f_x(h)\right)$ is the error term
Using this notation,
$f$ is differentiable at $x \Leftrightarrow E_f(h)=o(h)$ as $h \rightarrow 0$
