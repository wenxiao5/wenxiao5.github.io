...
\section{Compactness \small{(@ Lec 06 of ECON 204)}}
...
\begin{definition}
    \normalfont
    A set $A$ in a metric space is \textbf{compact} if every open cover of $A$ contains a \underline{finite subcover} of $A$. In other words, if $\{U_\lambda : \lambda \in \Lambda\}$ is an open cover of $A$, there exist $n \in N$ and $\lambda_1, \cdots , \lambda_n \in \Lambda$ such that $A\subseteq \cup_{\lambda\in\Lambda}U_\lambda$
\end{definition}
