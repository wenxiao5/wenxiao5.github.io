...
\begin{theorem}
    Let $X\subseteq \mathbb{E}^n$, $Y\subseteq \mathbb{E}^m$, and $\Psi: X \rightarrow 2^Y$.
    \begin{enumerate}[(1).]
        \item If $\Psi$ is \textbf{closed-valued} and \textbf{uhc}, then $\Psi$ has \textbf{closed graph}.
        \item If $\Psi$ has \textbf{closed graph} and there is an \textbf{open set} $W$ with $x_0 \in W$ and a \textbf{compact set} $Z$ such that $x \in W \cap X \Rightarrow \Psi(x) \subseteq Z$, then $\Psi$ is uhc at $x_0$.
    \end{enumerate}
\end{theorem}
