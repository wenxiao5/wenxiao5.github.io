...
\begin{definition}[Limit Point]
    \normalfont $x$ is a limit point of $\left\{x_{k}\right\}$ if $\exists \text { a subsequence of }\left\{x_{k}\right\} \text { that converges to } x$.
\end{definition}

\begin{definition}[Limit Point, Alternative Definition]
    \normalfont
    Let $(X,d)$ be a metric space. A point $x\in X$ is a \underbar{limit point} of a set $A\subseteq X$ if $$\forall \varepsilon>0, B_\varepsilon(x)\cap A\nsubseteq\{x\}$$
\end{definition}
