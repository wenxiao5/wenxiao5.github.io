...
\normalfont
    Let $(X, d)$ and $(Y, \rho)$ be metric spaces. A function $f : X \rightarrow Y$ is continuous at a point $x_0 \in X$ if $\forall \varepsilon > 0, \exists \delta(x0, ε) > 0 s.t. d(x, x0) < δ(x0, ε) ⇒ ρ(f(x), f(x0)) < ε$.
\end{definition}
