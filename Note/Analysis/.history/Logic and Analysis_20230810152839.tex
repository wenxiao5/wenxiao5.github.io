...
\begin{theorem}
    ...
    y^{\prime}=\left(y-y_s\right)^{\prime}=M\left(y-y_s\right)
    $$
    where M is a real $n\times n$ matrix. Suppose that M can be diagonalized over the complex field $\mathbb{C}$. Let $U$ be the standard basis of $\mathbb{R}^n$ and $V = \{v_1, . . . , v_n\}$ be a basis of (complex) eigenvectors corresponding to the eigenvalues $\lambda_1, . . . , \lambda_n \in \mathbb{C}$. Then the solution of the initial value problem is given by
    \begin{equation}
        \begin{aligned}
