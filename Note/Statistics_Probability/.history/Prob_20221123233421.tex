...
Let $X_1,X_2,...$ be an i.i.d. sequence with each $X_i$ taking values $\pm 1$ with probability $\frac{1}{2}$ each. Set $S_0 = 0$ and for any integer $t > 0$, let $S_t = X_1 +\cdots+X_t$. Then, $S_0,S_1,S_2,...$ is a simple symmetric random walk with $\mathbb{E}(S_t) = 0$ and $\textnormal{Var}(S_t) = t$ for $t = 0,1,...$.

As a sum of i.i.d. random variables, for large $t$, $S_t$ is approximately normally distributed by the central limit theorem.
