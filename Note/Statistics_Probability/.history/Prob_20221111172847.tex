...
\begin{center}
    \fcolorbox{black}{green!10}{\parbox{.9\linewidth}{\begin{definition}[Definition 1 of PP]
        ...
        \begin{enumerate}
            \item $N_0=0$.
            \item For all $t>0, N_t$ has a Poisson distribution with parameter $\lambda t$.
...
\textbf{Remark.} The above definition leads to a direct method for simulating a Poisson Process in $[0, t]$.
...
\subsection{Conditioning on the number of arrivals in a Poisson Process}
...
$$
P\left(N\left(J_i\right)=k_i \forall i \in[1: m] \mid N(1)=k\right)=\frac{k !}{k_{1} ! \ldots k_{m} !} \Pi_{i=1}^m\left|J_i\right|^{k_i}
$$
...
Remark 11.6. The above theorem is saying that the distribution of the random vector $\left(N\left(J_1\right), \ldots, N\left(J_m\right)\right)$ is distributed as Multinomial with number of trials $n$ and probabilities $\left(\left|J_1\right|, \ldots,\left|J_m\right|\right)$

Proof. The random variables $N\left(J_i\right)$ are independent Poisson r.v.s with means $\lambda\left|J_i\right|$ by the definition of a Poisson process. Hence, for any nonnegative integers $k_1, \ldots, k_m$ that sum to $\mathrm{k}$
$$
P\left(N\left(J_i\right)=k_i \forall i \in[1: m]\right)=\Pi_{i=1}^m\left(\lambda\left|J_i\right|\right)^{k_i} \frac{\exp \left(-\lambda\left|J_i\right|\right)}{k_{i} !}=\lambda^k \exp (-\lambda) \Pi_{i=1}^m \frac{\left|J_i\right|^{k_i}}{k_{i} !}
$$
Dividing this by
$$
