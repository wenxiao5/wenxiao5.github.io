...
\begin{note}
    \begin{enumerate}
        ...
        \item $P(\emptyset)=0$ because $P(\emptyset)=P(\emptyset^C)-P(\emptyset^C)$.
    \end{enumerate}
\end{note}
\begin{example}
    \textbf{Coin Tossing:} $S=\{H,T\}$ has $\sigma-$algebra $\mathcal{B}=2^S=\{\emptyset,\{H\},\{T\},\{H,T\}\}$ (which is called the "discrete $\sigma-$algebra").
...
\begin{note}
    ...
    If $S$ is (at most) countable, then $\mathcal{B}=2^S$ is a standard choice. It suffices to specify $P(\{s\}), s\in S$.
\end{note}
