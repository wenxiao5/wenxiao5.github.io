...
\begin{claim}
    For standard Brownian motion, $\textnormal{Cov}(W_s, W_t) = \min\{s, t\}$.
\end{claim}
...
\begin{proof}
    ...
    \begin{equation}
        \begin{aligned}
            \textnormal{Cov}(W_s,W_t)=\textnormal{Var}(W_s)&\Leftrightarrow \textnormal{Cov}(W_s,W_t-W_s)=0\\
            &\Leftrightarrow \textnormal{Var}(W_t-W_s)=\textnormal{Var}(W_t)-\textnormal{Var}(W_s)
        \end{aligned}
        \nonumber
    \end{equation}
\end{proof}
...
\begin{lemma}
    If a Gaussian process having continuous paths, mean $0$, and covariance function $\textnormal{Cov}(W_s,W_t) = \textnormal{Var}(W_{\min\{s, t\}})$ is a \textbf{standard Brownian motion}.
\end{lemma}
