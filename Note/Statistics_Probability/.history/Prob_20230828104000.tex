...
\begin{note}
    Any random variable $X$ induces a probability space $(S_X,\mathbb{B}_X,P_X)$, where $S_X=\mathbb{R}$, $\mathbb{B}_X=\mathcal{B}(\mathbb{R})$, and $P_X=P\circ X^{-1}$ (that is $P_X(B)=P\circ X^{-1}(B)\triangleq P(\{s\in S: X(s)\in B\})$)
\end{note}
