...
\begin{proposition}
    The correspondence theorem generalizes to $\mathbb{R}^2$: $\exists$ necessary and sufficient conditions for $F_{X,Y}$ to be a joint cdf.
\end{proposition}


\begin{definition}
    \normalfont
    Let $(X,Y)$ be a bivariate random vector with joint cdf $F_{X,Y}$. $(X,Y)$ is \textbf{discrete} if and only if $\exists f_{X,Y}: \mathbb{R}^2 \rightarrow [0,1]$ such that
    \begin{equation}
        \begin{aligned}
