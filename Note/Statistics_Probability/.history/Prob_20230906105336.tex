...
\begin{proposition}
    If $f(\cdot\mid\epsilon)$ is pdf, then
    \begin{equation}
        \begin{aligned}
            &1=\int_{-\infty}^\infty f(x\mid\epsilon) dx=c(\epsilon)\int_{-\infty}^\infty h(x) e^{\sum_{j=1}^k t_j(x)w_j(\epsilon)} dx\\
            \Rightarrow &\int_{-\infty}^\infty h(x) e^{\sum_{j=1}^k t_j(x)w_j(\epsilon)} dx=c(\epsilon)^{-1}
        \end{aligned}
        \nonumber
    \end{equation}
    The support of $f(\cdot\mid\epsilon)$, \textnormal{supp}($f(\cdot\mid\epsilon))=\{x: f(x\mid\epsilon)> 0\}$ ,does not depend on $\epsilon$
\end{proposition}
...
\begin{theorem}[Jensen's Inequality]\label{Jen_in}
