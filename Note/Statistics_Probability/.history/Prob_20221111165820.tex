...
$$
Hence, $X$ has an exponential distribution with parameter $\lambda$ or mean $1 / \lambda$. Recall that the pdf of the Exponential (1) distribution is given by $f(x)=\exp (-x)$ for $x>0$, and an Exponential with parameter $\lambda$ can always be represented as $\frac{1}{\lambda} \operatorname{Exp}(1)$.
