...
\begin{definition}[Probability Space]
    \normalfont
    A \textbf{probability space} is triple $(S,\mathcal{B},P)$, where $S$ is a \underline{sample space}, $\mathcal{B}$ is a \underline{$\sigma-$algebra} of \underline{events}, and $P$ is a \underline{probability function}.
\end{definition}
...
\begin{definition}[$\sigma-$Algebra of Events]
    ...
    A \underline{collection $\mathcal{B}$ of events} is a \textbf{$\sigma-$algebra} \underline{if and only if}
    \begin{enumerate}
        \item $\emptyset\in \mathcal{B}$;
        \item $B\in \mathcal{B} \Rightarrow B^C \in \mathcal{B}$;
        \item $B_1,B_2,...\in \mathcal{B} \Rightarrow \cup_{i=1}^\infty B_i\in \mathcal{B}$.
    \end{enumerate}
\end{definition}
\begin{note}
    $S=\emptyset^C\in \mathcal{B}$.
\end{note}
\begin{example}
    \textbf{Coin Tossing:} $S=\{H,T\}$ has $\sigma-$algebra $\mathcal{B}=2^S=\{\emptyset,\{H\},\{T\},\{H,T\}\}$ (which is called the "discrete $\sigma-$algebra").
\end{example}

Standard $\sigma-$algebra when $S=\mathbb{R}$:
\begin{definition}[Borel $\sigma-$algebra]
    \normalfont
    The \textbf{Borel} $\sigma-$algebra on $\mathbb{R}$, $\mathbb{B}(\mathbb{R})$, is the smallest $\sigma-$algebra contains all open sets.
\end{definition}

\subsection{Probability Function}
\begin{definition}[Probability Function]
    \normalfont
    A function $P: \mathcal{B} \rightarrow [0,1]$ is a \textbf{probability function} \underline{if and only if}
    \begin{enumerate}
        \item $P(S)=1$;
        \item If $B_1,B_2,...\in \mathcal{B}$ are disjoint event, then $P(\cup_{i=1}^\infty B_i)=\sum_{i=1}^\infty P(B_i)$.
    \end{enumerate}
\end{definition}
\begin{note}
    \begin{enumerate}
        \item $\mathcal{B}$ is the domain of $P$.
        \item $S=\emptyset^C\in \mathcal{B}$ because $\mathcal{B}$ is a $\sigma-$algebra.
...
\begin{note}
    ...
    \begin{enumerate}
        \item We don't specify $\mathcal{B}$ unless we have to.
        \item It suffices to specify $P()$ on a subset of $\mathcal{B}$. (In the example, we can only specify $P(\{H\})$ and $P(\{T\})$).
    \end{enumerate}
\end{note}
\begin{note}
    \textbf{Generalization:}\\
    If $S$ is \underline{(at most) countable}, then $\mathcal{B}=2^S$ is a standard choice. It suffices to specify $P(\{s\}), s\in S$.
\end{note}
...
\begin{definition}[Random Variable]
    \normalfont
    Let $(S,\mathcal{B},P)$ be a probability space. A \textbf{random variable} is a function $X: S \rightarrow \mathbb{R}$, which is "Borel measurable".
\end{definition}
