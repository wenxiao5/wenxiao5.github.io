...
\begin{proposition}
    A process $W$ is Gaussian with mean $0$, and \underline{$\textnormal{Cov}(W_s,W_t)=\textnormal{Var}(W_s),\forall s\leq t$} \textbf{if and only if} $W$ has (1). Independent increments; (2). Stationary normal increments.
\end{proposition}
\begin{proof}
    $\forall s\leq t$, $\textnormal{Cov}(W_s,W_t)=\textnormal{Cov}(W_s,(W_t-W_s)+W_s)=\textnormal{Var}(W_s)+\textnormal{Cov}(W_s,W_t-W_s)$.
    \begin{equation}
        \begin{aligned}
            \textnormal{Cov}(W_s,W_t)&=\textnormal{Var}(W_s)\\
            \textnormal{Cov}(W_s,W_t-W_s)&=0
        \end{aligned}
        \nonumber
    \end{equation}
    $\textnormal{Cov}(W_s,W_t)=\textnormal{Var}(W_s)$ $\Leftrightarrow$ $\textnormal{Cov}(W_s,W_t-W_s)=0$ $\Leftrightarrow$ $\textnormal{Var}(W_t-W_s)=\textnormal{Cov}(W_t,W_t-W_s)-\textnormal{Cov}(W_s,W_t-W_s)$
    
    
    i.e., $W$ has independent increments.
    $\textnormal{Cov}(W_s,W_t)=\textnormal{Var}(W_s)$ $\Leftrightarrow$ $\textnormal{Cov}(W_s,W_t-W_s)=0$ i.e., $W$ has independent increments.\\
\end{proof}
