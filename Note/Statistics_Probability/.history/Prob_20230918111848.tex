...
\begin{definition}[Method of Moments in $\mathbb{R}^k$]
    ...
    \begin{equation}
        \begin{aligned}
            \mu'_j(\hat{\theta}_{MM})=\bar{X}^j=\frac{1}{n}\sum_{i=1}^n X_i^j,\quad (j=1,...,k)
        \end{aligned}
        \nonumber
    \end{equation}
...
\begin{example}\quad
    \begin{enumerate}
        \item Suppose $X_i\sim \textnormal{i.i.d.}N(\mu,\sigma^2)$ where $\mu\in \mathbb{R}$ and $\sigma^2>0$ are unknown. The \underline{moment function} is
        \begin{equation}
            \begin{aligned}
                \mu'_1(\mu,\sigma^2)=\mu\\
