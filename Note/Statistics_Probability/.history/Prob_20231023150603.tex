...
\begin{theorem}[Glivenko-Cantelli Theorem]
    Suppose $X_1,X_2,...$ are i.i.d. drawn from a random distribution with c.d.f. $F(x)$. The \textbf{empirical distribution function }
\end{theorem}
\begin{proof}
    Easily proven by Law of Large numbers. $\frac{1}{n}\sum_{i=1}^{n}\mathbf{1}\{X_i\leq x\} \stackrel{a.s.}{\longrightarrow} \mathbb{E}[\mathbf{1}\{X_i\leq x\}]=\mathbb{P}(X_i\leq x)=F(x)$$
\end{proof}
