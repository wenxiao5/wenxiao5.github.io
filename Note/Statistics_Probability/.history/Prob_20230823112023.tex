...
\begin{definition}[$\sigma-$Algebra of Events]
    \normalfont
    A subset (of $S$) $X\subseteq S$ is called an \textbf{event}.\\
    A \underline{collection $B$ of events} is a \textbf{$\sigma-$algebra} \underline{if and only if}
    \begin{enumerate}
        \item $\emptyset\in B$;
        \item $B\in B \Rightarrow B^C \in B$;
        \item $B_1,B_2,...\in B \Rightarrow \cup_{i=1}^\infty B_i\in B$.
    \end{enumerate}
\end{definition}
\begin{note}
    $S=\emptyset^C\in B$.
\end{note}
\begin{example}
    \textbf{Coin Tossing:} $S=\{H,T\}$ has $\sigma-$algebra $B=2^S=\{\emptyset,\{H\},\{T\},\{H,T\}\}$ (which is called the "discrete $\sigma-$algebra").
\end{example}

\begin{definition}
    \normalfont
    A function $P: B \rightarrow [0,1]$ is a \textbf{probability function} \underline{if and only if}
    \begin{enumerate}
        \item $P(S)=1$;
        \item If $B_1,B_2,...\in B$ are disjoint event, then $P(\cup_{i=1}^\infty B_i)=\sum_{i=1}^\infty P(B_i)$.
    \end{enumerate}
\end{definition}
\begin{note}
    $B$ is the domain of $P$. $S=\empty^C\in B$ because $B$ is a $\sigma-$algebra.
\end{note}
