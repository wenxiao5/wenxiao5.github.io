...
$$y=\beta_0+\beta_1 x$$
where $\beta_0$ is the \textit{intercept}; $\beta_1$ is the \textit{slope}.\\ One Response $\mathcal{Y}$; One Predictor $\mathcal{X}$. $Y$ is a RANDOM VARIABLE that has a distribution for every level of the independent variable. The data come in pairs: $\{(x_i,y_i)\}_{i=1,...,n}$.
